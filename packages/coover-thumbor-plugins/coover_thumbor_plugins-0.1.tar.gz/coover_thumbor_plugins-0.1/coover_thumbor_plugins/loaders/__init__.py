...
__all__ = ['_get_bucket_and_key', '_get_bucket_and_key_and_expiration', '_get_bucket', '_get_key', '_validate_bucket', '_use_http_loader', '_validate']
...
def _get_bucket_and_key_and_expiration(context, url):
    """
    Returns bucket and key from url
    :param Context context: Thumbor's context
    :param string url: The URL to parse
    ...
    bucket = context.config.get('TC_AWS_LOADER_BUCKET')
    if not bucket:
        splitedUrl = url.lstrip('/').split('/')
        bucket = _get_bucket(url)
        url = '/'.join(splitedUrl[1:-1])
        expiration = splitedUrl[-1]

    key = _get_key(url, context)

    return bucket, key, expiration
