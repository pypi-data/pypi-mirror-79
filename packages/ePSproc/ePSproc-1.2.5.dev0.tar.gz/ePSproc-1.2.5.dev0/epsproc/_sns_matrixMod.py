"""Seaborn functions to visualize matrices of data.
...
PH 25/11/19
- Minor edits to imports to use local copy instead of Seaborn package version.
- Added delim option to _index_to_label(index, delim = "-")
...
def _index_to_label(index, delim = ","):
    """Convert a pandas index or multiindex to an axis label."""
    if isinstance(index, pd.MultiIndex):
        return delim.join(map(to_utf8, index.names))
    else:
        return index.name
