...
class SearchAnalytics:
    ...
    @regenerate_credentials
    def concurrent_query_asyncio(self, url: str, detail_level: int = 1):
        """
        ...
        bodies = create_body_list(self.body, detail_level=detail_level)
        extra_bodies = []
        ...
        if len(extra_bodies) >= 1:
            confirm_rows = confirm(
                f"More than 25.000 rows found for {len(extra_bodies)} query, include them /screw them"
            )
            if confirm_rows:
                asyncio.run(
                    main(
