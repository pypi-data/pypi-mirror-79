...
class SourceUndulator(object):
    ...
    def _sample_photon_energy_theta_and_phi(self,NRAYS):
        ...
        if photon_energy_spectrum == 'monochromatic':
            ...
            # print(">>>>THETA: ",theta.min(),theta.max(),sampled_theta.min(),sampled_theta.max())
            # print(">>>>PHI: ", phi.min(), phi.max(), sampled_phi.min(), sampled_phi.max())
            # plot_scatter(sampled_theta,sampled_phi)
