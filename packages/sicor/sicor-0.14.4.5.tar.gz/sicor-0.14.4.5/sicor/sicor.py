...
def get_ecmwf_data(variable, image, options, logger=None):
    """Wrapper for sicor.ECMWF.ECMWF_variable. For a given observation data, go back in time day by day
    ...
    was available.
    :param variable: Name of ECMWF variable, need to be present in data base root directory
    :param image: object derived from sicor.sensors.RSImage
    :param options: dictionary with options, minimum is:
    :param logger: None or instance of logging
    {"ECMWF": {"path_db": [path to ecmwf db], "max_delta_day": [number of days], "target_resolution": 20}}
    :return: numpy array with interpolated variable at given [target_resolution] for [variable] and [image] geo position
    """
    ...
    for delta_day in range(0, options["ECMWF"]["max_delta_day"]):
        try:
            logger.warning("Look %i day back in time to find suitable ECMWF data." % delta_day)
            return ECMWF_variable(variable=variable, path_db=options["ECMWF"]['path_db'],
...
def wv(fo, s2img, clear_areas, s2spr, s2tau, nanmean, sigma, rho_bounds,
    ...
    :param nanmean: TODO
    :param sigma: TODO
    :param rho_bounds: Lower bound for which no retrieval is performed
    :param bands: TODO
        ...
        if xi.shape[0] > 0:
            ...
            if reduce_lut is False:
                ...
                try:
                    ...
                    res = griddata(points=points[sel, :], values=rhos[sel], xi=xi)
...
class IO(object):
        ...
        for opts in options["output"]:
            if opts["type"] == "metadata":
                ...
                IO.write_l2a(granule_path=opts["fn"],
                             ...
                             options_loseless=opts["options_loseless"],
    ...
    def write_l2a(granule_path, band_fns, projection, data=None, uncert=None, msk=None, driver="JP2OpenJPEG",
                  logger=None,
                  options_loseless={"JP2OpenJPEG": ["QUALITY=100", "REVERSIBLE=YES"]}, clear_areas=None,
        ...
        if output_bands is not None and data is not None:
            outputs += [{
                ...
                "options": options_loseless[driver],
