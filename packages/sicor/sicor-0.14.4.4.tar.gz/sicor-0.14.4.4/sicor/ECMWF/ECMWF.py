...
1st: grib_api
download software here: https://software.ecmwf.int/wiki/display/GRIB/Home
...
class ECMWF_variable(object):
    def __init__(self, variable, path_db, var_date, dtype=np.float32, logger=None):
        """
        ...
        :return:
        :raise FileNotFoundError if file is not found in database
        """
        self.logger = logging.getLogger() if logger is None else logger
        self.fn = ECMWF_download.db_path(variable, var_date, path_db)

        if not isfile(self.fn):  # raise if file doesnt exists
            raise FileNotFoundError(self.fn)

        h5f = h5py.File(name=self.fn, mode="r")
        if 'steps' not in h5f.keys():
            self.steps = np.array([0], dtype=dtype)
        else:
            self.steps = np.array(h5f["steps"].value[:], dtype=dtype)
        self.lat = np.array(h5f["lat"].value[:], dtype=dtype)
        self.lon = np.array(h5f["lon"].value[:], dtype=dtype)
        self.variable = np.array(h5f[variable].value[:, :, :], dtype=dtype)
        self.var = RegularGridInterpolator(points=(self.steps, self.lat[::-1], self.lon),
                                           values=self.variable[:, ::-1, :], method="linear")
        h5f.close()
    ...
    def __call__(self, step, lons, lats, shape=None, order=3, dtype=np.float32, force_bounds=True):
        ...
        if step < self.steps[0] or step > self.steps[-1]:
            ...
            if force_bounds is True:  # make sure step is within bounds
                ...
                raise ValueError("Parameter step is our of bounds.")
