...
def read_classical_bayesian_from_hdf5_file(filename):
    ...
    with h5py.File(filename, 'r') as h5f:
        ...
        kwargs_cb = {"bns": [h5f[name].value for name in json.loads(h5f["bns_names"].value)],
                     "hh_full": h5f["hh_full"].value,
                     "hh": {h5f["%s_key" % name].value: h5f["%s_value" % name].value for name in
                            json.loads(h5f["hh_names"].value)},
                     "hh_n": {key: value for key, value in zip(h5f["hh_n_keys"].value, h5f["hh_n_values"].value)},
                     "n_bins": json.loads(h5f["n_bins"].value), "classes": h5f["classes"].value,
                     "n_classes": json.loads(h5f["n_classes"].value),
                     "bb_full": [h5f[name].value for name in json.loads(h5f["bb_full_names"].value)]}
