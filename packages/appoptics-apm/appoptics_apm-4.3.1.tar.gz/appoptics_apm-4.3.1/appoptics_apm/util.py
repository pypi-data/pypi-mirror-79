...
if TEST_MODE:
    ...
    util_logger.error("AppOptics APM Oboe running in APPOPTICS_TEST mode; will not emit reports")
else:
    try:
...
class Context(object):
    ...
    @classmethod
    def start_trace(cls, layer, xtr=None, force=False, url_getter=None):
        ...
        if sample_request(xtr, url_getter) or force:
        ...
        if evt:
            ...
            if sample_source:
                ...
                util_logger.debug("No sample_source attribute found")
            if sample_rate:
                event.add_info('SampleRate', int(sample_rate))
            else:
                util_logger.debug("No sample_rate attribute found")
    ...
    def set_transaction_name(self, trans_name=""):
        ...
        if self.is_valid():
            ...
            util_logger.debug("Transactin name ignored as current execution flow is not monitored by agent")
        return False
    ...
    def end_trace(self, event):  # Reports the last event in a trace
        ...
        if isinstance(event, Event):
            ...
            if http_span:
                event.add_info('Method', getattr(Context.transaction_dict, 'request_method', None))
...
def start_http_trace(
    ...
    ctx, evt = Context.start_trace(layer, xtr=xtr, force=forced_trace, url_getter=url_getter)
...
def _reporter():
    ...
    if not reporter_instance:
        reporter_instance = Reporter(
            ...
            proxy=appoptics_apm.config.get('proxy'),
        )
...
def sample_request(xtr, url_getter):
    ...
    setattr(Context.transaction_dict, 'do_metrics', do_metrics)
...
def report_layer_init(layer='Python', keys=None):
    ...
    try:
        with open(os.path.join(os.path.dirname(__file__), './swig/VERSION')) as version_file:
            ver_keys['Python.AppOpticsExtension.Version'] = version_file.read().rstrip('\n')
    except Exception as e:
        ver_keys['Python.AppOpticsExtension.Version'] = "Unknown"
