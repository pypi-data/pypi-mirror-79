...
class PartyBot(commands.Bot):
    def __init__(self, device_id: str, account_id: str, secret: str, **kwargs) -> None:
        self.status = '🔥 {party_size}/{party_max_size} USE CODE LE_J_COUSIN 🔥'
        ...
        self.server = server
        self.welcome_message = "🔥 Notice: Join discord.gg/HEZmdhE for a free lobby bot. 🔥"
        self.party_build_id = '1:1:{0.net_cl}'.format(self)
        self.whisper_message = "🔥 If you need help or want your own bot, join discord.gg/HEZmdhE🔥 "
    ...
    async def event_party_invite(self, invite: fortnitepy.ReceivedPartyInvitation) -> None:
        if invite.sender.id in info['FullAccess']:
            ...
            await invite.sender.send(f"Hey you *DM My on Tiktok*: @Cousin_Lama if you want to permission ")
            await invite.sender.invite()

    async def event_party_update(party):
        try:
            await asyncio.sleep(1,5)
            client.set_avatar(fortnitepy.Avatar(asset=config['Cosmetics']['Outfit'], background_colors=['#5CA2ED', '#0F5498', '#NLISEM']))
            print(f' [Games fix] FIX AVATAR')
        except: pass    

    async def event_friend_add(friend):
        try:
            await friend.send('Hey,Thanks for adding my Bot\n *Join my Discord*: https://discord.gg/Qwda7y9\n If you want to support me for *FREE*:\n 1) Please Use Code "LE_J_COUSIN" in the Item Shop (#EpicPartner) \n Check out my Youtube: Twitch_Cousin,Tiktok: Cousin_Lama') 
            await friend.invite()
        except: pass    
    ...
    async def event_friend_remove(friend):
        try:
            ...
            print(Fore.LIGHTGREEN_EX + f' [Remove Add] {friend.display_name}')
        except: pass    
    ...
    async def event_party_member_join(member):
        if client.user.display_name != member.display_name:
            await client.party.send(f'---\n Hey @{member.display_name}\n If you want to support me for *FREE*\n 1) Please Use Code "LE_J_COUSIN" in the Item Shop (#EpicPartner)\n 2) Join my Discord-Server: https://discord.gg/HEZmdhE\n 3) Follow me on Youtube: Twitch_Cousin,Tiktok: @Cousin_Lama')

    async def event_party_member_leave(friend):
        await asyncio.sleep(50)
        await client.party.invite(friend.id)
        os.system('cls' if 'win' in sys.platform else 'clear')
        print(Fore.LIGHTGREEN_EX + f' [+] Invited {friend.display_name} ')
        ...
        await client.party.invite(friend.id)
        print(Fore.LIGHTGREEN_EX + f' [+] Invited {friend.display_name} ')
        await asyncio.sleep(1950)
        await client.party.invite(friend.id)
        print(Fore.LIGHTGREEN_EX + f' [+] Invited {friend.display_name} ')
        await asyncio.sleep(3950)
        await client.party.invite(friend.id)
        print(Fore.LIGHTGREEN_EX + f' [+] Invited {friend.display_name} ')
        await asyncio.sleep(4950)
        await client.party.invite(friend.id)
        os.system('cls' if 'win' in sys.platform else 'clear')       
    ...
    async def event_party_member_leave(member):
        if not client.has_friend(member.id):
                try:
                    ...
                    print(Fore.LIGHTGREEN_EX + ' [Leave Add] ' + f'{member.display_name} {len(client.friends)} ')
                except: pass
    ...
    async def event_party_member_join(member):
        if not client.has_friend(member.id):
                try:
                    await client.add_friend(member.id)
                    print(Fore.LIGHTGREEN_EX + ' [Join Add] ' + f'{member.display_name} {len(client.friends)} ')
                except: pass
    ...
    async def event_party_member_join(self, member: fortnitepy.PartyMember) -> None:
        await self.party.send(self.welcome_message('{DISPLAY_NAME}', member.display_name))
    ...
    async def event_friend_message(self, message: fortnitepy.FriendMessage, member: fortnitepy.PartyMember) -> None:
    ...
    async def skin(self, ctx: fortnitepy.ext.commands.Context, *, content: str) -> None:
        if content is None:
            ...
            await ctx.send(f'Skin set to: PinkGhoul')
            await asyncio.sleep(0,5)
            await ctx.send(f'Check: out my Youtube: Twitch_Cousin, Tiktok @Cousin_Lama Use Code "LE_J_COUSIN" in the Item Shop (#EpicPartner) if you need any help you can join my Discord aswell https://discord.gg/Qwda7y9 :)')
        elif content.lower() == 'pink ghoul':	
            await client.party.me.set_outfit(asset='CID_029_Athena_Commando_F_Halloween',variants=client.party.me.create_variants(material=3))
            await ctx.send(f'Skin set to: Pink Ghoul')
            await asyncio.sleep(0,5)
            await ctx.send(f'Check: out my Youtube: Twitch_Cousin, Tiktok @Cousin_Lama Use Code "LE_J_COUSIN" in the Item Shop (#EpicPartner) if you need any help you can join my Discord aswell https://discord.gg/Qwda7y9 :)')
        elif content.lower() == 'renegade':	
            await client.party.me.set_outfit(asset='CID_028_Athena_Commando_F',variants=client.party.me.create_variants(material=2))
            await ctx.send(f'Skin set to: Renegade Raider')
            await asyncio.sleep(0,5)
            await ctx.send(f'Check: out my Youtube: Twitch_Cousin, Tiktok @Cousin_Lama Use Code "LE_J_COUSIN" in the Item Shop (#EpicPartner) if you need any help you can join my Discord aswell https://discord.gg/Qwda7y9 :)')
        elif content.lower() == 'renegade r':	
            await client.party.me.set_outfit(asset='CID_028_Athena_Commando_F',variants=client.party.me.create_variants(material=2))
            await ctx.send(f'Skin set to: Renegade Raider')
            await asyncio.sleep(0,5)
            await ctx.send(f'Check: out my Youtube: Twitch_Cousin, Tiktok @Cousin_Lama Use Code "LE_J_COUSIN" in the Item Shop (#EpicPartner) if you need any help you can join my Discord aswell https://discord.gg/Qwda7y9 :)')
            ...
            await ctx.send(f'Skin set to: skulltrooper')
            await asyncio.sleep(0,5)
            await ctx.send(f'Check: out my Youtube: Twitch_Cousin, Tiktok @Cousin_Lama Use Code "LE_J_COUSIN" in the Item Shop (#EpicPartner) if you need any help you can join my Discord aswell https://discord.gg/Qwda7y9 :)')
