...
class BaseLog:
    ...
    def generate_loggers(self, when, backup_count):
        ...
        for level in self.log_levels:
            ...
            handler_dict[level]['level'] = level.upper() if level != 'trace' else 'NOTSET'
            if level == 'info':
        ...
        for logger in loggers_dict.keys():
            if logger == self.job_log_name:
                loggers_dict[logger]['level'] = 'NOTSET'
            else:
    ...
    def trace(self, *args, **kwargs):
        message = self.trace_model('TRACE', kwargs)
