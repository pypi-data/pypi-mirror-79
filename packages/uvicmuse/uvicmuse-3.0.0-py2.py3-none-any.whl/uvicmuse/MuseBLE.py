...
class MuseBLE(object):
    ...
    async def _write_command(self, command):
        await self.client.write_gatt_char(constants.MUSE_CONTROL_HANDLE, bytearray(command), False)

    # _subscribe functions

    async def _subscribe_control(self):
        await self._subscribe_to_uuid(constants.MUSE_CONTROL_HANDLE, self._handle_control)

    async def _subscribe_eeg(self):
        await self._subscribe_to_uuid(uuid=constants.MUSE_GATT_ATTR_TP9, callback=self._handle_eeg)
        await self._subscribe_to_uuid(uuid=constants.MUSE_GATT_ATTR_AF7, callback=self._handle_eeg)
        await self._subscribe_to_uuid(uuid=constants.MUSE_GATT_ATTR_AF8, callback=self._handle_eeg)
        await self._subscribe_to_uuid(uuid=constants.MUSE_GATT_ATTR_TP10, callback=self._handle_eeg)
        await self._subscribe_to_uuid(uuid=constants.MUSE_GATT_ATTR_RIGHTAUX, callback=self._handle_eeg)

    async def _subscribe_ppg(self):
        try:
            await self._subscribe_to_uuid(uuid=constants.MUSE_GATT_ATTR_PPG1, callback=self._handle_ppg)
            await self._subscribe_to_uuid(uuid=constants.MUSE_GATT_ATTR_PPG2, callback=self._handle_ppg)
            await self._subscribe_to_uuid(uuid=constants.MUSE_GATT_ATTR_PPG3, callback=self._handle_ppg)
        except BleakError:
            raise ppg_error

    async def _subscribe_gyro(self):
        await self._subscribe_to_uuid(uuid=constants.MUSE_GATT_ATTR_GYRO, callback=self._handle_gyro)

    async def _subscribe_acc(self):
        await self._subscribe_to_uuid(uuid=constants.MUSE_GATT_ATTR_ACCELEROMETER, callback=self._handle_acc)

    async def _subscribe_telemetry(self):
        # Handles telemetry (Battery, temp, ...) incoming data
        await self._subscribe_to_uuid(uuid=constants.MUSE_GATT_ATTR_TELEMETRY, callback=self._handle_tele)
    ...
    def _init_timestamp_correction(self):
        ...
        self.reg_params = np.array(
            [time(), 1. / constants.MUSE_SAMPLING_EEG_RATE])
        self.reg_ppg_sample_rate = np.array(
            [time(), 1. / constants.MUSE_SAMPLING_PPG_RATE])

    # Handlers
    def _handle_control(self, sender, packet):
        # Handles the message coming from the control sequence
        assert str(sender).__eq__(constants.MUSE_GATT_ATTR_CONTROL.upper()), "_handle_control is receiving a message " \
    ...
    def _handle_ppg(self, sender, data):
        ...
        if handle == 62:
            ...
            indices = np.arange(0, constants.LSL_PPG_CHUNK) + self.sample_index_ppg
            self.sample_index_ppg += constants.LSL_PPG_CHUNK
    ...
    def _handle_acc(self, sender, packet):
        ...
        if str(sender).lower() != constants.MUSE_GATT_ATTR_ACCELEROMETER.lower():
            return
        timestamps = [time()] * 3
        ...
        packet_index, samples = self._unpack_imu_channel(
            packet, scale=constants.MUSE_ACCELEROMETER_SCALE_FACTOR)
    ...
    def _handle_gyro(self, sender, packet):
        ...
        sampling rate: ~17 x second (3 samples in each message, roughly 50Hz)"""
        if str(sender).lower() != constants.MUSE_GATT_ATTR_GYRO.lower():
            return
        ...
        packet_index, samples = self._unpack_imu_channel(
            packet, scale=constants.MUSE_GYRO_SCALE_FACTOR)
    ...
    def _handle_tele(self, sender, packet):
        ...
        if str(sender).lower() != constants.MUSE_GATT_ATTR_TELEMETRY.lower():
            return
        timestamp = time()
