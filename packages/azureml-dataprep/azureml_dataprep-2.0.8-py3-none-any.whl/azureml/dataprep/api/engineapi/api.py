...
class EngineAPI:
    ...
    @update_aml_env_vars(get_engine_api)
    def shutdown_engine(self, message_args: int, cancellation_token: CancellationToken = None) -> None:
        response = self._message_channel.send_message('Engine.Shutdown', message_args, cancellation_token)
        return response
