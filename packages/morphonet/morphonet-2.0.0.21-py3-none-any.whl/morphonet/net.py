...
class Net:
    ...
    def listDataSet(self): #LIST ALL  DATASET 
        if self._isConnected():
            ...
            self._showDataSets(data)
    def shareDatasetWithUser(self,id_user,how): #SHARE A DATASET with USER (how = 0:reader , 1:manager )
        if self._ownDataSet():
    ...
    def shareDatasetWithGroup(self,id_group,how): #SHARE A DATASET with GROUP
        if self._ownDataSet():
            ...
            if not ok: 
                ...
                print(ss+"your share is created (with id "+str(data['id'])+')')
    def createDataSet(self,dataname,minTime=0,maxTime=0,id_NCBI=0,id_type=0,spf=-1,dt=-1,serveradress=None): #CREATE A NEW DATA SET
        self.id_NCBI=id_NCBI
        self.id_type=id_type
        if self._isConnected():
            data=self._request({"createdataset":dataname,"minTime":minTime,"maxTime":maxTime,"serveradress":server,"id_NCBI":self.id_NCBI,"id_type":self.id_type,"spf":spf,"dt":dt},'/api/createdatasetapi/','POST')
    ...
    def deleteInfosById(self,id_infos):
        if self._isDataSet():
            data=self._request({"deletecorrespondenceid":self.id_dataset,"idinfos":id_infos},'/api/deleteinfoidapi/','POST')
            ids,ok=tryParseInt(data)
            if not ok: 
                print(strred(' ERROR : '+str(id_infos)+' '+str(data)))
            else :
                print(ss+"Infos with id "+str(ids)+" deleted")
    def getInfosByName(self,infos):
        if self._isDataSet():
            ...
            return infos
    def getInfosById(self,id_infos):
        if self._isDataSet():
            data=self.binary_request({"getinfosid":self.id_dataset,"idinfos":id_infos},'/api/getinfoidapi/','GET')
            #infos=bz2.decompress(data['info'].decode('utf-8'))
            infos=bz2.decompress(data)
            return infos
        ...
        for line in infos:
            if len(line)>0 and line[0]!="#":
                if line.find("type")==0:
                    ...
                    objects[tab[0]]=tab[1]
        return objects
    def getObjectsFromInfosId(self,id_infos):
        infos=self.getInfosById(id_infos)
        return self._getObjects(infos)
    def getObjectsFromInfos(self,ninfos):
        infos=self.getInfosByName(ninfos)
        return self._getObjects(infos)

    def shareInfosWithUser(self,id_infos,id_user,how): #SHARE A INFO with USER (how = 0:reader , 1:manager )
        if self._ownDataSet():
            data=self._request({"shareinfo":self.id_dataset,"id_user":id_user,"idinfos":id_infos,"how":how},'/api/shareuserapi/','POST')
            ids,ok=tryParseInt(data['id'])
            if not ok: 
                print(strred(' ERROR : Share not created '+str(data['id'])))
            else :
                print(ss+"your share is created (with id "+str(data['id'])+')')
    def unshareInfosWithUser(self,id_infos,id_user,how): #UNSHARE A INFO with USER
        if self._ownDataSet():
            data=self._request({"shareinfo":self.id_dataset,"id_user":id_user,"idinfos":id_infos,"how":how},'/api/unshareuserapi/','POST')
            if data['status'] == "failed": 
                print(strred(' ERROR : Share not deleted'))
            else :
                print(ss+"your share is deleted")
    def unshareInfosWithGroup(self,id_infos,id_group,how): #SHARE A INFO with GROUP
        if self._ownDataSet():
            data=self._request({"shareinfo":self.id_dataset,"id_group":id_group,"idinfos":id_infos,"how":how},'/api/unsharegroupapi/','POST')
            if data['status'] == "failed": 
                print(strred(' ERROR : Share not deleted'))
            else :
                print(ss+"your share is deleted")
    def shareInfosWithGroup(self,id_infos,id_group,how): #SHARE A INFO with GROUP
        if self._ownDataSet():
            data=self._request({"shareinfo":self.id_dataset,"id_group":id_group,"idinfos":id_infos,"how":how},'/api/sharegroupapi/','POST')
            ids,ok=tryParseInt(data['id'])
            if not ok: 
                print(strred(' ERROR : Share not created '+str(data['id'])))
            else :
