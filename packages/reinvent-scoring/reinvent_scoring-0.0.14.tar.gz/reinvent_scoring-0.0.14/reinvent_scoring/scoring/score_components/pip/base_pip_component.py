...
class BasePiPComponent(BaseScoreComponent):
    ...
    def _calculate_score(self, smiles: List[str]) -> np.array:
        ...
        response = loop.run_until_complete(self._post_batch(self._request_url, smiles))
    ...
    def _create_url(self, component_name) -> str:
        request_url = f"https://pip.pre.dmta.astrazeneca.net/seldon/pip-prod/{component_name}/api/v0.1/predictions"
        return request_url

    async def _post_request(self, request_url, data) -> str:
        async with aiohttp.ClientSession() as session:
            async with session.post(request_url, json=data) as response:
                return await response.json()

    async def _post_batch(self, url, smiles):
        data = self._format_data(smiles)
        result = await self._post_request(url, data)
        return result
