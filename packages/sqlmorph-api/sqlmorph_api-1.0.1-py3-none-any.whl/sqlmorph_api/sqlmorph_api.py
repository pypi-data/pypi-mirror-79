...
def createFiles(results, input, target):
    fixedInput = input.strip("/")
    directory = "%s.%s/" % (fixedInput, target)

    for result in results:
        length = len(fixedInput)+1
        filename = result["name"].split('/')[-1:][0]
        filepath = directory + result["name"][length:-len(filename)]
...
def makeRequests(baseUrl, files, source, target, authentication):
    requests = []
    for myFile in files:
...
def getConvertRequestResponse(baseUrl, data, headers):
    
    response = request.urlopen(myRequest)
    return response.read().decode('utf-8')

def handleResponses(baseUrl, responses, authentication):
    ...
    while responsesToProcess:
        ...
        results.append({**responseToProcess, "result": queries})
...
def runner(runnerArgs):
    ...
    requiredParams.add_argument('--source', required=True, type=str, help="Source dialect", choices=["mssql", "hana", "teradata", "oracle", "impala"], dest="source")
    requiredParams.add_argument('--target', required=True, type=str, help="Target dialect", choices=["impala", "snowflake"], dest="target")
    ...
    responses = makeRequests(baseUrl, files, args.source, args.target, args.auth)
    endRequests = time.time()
    startResponses = time.time()
    results, errors = handleResponses(baseUrl, responses, args.auth)
    endResponses = time.time()

    if not isDir:
        print('\n'.join(results[0]["result"]))
    else:
        createFiles(results, args.input, args.target)
