...
def apply_func(func,xarrays,dims_apply, method_dims_no_apply='outer',filenames_out = None, attributes = None,maximum_input_memory_chars = 1024*1024*1024 ,squeeze_apply_dims = False,release=False):
            ...
            if attributes is not None:
                for attribute_key,attribute_value in attributes[incout].items():
                    if attribute_key is not 'variable':
                        ncout[variables_out[incout]].setncattr(attribute_key,attribute_value)
        ...
        for ixarray_out,xarray_out_transposed in enumerate(xarrays_out_transposed):
             ...
             if np.any(np.array(pivot[1:]) <  np.array(pivot[:-1])):
                 ...
                 if filenames_out is not None:
                    ...
                    fnout = filenames_out[ixarray_out] #'testing_'+str(iarray)+'.nc'
                    os.system('mv '+fnout+' '+fnout_temp+'_temp')
                    xarrays_out_transposed_temp = xr.open_dataset(fnout+'_temp')[variables_out[ixarray_out]]
                    xarrays_out_transposed_temp.to_netcdf(fnout)
                    xarrays_out_transposed_temp.close()
                    os.system('rm '+fnout_temp+'_temp')
                    if not release:
...
def apply_func_per_group(groups,func,xarrays,dims_apply, method_dims_no_apply='outer', maximum_input_memory_chars = 1024*1024*1024 ):
    if type(xarrays).__name__ != 'tuple':
        xarrays = (xarrays,)
    ...
    global_dimsout = {}
    first = True
    for dimsout,seldimsin in groups:
        #import pdb; pdb.set_trace()
