...
class ModelServerClient(base_client.BaseClient):
    def new_model_server(
        self, server_name, path, type, cluster_name, docker_image=None, owner=None
    ):
    ...
    def update_model_server(self, server_name, path, docker_image=None, owner=None):
    ...
    def renew_model_server_token(self, server_name, owner=None):
        ...
        """
        owner = owner or self.owner
        url = url_path_join(MODEL_SERVER_URL, owner, server_name, "renew-token")
        r = self.request("post", url)
        self.check_and_raise(r)
        return self.get_json(r)["model_server"]
