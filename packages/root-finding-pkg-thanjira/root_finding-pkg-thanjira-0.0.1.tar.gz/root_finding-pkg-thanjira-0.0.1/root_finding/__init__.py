...
def secant(f,x1, x2,tolerance=10 ** -5,maxiter =1000 ,plot1 = False):
   ...
   function : The function f should be unimodal on the interval [x1,x2] 
    ...
    if f(x1)*f(x2) >0 :
        xnew = 0
        print("error : Root is not found on this interval.")
...
def newtonND(f, Jac,x0, tolerance = 1.e-6, maxiter = 100):
    '''
    ...
    f : function
        ...
        which recommended define by the definition of array vector function.
        Eg :  def f(x):
    ...
    jac : ndarray
         ...
         which recommended define by the python lambda or definition of array vetor.
         Eg :
             def jac(x):
    ...
    maxiter : int
        An integer specifying the maximum step required on running one process 
        which default value is assigned to 100 iterates.
    ...
    for i in range (maxiter):
        ...
        if (np.sqrt(np.linalg.norm(xnew-x0)) <= tol):
            i +=1
            return xnew
        x0 = xnew
        if i == maxiter-1:
...
def quasiNewton_bfgs(f, fprime,x0, maxiter=200, tolerance=10e-6):
    """
    ...
    fprime : function
        The derivative function of initial function, the gradient of `func`.
    ...
    maxiter : int
        An integer specifying the maximum step required on running one process 
        which default value is assigned to 200 iterates.
