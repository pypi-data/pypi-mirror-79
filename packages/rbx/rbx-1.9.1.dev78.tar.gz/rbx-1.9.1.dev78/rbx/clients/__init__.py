...
class Client:
    ...
    def _request(self, method, path, data=None, headers=None):
        ...
        try:
            ...
            raise ClientException(message=str(e), status_code=500, url=url)
        ...
        if response.status_code == 401:
            self.token = None
            raise ClientException(self.get_message(response),
                                  details=payload,
                                  status_code=401,
                                  url=url)
        ...
        try:
            ...
            raise ClientException('Invalid JSON response', details=extra, status_code=500, url=url)
    ...
    def request(self, method, path, data=None, headers=None):
        ...
        try:
            return self._request(method=method, path=path, data=data, headers=headers)
