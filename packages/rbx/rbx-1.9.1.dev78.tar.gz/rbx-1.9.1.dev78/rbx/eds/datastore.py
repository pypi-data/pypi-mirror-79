...
from ..core.exceptions import Invalid, TransientException
...
class Datastore:
    ...
    def _get_schema_for_kind(self, kind):
        ...
        Raise an Invalid Exception if not found.
        """
        match = [schema for schema in self.schema.values() if schema.name == kind]
        if match:
            return match[0]

        raise Invalid('No schema defined <Kind: {}>'.format(kind))
    ...
    def ancestor(self, kind, payload=None, **kwargs):
        ...
        if not schema.ancestor:
            raise Invalid('No ancestor defined <Schema: {}>'.format(schema))
    ...
    def relatives(self, kind, payload=None, **kwargs):
        ...
        if type(schema.relatives) not in (list, tuple):
            raise Invalid('No relatives defined <Schema: {}>'.format(schema))
