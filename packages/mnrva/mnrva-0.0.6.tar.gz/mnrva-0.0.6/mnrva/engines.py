...
class Redis(object):
	def __init__(self):
		self.r = redis.Redis(host=os.environ.get('REDIS_HOST'), port=os.environ.get('REDIS_PORT'), db=0, decode_responses=True)
...
class Elastic(object):
	...
	def insertData(self, index, id_doc, data, tmout="120s"):
		result = self.es.index(index=index, doc_type='_doc', id=id_doc, body=data, timeout=tmout)
		try:
