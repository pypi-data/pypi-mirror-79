...
from schedule.SyncTaskExecutor import SyncTaskExecutor

sync_task_executor = SyncTaskExecutor()

def stop_job(job_id):
    sync_task_executor.stop_job(job_id)  #有可能是同步执行的，尝试停止
...
def stop_task(job_id, task_id, is_executing: bool):
    ...
    :return:
    """
    sync_task_executor.stop_task(job_id, task_id)  #有可能是同步执行的，尝试停止
    task_thread = TaskThreadHolder.load_task_thread(job_id)
...
def handle_trigger(task_param, is_sync: bool = False):
    ...
    if is_sync:
        ...
        # 同步执行，不能直接在当前线程执行，如果是云调度当前线程是接收任务的线程，停止任务就不能把线程杀了
        return sync_task_executor.run_sync(task_thread, task_param)
    else:
        # 异步执行
