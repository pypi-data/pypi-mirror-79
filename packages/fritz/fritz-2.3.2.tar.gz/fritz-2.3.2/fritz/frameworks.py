...
class SnapMLFile(FrameworkFileBase):
    ...
    def __init__(self, model, is_byte_representation=False):
        super().__init__(SNAPML, model, is_byte_representation)

    def to_bytes(self):
        serialized = self.model.SerializeToString()
        return io.BytesIO(serialized)

    @classmethod
    def build_from_file(cls, path, skip_model_cls_init=False):
        raw = open(path, "rb").read()
        # pylint: disable=no-member
        model = tf.compat.v1.GraphDef.FromString(raw)
        return cls(model)
