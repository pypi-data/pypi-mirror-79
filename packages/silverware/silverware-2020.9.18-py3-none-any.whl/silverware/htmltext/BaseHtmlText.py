...
class BaseHtmlText:
	def __init__(self, obj, _id, extract, ignore_function=None, depth=0, ids=None, log=None, num_jobs=1):
		"""
		...
		self.parse(tag=obj, extract=extract)
	...
	def parse(self, tag, extract):
		tag_children = get_children(tag, ignore_single_children=False, extract=extract)
		...
		if len(tag_children) == 0:
			...
			self.parse_children(tag_children=tag_children, extract=extract)
	...
	def _get_tag_and_child(self, tag, num_jobs, child_id, extract):
	...
	def get_tags_and_children(self, tag_children, extract):
		...
		if self._num_jobs < 2:
			tags_and_children = [
				self._get_tag_and_child(tag=_tag, num_jobs=1, child_id=_child_id, extract=extract)
			...
			tags_and_children = processor(
				delayed(self._get_tag_and_child)(tag=_tag, num_jobs=int(_num_jobs), child_id=_child_id, extract=extract)
				for _tag, _num_jobs, _child_id in tags_and_num_jobs_and_ids
			)

		return tags_and_children

	def parse_children(self, tag_children, extract):

		tags_and_children = self.get_tags_and_children(tag_children=tag_children, extract=extract)
