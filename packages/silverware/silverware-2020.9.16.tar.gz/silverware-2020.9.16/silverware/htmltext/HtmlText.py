...
class HtmlText(BaseHtmlText):
	def __init__(self, obj, ignore_function=None, log=False):
		super().__init__(obj=obj, ignore_function=ignore_function, depth=0, _ids=None, log=log)
		self._data = None
		self._font_size_data = None
		self._font_sizes = None
		self._full_data = None

	@classmethod
	def _get_state_attribute_names(cls):
		return ['_data', '_font_size_data', '_font_sizes', '_full_data', '_logs', '_ids'] + super()._get_state_attribute_names()

	def __setstate__(self, state):
		super().__setstate__(state=state)
		self._set_logs(logs=self._logs)
		self._set_ids(_ids=self._ids)
	...
	def font_size_data(self):
		"""
		produces a aggregate data of font sizes
		:rtype: DataFrame
		"""
		if self._font_size_data is None:

			group_columns = [col for col in ['font_weight', 'font_family', 'font_size'] if col in self.data.columns]
			data = self.data[['num_characters', 'num_strings'] + group_columns]
			data['count'] = 1
			...
			self._font_size_data = df.rename(columns={'count_sum': 'count'}).drop(columns=['count_median'])
		return self._font_size_data
	...
	def full_data(self):
		...
		if self._full_data is None:
			self._full_data = self.data.merge(right=self.font_size_data)
