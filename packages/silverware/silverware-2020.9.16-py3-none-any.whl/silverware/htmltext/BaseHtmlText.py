...
class BaseHtmlText:
	def __init__(self, obj, ignore_function=None, depth=0, _ids=None, log=None):
		"""
	...
	def _get_state_attribute_names(cls):
		return [
			'_children', '_string', '_num_strings', '_font_size', '_style', '_complete_style', '_production_depth',
			'_index', '_id'
		]

	@classmethod
	def _get_empty_attribute_names(cls):
		return ['_tag', '_parent', '_ignore_function', '_logs', '_ids']

	def __getstate__(self):
		return {key: getattr(self, key) for key in self._get_state_attribute_names()}
	...
	def __setstate__(self, state):
		...
		for key in self._get_empty_attribute_names():
