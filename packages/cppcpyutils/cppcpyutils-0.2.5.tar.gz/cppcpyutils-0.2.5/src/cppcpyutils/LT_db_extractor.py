...
def options():
    ...
    parser.add_argument("-i", "--frameid", help="image frame # to download. Argument accepts multiple frames delimited by space. Image frames start at 1.",
...
def main():
    ...
    if args.frameid is None:
        frameid = tuple(range(0,100))# HOpefully this is enough to get all of them ?! how to get all numbers??
        # CAST(frame as CHAR) LIKE '%' -- could use this to match as a character
    else:
        frameid = tuple(args.frameid)  #-- str(args.frameid) to match a single value as a character with LIKE. how to match multiple??
