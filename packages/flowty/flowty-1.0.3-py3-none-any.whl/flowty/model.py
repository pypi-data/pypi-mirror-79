...
class Model:
    ...
    def setObjective(self, expr: Union[LinExpr, Tuple[str, LinExpr]]):
        ...
        checked(
            libFlowty.FLWT_Model_setObjective(
                self.__model, objExpr._LinExpr__linExpr, sense
            )
        )
    ...
    def setParam(self, key: str, value: Union[str, int, float]) -> None:
        ...
        |-|-|-|-|
        | `Algorithm` | `str` | What algorithm to use. Options are `PathMIP` for the path MIP algorithm, `MIP` for the branch-and-cut algorithm [COIN Cbc](https://www.coin-or.org), and `DP` for the dynamic programming algorithm. | `PathMIP` |
