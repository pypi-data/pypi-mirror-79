...
class Session:
    ...
    def run_plan(self, request: TestPlanRunRequest) -> HttpLikeTestPlanRunResponse:
        ...
        # PDI-531: preserving sdkVersion causes all camel_case keys to become camelCase, so batch_id/batch_name fail
        request_with_casing_fix = request.to_dict(casing=Casing.SNAKE)
        if request_with_casing_fix["testplan"] and request_with_casing_fix["testplan"]["sdk_version"]:
            request_with_casing_fix["testplan"]["sdkVersion"] = request_with_casing_fix["testplan"]["sdk_version"]
            del request_with_casing_fix["testplan"]["sdk_version"]

        dict_response: Dict = self._request(topic, request_with_casing_fix, {QOS: 1, TIMEOUT: 60 * 1000})
