...
MODE_MARKER_PATTERN = '#\\s*lib3to6:\\s*(?P<mode>disabled|enabled)'
MODE_MARKER_RE = re.compile(MODE_MARKER_PATTERN, flags=re.MULTILINE)
ModuleHeader = typ.NamedTuple('ModuleHeader', [('coding', str), ('text', str)])


def _parse_header_line(line_data, coding):
    if isinstance(line_data, bytes):
        return line_data.decode(coding)
    if isinstance(line_data, str):
        return line_data
    bad_type = type(line_data)
    errmsg = "Invalid type: line_data must be str/bytes but was '{0}'".format(
        bad_type)
...
def get_available_classes(module, clazz):
    ...
    return {normalize_name(attr_name): attr for attr_name, attr in
        maybe_classes.items() if isinstance(attr, type) and issubclass(attr,
        clazz)}
