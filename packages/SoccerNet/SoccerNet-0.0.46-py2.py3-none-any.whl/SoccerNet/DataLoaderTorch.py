...
def video2clips(video, stride, clip_len):
    ...
    idx = torch.arange(start=start_idx, end=end_idx +
                       math.ceil(clip_len/2), step=stride).long()
    idxs = []
...
class ClipDataset(torch.utils.data.Dataset):
    def __init__(self, video_path, FPS_desired, clip_len):
        ...
        for i_frame, frame in tqdm(enumerate(videodata), total=numframe):
            ...
            off_side = int((frame.shape[1] - 112)/2)
        ...
        self.clips = video2clips(vframes, stride=stride, clip_len=16)
