...
def select_runs(expression: str, repo_path: Optional[str] = None):
    if repo_path is not None:
        repo = AimRepo(repo_full_path=repo_path)
    else:
        repo = AimRepo.get_working_repo(mode=AimRepo.READING_MODE)
    ...
    parser = Expression()
    parsed_expr = parser.parse(expression.strip())

    if 'run.archived' not in parsed_expr:
        default_expression = 'run.archived is not True'
    else:
        default_expression = None

    return repo.select_runs(parsed_expr.node, default_expression)
