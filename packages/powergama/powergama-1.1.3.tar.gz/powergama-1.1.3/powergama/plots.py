...
#                    draw_par_mer=False,showTitle=True, colors=True
    ...
    branch = branch.merge(node[['id','lat','lon']],how='left',
                            left_on='node_to',right_on='id')
    dcbranch = dcbranch.merge(node[['id','lat','lon']],how='left',
                            left_on='node_from',right_on='id')
    dcbranch = dcbranch.merge(node[['id','lat','lon']],how='left',
                            left_on='node_to',right_on='id')
    generator = generator.merge(node[['id','lat','lon']],how='left',
                            left_on='node',right_on='id')
    consumer = consumer.merge(node[['id','lat','lon']],how='left',
                            left_on='node',right_on='id')
    gentypes = pg_data.getAllGeneratorTypes()
    areas = pg_data.getAllAreas()
    node = node.reset_index().rename(columns={'index':'index_orig'})
    node = node.merge(pd.DataFrame(areas).reset_index(),how='left',
                      left_on='area',right_on=0).rename(
    ...
    for i,n in node.iterrows():
        if not (n[['lat','lon']].isnull().any()):
            data = [n['lat'],n['lon'],
                    "Node={}, area={}".format(n['id'],n['area'])]
            if pg_res is not None:
                data[2] = "{}; nodalprice={:g}".format(
                        data[2],n['nodalprice'])
...
#        value_col=None
    ...
    FeatureCollection(locationsBdc,callback=callbackBranch,
...
class FeatureCollection(folium.map.FeatureGroup):
    ...
    def __init__(self, data, callback, addto, colour,
        ...
        self._template = jinja2 .Template(u"""
            ...
            (function(){
                ...
                for (var i = 0; i < data.length; i++) {
                    ...
                    +"{}".format(self._count)+u"""(row,colour);
                    feature.addTo(addto);
                };
            })();
            {% endmacro %}""")
