...
impl ParsingStage {
    pub fn headers_complete(&self) -> bool {
        (self != &ParsingStage::NotParsed) && (self != &ParsingStage::HeadersError)
    }
                ...
                if let httparse::Status::Complete(size) = res {
                    ...
                    for header in req.headers.iter() {
                        match header.name.to_lowercase().as_str() {
                            ...
                            &_ => {
                                let key =
                                    if header.name.to_lowercase().as_str() == CONTENT_TYPE_HEADER {
                                        CONTENT_TYPE.to_string()
                                    } else {
                                        let mut key = "HTTP_".to_string();
                                        key.push_str(
                                            &header.name.to_ascii_uppercase().replace("-", "_"),
                                        );
                                        key
                                    };
