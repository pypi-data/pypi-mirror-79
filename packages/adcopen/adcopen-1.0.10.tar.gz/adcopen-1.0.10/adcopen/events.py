...
class Events(object):
    ...
    def add_subscribe(event:str, func):
        ...
        #print( f"Events subs: {Events.subs}")
            ...
            try:
                for ev in Events.subs[event]:
                    ...
                    if asyncio.iscoroutinefunction(ev["func"]):
                        ...
                        #print(f"Event {event} is a coroutine")
                        ...
                        try:
                            ...
                            logging.warning(f"Exception calling {event} async callback: {traceback.format_exc()}")                                

                    else:

                        #print(f"Event {event} is NOT a coroutine")

                        try:
                            ev["func"](*args, **kwargs)
                        except:
                            logging.warning(f"Exception calling {event} synchronous callback: {traceback.format_exc()}")                              
            except:
                logging.warning(f"Exception processing event {event} :\n {traceback.format_exc()}")   
                logging.warning( f"Events subs: {Events.subs}")
