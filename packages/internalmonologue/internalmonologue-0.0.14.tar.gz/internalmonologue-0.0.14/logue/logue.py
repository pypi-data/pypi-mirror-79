...
def error_response(response,api_name,querystring):
    if response.status_code not in (200,201):
        ...
        raise Exception("Unknown Exception: Logs Printed Above")
...
def list_board_ids():
    ...
    error_response(response,"list_boards",querystring)
...
def create_board():
    ...
    error_response(response,"create_board",querystring)
...
def create_todays_list():
    ...
    response = requests.request("POST", url, params=querystring)
    error_response(response,"Create List",querystring)
...
def create_card(list_id, input_string):
    ...
    response = requests.request("POST", url, params=querystring)
    error_response(response,"create_card",querystring)
    return True
...
def get_all_trello_lists():
    ...
    querystring = {"cards": "none", "key": API_KEY,
                   "token": TEMPORARY_TOKEN}
    response = requests.request("GET", url, params=querystring)
    error_response(response,"get_lists",querystring)
