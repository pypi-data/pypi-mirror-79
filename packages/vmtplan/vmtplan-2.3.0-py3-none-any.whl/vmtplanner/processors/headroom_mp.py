...
class Cluster(LoggingMixin):
    ...
    def __init__(self, connection, uuid, name, members=None, realtime_members=None,
        ...
        self.headroom = defaultdict(lambda: defaultdict(lambda: {}))
        ...
        if not response or not memberlist:
            self.log(f'Skipping empty cluster [{self.uuid}]:[{self.name}]', level='debug')
            return None

        if growth_start:
            self.get_growth(growth_start)
    ...
    def _apply_templates(self, type, group, name, templates):
        ...
        for o in Cluster.type_commodity[type]:
            self.headroom[type][name][o] = self.group_commodity_headroom(
                    self.groups[type][group]['members'], o, [i], mode)
            ...
            self.headroom[type][name][o]['GrowthPerDay'] = f'{self.growth:.2f}'
        ...
        for type in self.groups:
            ...
            for group in self.groups[type]:
                if self.groups[type][group]['templates'] is None:
                    self.log(f'Skipping [{type}] group [{key}], no template assigned.', level='debug')
                    continue
    ...
    def get_growth(self, timestamp):
        self.log(f'Calculating cluster growth')
        stats = ['numVMs']
        then = self._vmt.get_entity_stats(scope=[self.uuid], start_date=timestamp, end_date=timestamp, stats=stats, fetch_all=True)[0]
        now = self._vmt.get_entity_stats(scope=[self.uuid], stats=stats, fetch_all=True)[0]

        # (cur val - prev val) / days delta
        growth = (now['stats'][0]['statistics'][0]['value'] - then['stats'][0]['statistics'][0]['value']) \
                 / (read_isodate(now['stats'][0]['date']) - read_isodate(then['stats'][0]['date'])).days
        self.growth = growth if growth > 0 else 0
    ...
    def get_stats(self):
        ...
        return self._vmt.get_market_entities_stats(self.market_id, filter=json.dumps(dto), fetch_all=True)
    ...
    def update_stats(self):
        ...
        for s in self.get_stats():
            if s['className'] not in Cluster.types:
                continue
    ...
    def update_groups(self, group_list, template_list):
    ...
    def update_group_templates(self, type, name, templates, cache=None):
        ...
        if name == 0:
            ...
            if not tpl:
                try:
                    name = self.get_default_template(cache)
                    x = Template(name, targets=[self.name])
                    x.get_resources(self._vmt)

                    self.log(f'Using default cluster template [{name}]', level='debug')
                    tpl = set([x])
                except ValueError:
...
class ClusterHeadroom(BaseBalancePlan):
    ...
    def __init__(self, connection, spec=None, market='Market', scope=None,
        ...
        self.growth_refdt = int(time.mktime((datetime.datetime.now() + datetime.timedelta(days=-1*growth_lookback)).timetuple()) * 1000)
    ...
    def _update_members_xl(self, cluster):
        ...
        for e in keys:
            ...
            if e in cluster.realtime_members:
                ent = {x: cache[e][x] for x in cluster.entity_parts}

                if cache[e]['className'] == 'PhysicalMachine':
                    cluster.add_member(ent, cache[e]['className'])
                    ...
                    for s in cache[e]['providers']:
                        ...
                        ent = {x: cache[s['uuid']][x] for x in cluster.entity_parts}
                        cluster.add_member(ent, cache[e]['className'])
                        del cache[s['uuid']]
    ...
    def _update_members_classic(self, cluster):
        ...
        def processchain(type):
            try:
                ...
                self.log(f'Cluster [{cluster}]:[{self.clusters[cluster]["name"]}] has no members of type {type}', level='warn')
                continue

            keys = list(self.__e_cache[t].keys())

            for k in keys:
                if self.__e_cache[t][k]['realtimeMarketReference']['uuid'] \
                in cmember:
                    rid = self.__e_cache[t][k]['realtimeMarketReference']['uuid']
                    ent = {x: entity[x] for x in cluster.entity_parts}
    ...
    def _post_cluster_headroom(self):
        ...
        for c in [x for x in self._get_plan_scope() if x['className'] == 'Cluster']:
            ...
            obj.get_growth(self.growth_refdt)
            obj.apply_templates()
