...
class FeeNoEquipSingleEmbModel(MLSLightGBMModel):
    ...
    def __init__(
        ...
        - model_features: (list(str)) 피쳐 리스트 (필수 피쳐: `age`, `bas_fee_amount`, `filter_five_g`)
    ...
    def predict(self, x: List[Any], **kwargs) -> Dict[str, Any]:
        ...
        preprocessed_x.append(now.day / (now.replace(day=1) - timedelta(days=1)).day)

        # 상품 선택을 위한 feature 추출
        try:
            age = preprocessed_x[self.features.index("age")]
            bas_fee_amount = preprocessed_x[self.features.index("bas_fee_amount")]
        ...
        # Prediction
        y = self.models[0].predict(np.array([preprocessed_x])).flatten().tolist()[0]
        if y <= 0:
            return {"items": []}

        # 상품 선택
        pred_bas_fee_amt = bas_fee_amount + y * 1000
        rec_prod = None
        min_diff = 999999
        for p in products:
