...
class SeleniumCrawlerForPhantom(webdriver.PhantomJS):
    ...
    def __init__(self, path2driver: str, download_path=None):
            ...
            if len(tmp_df) != 0:
                ...
                for data_line in tqdm(tmp_df):
                    ...
                    if not self.get_2(data_line[0]):
                        self.log_error()
                        continue
                    tmp_result = self.routine4selenium(order_list=tmp_order_list, get_dict=tmp_get_dict, click_dict=tmp_click_dict,
