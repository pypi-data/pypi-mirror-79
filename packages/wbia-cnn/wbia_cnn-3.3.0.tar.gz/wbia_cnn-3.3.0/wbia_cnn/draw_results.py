...
def interact_patches(
    ...
    if sortby is not None:
        ...
        if flat_metadata is not None:
            if 'fs' in flat_metadata:
                ...
                sortx_lists = [
                    flat_metadata['fs'].take(idxs).argsort()[::-1] for idxs in idx_lists
                ]
    ...
    interact = make_InteractSiamPatches(
        ibs,
        data_lists,
        label_list,
        flat_metadata,
...
def make_InteractClasses(*args, **kwargs):
    ...
    class InteractAnnotClasses(abstract_interaction.AbstractInteraction):
        ...
        def _show_page(inter):
            ...
            pnum_ = pt.make_pnum_nextgen(
                nCols=inter.nCols, nSubplots=len(inter.df_chunks)
            )
            for df_chunk in inter.df_chunks:
                ...
                data_lists = [
                    ibs.get_image_thumbnail(annots_chunk.gids, draw_annots=True)
                ]
            ...
            pt.adjust_subplots(
                top=0.95, left=0, right=1, bottom=0.00, hspace=0.1, wspace=0
            )
...
def get_patch_multichunks(data_lists, label_list, flat_metadata, multiindicies):
    ...
    for indicies in multiindicies:
        img, offset_list, sf_list, stacked_orig_sizes = get_patch_chunk(
            data_lists, label_list, flat_metadata, indicies, border_color
        )
        ...
        solidbar = np.zeros((img.shape[0], int(img.shape[1] * 0.1), 3), dtype=img.dtype)
...
def get_patch_chunk(
    ...
    if flat_metadata is None:
        ...
        flat_metadata_subset = dict(
            [(key, ut.take(vals, indicies)) for key, vals in six.iteritems(flat_metadata)]
        )

    # import utool
    # with utool.embed_on_exception_context:
    patch_list_subsets_ = [
        [vt.ensure_3channel(patch) for patch in ut.take(warped_patch_list, indicies)]
    ...
    if label_list is not None:
        ...
        if data_per_label in [1, 2]:
            ...
            unique_colors = [
                tuple(pt.color_funcs.to_base255(color)[0:3])[::-1]
        ...
        patch_list_subsets = [
            [
                vt.draw_border(
                    patch, color=colorfn[label], thickness=thickness, out=patch
                )
    ...
    patchsize_lists = [
        [vt.get_size(patch) for patch in patch_list] for patch_list in patch_lists
    ]
    ...
    stacked_patches, offset_list, sf_list = vt.stack_multi_images2(
        multiimg_list, offsets_list, sfs_list, vert=not vert, modifysize=False
    )
    ...
    if patch_texts is not None:
        ...
        for offset, patchsize, sf, text in zip(
            ...
            # Bordered text
            vt.draw_text(
                img, text, org, thickness=6, textcolor_rgb=textcolor_rgb1, **fontkw
            )
            vt.draw_text(
                img, text, org, thickness=2, textcolor_rgb=textcolor_rgb2, **fontkw
            )
    else:
        img = stacked_patches
...
def visualize_score_separability(
    ...
    tn_support = np.array(
        ut.compress(flat_metadata['fs'], ut.not_list(label_list))
    ).astype(np.float64)
