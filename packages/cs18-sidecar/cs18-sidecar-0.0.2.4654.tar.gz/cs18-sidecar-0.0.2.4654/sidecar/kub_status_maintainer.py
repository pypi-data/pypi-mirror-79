...
class KubStatusMaintainer(StatusMaintainer):
    ...
    def update_sandbox_annotation(self, key: str, value: str):
        annotation = {key: value}
        self.kub_api_service.update_sandbox_entity(annotation)

    def update_qualiy_status(self, status: str):
        self.update_sandbox_annotation(Const.QUALIY_STATUS, status)

    def add_sandbox_error(self, error: SandboxError):
        try:
            errors_as_json_str = self.kub_api_service.get_sandbox_entity_annotation(Const.SANDBOX_ERRORS)
            errors = self._json_str_to_errors(errors_as_json_str)
            errors.append(error)
            errors_as_json_str = self._errors_to_json_str(errors)
            self.update_sandbox_annotation(Const.SANDBOX_ERRORS, errors_as_json_str)
    ...
    def _modify_logical_apps(self, app_name: str, modifier: Callable[[dict], None]):
        with self._services_lock:
            ...
            app_services = self.kub_api_service.get_all_services(f"{Const.APP_NAME_TAG}=={app_name}")
