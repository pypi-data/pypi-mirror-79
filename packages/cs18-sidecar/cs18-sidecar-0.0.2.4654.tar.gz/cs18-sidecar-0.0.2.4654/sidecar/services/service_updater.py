...
class K8SServiceUpdater(IServiceUpdater):
    ...
    def on_update_status(self, name: str, status: str):
        with self.lock:
            pod = self._get_services_execution_pod_with_retries()
            service_data = KubApiPodReader.get_service_json(pod, name)
            service_data["status"] = status
            annotation_change = {name: json.dumps(service_data)}
            self._k8s_service.update_pod(KubApiPodReader.get_pod_name(pod), annotation_change)

    def _get_services_execution_pod_with_retries(self) -> dict:
        pod = Utils.retry_on_exception(
            func=lambda: self._get_services_execution_pod(),
            logger=self.logger,
            logger_msg="trying to get service execution pod")
        return pod

    def _get_services_execution_pod(self) -> dict:
        services_execution_filter_query = f"{Const.CONTROL_ELEMENT}=={Const.SERVICES_EXECUTOR_ELEMENT_NAME}"
        pods = self._k8s_service.get_pods_by_filter(filter_query=services_execution_filter_query)
        if not pods:
            raise Exception(f"could not find any service execution pods")
        return pods[0]
