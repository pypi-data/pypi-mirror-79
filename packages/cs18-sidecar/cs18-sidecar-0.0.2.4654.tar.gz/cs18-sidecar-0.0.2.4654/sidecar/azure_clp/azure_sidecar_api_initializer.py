...
class AzureSidecarApiInitializer:
    ...
    def initialize(self):
        ...
        azure_sandbox_public_address_fetcher = AzureSandboxPublicAddressFetcher(logger=self._logger,
                                                                                config=self._config,
        ...
        built_in_inputs_provider = self._create_cp_agnostic_components(
            app_instance_service=azure_app_instance_service,
            sandbox_deployment_end_updater=azure_sandbox_deployment_end_updater,
            apps_configuration_end_tracker=apps_configuration_end_tracker,
            sandbox_domain_name_fetcher=SandboxDomainNameFetcher(sandbox_id=self._config.sandbox_id),
            sandbox_public_address_fetcher=azure_sandbox_public_address_fetcher
        )
    ...
    def _create_cp_agnostic_components(self, app_instance_service: IAppInstanceService,
        ...
        built_in_inputs_provider = BuiltInInputsProvider(
            sandbox_id=self._config.sandbox_id,
            sandbox_domain_name_fetcher=sandbox_domain_name_fetcher,
            sandbox_public_address_fetcher=sandbox_public_address_fetcher
        )
