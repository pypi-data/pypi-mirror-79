...
def qparam(
    ...
    :param location: (optional) The name of the query parameter to use instead of
        ``name``.
    :param multiple: (optional) Flag indicating wether the query parameter can be
...
def url_for(endpoint, _ignore_version=False, **values):
    ...
    Note that setting ``SERVER_NAME`` to an appropriate value in the application's
    configuration is necessary to use this function outside of a request context.

    :param endpoint: The endpoint (name of the function) of the URL.
    :param _ignore_version: (optional) Flag indicating wether the API version should be
...
def modify_query_params(
    ...
    :param toggle: (optional) A dictionary mapping parameter keys to either single
        ...
        operation will be done, depending on wether the original query parameters
