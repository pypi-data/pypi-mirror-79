...
class DB():
    ...
    def getGitInfo(partnerId, client, platform):
        mysql = DB()
        sql = "select gitToken,gitId,tableName,gitLink from partnerManager WHERE partnerId='%s' AND client='%s' AND platform='%s'" % (
            partnerId, client, platform)
        result = mysql.select(sql)
        mysql.close()
        return result
    ...
    def getGerritInfo(partnerId, client, platform):
        mysql = DB()
        sql = "select gerritUser,gerritPassword, gerritProject,gerritUrl,tableName from partnerManager WHERE partnerId='%s' AND client='%s' AND platform='%s'" % (
            partnerId, client, platform)
        result = mysql.select(sql)
        # str = result[0]
        mysql.close()
