...
class AnnotationSetInsights(mixins.ListModelMixin,
    ...
    def list(self, request, *args, **kwargs):
        ...
        stats_by_tags = self.calc_stats_by_tags(split_by_tags, annotation_set)
        for class_name, class_stat in stat.classes.items():
            ...
            if stats_by_tags and class_name in stats_by_tags:
                class_stat['split_by_tags'] = [{
                    'name': tag,
                    **tag_stat
                } for tag, tag_stat in stats_by_tags[class_name].items()]

            data.append(class_stat)

        if stat.tags and len(split_by_tags)==0:
            ...
            for tag in split_by_tags:
                for class_name in annotation.classes:
                    ...
                    if tag in annotation.tags:
                        ...
                        tag_stat['objects_per_image'] = tag_stat['total_annotation_objects'] / tag_stat[
                            'total_images']
                    class_stat[tag] = tag_stat
                    stats_by_tags[class_name] = class_stat
        return stats_by_tags
