...
class ConfigDict(dict):
    ...
    def from_server(cls):
        ...
        payload = {}
        req = ProcJobRequest("proc-list", payload)
        config = req()
...
class BaseDiscoveryRequest(BaseGETRequest):
    """To discover the possible parameters on the LXPER engine."""
...
################################################################################
# Section C. Job / Discovery Request Classes
################################################################################

class EngineDiscoveryRequest(BaseDiscoveryRequest):
    """Request sent to Engine API to discover the possible parameters and availability"""
    def __init__(self, 
                 engine_id: str):
        url = f"http://atm-{engine_id}.atm.svc.cluster.local:8000"
        super().__init__(url)

class ProcDiscoveryRequest(BaseDiscoveryRequest):
    """Request sent to Proc API to discover the possible parameters and availability"""
    def __init__(self, 
                 proc_id: str):
        url = f"http://atm-engine-proc.atm.svc.cluster.local:8000/{proc_id}"
        super().__init__(url)

class EngineJobRequest(BaseJobRequest):
    """Request sent to Engine API to make the actual work done"""
    def __init__(self, 
                 engine_id: str, 
                 payload: Dict):
        url = f"http://atm-{engine_id}.atm.svc.cluster.local:8000/generate"
        super().__init__(url, payload)
        
class ProcJobRequest(BaseJobRequest):
    """Request sent to Proc API to make the actual work done"""
    def __init__(self, 
                 proc_id: str, 
                 payload: Dict):
        url = f"http://atm-engine-proc.atm.svc.cluster.local:8000/{proc_id}/generate"
        super().__init__(url, payload)
...
class Client:
    ...
    def _add_endpoints(self) -> None:
        ...
        for endpoint_id in self._config.endpoints:
            ...
            try:
                if endpoint_id.startswith("engine-"):
                    func = ATMFunction(endpoint_id, EngineJobRequest, EngineDiscoveryRequest)
                elif endpoint_id.startswith("proc-"):
                    func = ATMFunction(endpoint_id, ProcJobRequest, ProcDiscoveryRequest)
                else:
                    raise ValueError(f"not understood prefix of endpoint id. {endpoint_id}")
