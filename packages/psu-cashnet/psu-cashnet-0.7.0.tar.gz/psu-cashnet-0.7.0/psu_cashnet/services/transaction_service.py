...
def create_transaction(
    ...
    try:
        ...
        for ii in requested_items:
            ...
            if type(ii) is CatalogItem:
                catalog_items.append(ii)
            else:
                ci = CatalogItem.objects.get(id=ii)
                if ci:
                    catalog_items.append(ci)
                else:
                    invalid_items = True
                    error_service.unexpected_error(
                        'Invalid catalog item selected.',
                        f'Could not find a valid catalog item from "{ii}"'
                    )
    ...
    try:
        for ii in catalog_items:
            log.info(f"Adding {ii} of type {type(ii)} to {tx}")
            pi = PurchasedItem()
            pi.transaction = tx
            pi.catalog_item = ii
            pi.purchase_amount = ii.current_price()
            pi.save()
            log.info(f"Added {ii} to {tx}")
    except Exception as ee:
        error_service.unexpected_error(
