...
class CustomBreakdown(data.DatedBreakdown):
    __slots__ = ('__dayRanges')
    def __init__(self, dayRanges):
        self.__dayRanges = degreedays.time.DayRanges._check(dayRanges)
    def _equalityFields(self):
        return self.__dayRanges
    @property
    def dayRanges(self): return self.__dayRanges
    def __repr__(self):
        return 'CustomBreakdown(%r)' % self.__dayRanges
    def _toXml(self):
        return XmlElement('CustomBreakdown').addChild(self.__dayRanges._toXml())
...
class TemperatureTimeSeriesCalculation(data.TimeSeriesCalculation):
    ...
    @property
    def temperatureUnit(self): return self.__temperatureUnit
    def __repr__(self):
