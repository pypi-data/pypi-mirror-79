...
class Postgres(Construct):
    def __init__(self, scope: Construct, id: str, *, master_username: str, vpc: IVpc, database_name: str,
                 ...
                 version: PostgresEngineVersion = PostgresEngineVersion.VER_10_11,
                 encryption_key: typing.Optional[IKey] = None) -> None:
        ...
        parameter_group = ParameterGroup(scope, 'rds-parameter-group',
                                         engine=DatabaseInstanceEngine.postgres(version=version),
                                         parameters=parameters)
        option_group = OptionGroup(scope, 'rds-option-group', engine=DatabaseInstanceEngine.postgres(version=version),
                                   configurations=[])
        self.instance = DatabaseInstance(
            scope, "db-instance",
            master_username=master_username,
            engine=DatabaseInstanceEngine.postgres(version=version),
            instance_type=InstanceType.of(instance_class, instance_size),
            multi_az=True,
