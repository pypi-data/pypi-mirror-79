...
class BaseConsumer(object):
    def __init__(self, queue, enable_threading=True):
        self.q = queue
        self.channel = queue.channel
        self.connection = queue.connection
        self.queue_name = queue.queue_name
        self.threads = []
        self.on_message_callback = functools.partial(self.on_message, args=(self.connection, self.threads))
    ...
    def on_message(self, channel, method, properties, body, args):
        ...
        if self.enable_threading:
            t = threading.Thread(target=self.process_in_thread, args=(conn, channel, method, properties, decode_bson(body)))
            t.start()
            thrds.append(t)
        else:
            self.process(channel, method, properties, decode_bson(body))

    def ack_message(self, channel, delivery_tag):
        channel.basic_ack(delivery_tag)

    def process_in_thread(self, conn, channel, method, properties, body):
        thread_id = threading.get_ident()
        LOGGER.info('Thread id: %s Delivery tag: %s Message body size: %s', thread_id,
                    method.delivery_tag, len(body))
        self.process(channel, method, properties, body)
    ...
    def get_and_publish(self):
        ...
        self.channel.basic_consume(self.queue_name, self.on_message_callback, auto_ack=True)
