...
@app.command()
def black(msg: Text = str(), bold: Bool = False, underline: Bool = False,
...
@app.command()
def red(msg: Text = str(), bold: Bool = False, underline: Bool = False,
...
@app.command()
def green(msg: Text = str(), bold: Bool = False, underline: Bool = False,
          blink: Bool = False, err: Bool = False, e: Bool = False) -> None:
    """
    Green.
...
@app.command()
def yellow(msg: Text = str(), bold: Bool = False, underline: Bool = False,
...
@app.command()
def blue(msg: Text = str(), bold: Bool = False, underline: Bool = False,
         blink: Bool = False, err: Bool = False, e: Bool = False) -> None:
    """
    Blue.
...
@app.command()
def magenta(msg: Text = str(), bold: Bool = False, underline: Bool = False,
            blink: Bool = False, err: Bool = False, e: Bool = False) -> None:
    """
    Magenta.
...
@app.command()
def cyan(msg: Text = str(), bold: Bool = False, underline: Bool = False,
         blink: Bool = False, err: Bool = False, e: Bool = False) -> None:
    """
    Cyan.
...
@app.command()
def white(msg: Text = str(), bold: Bool = False, underline: Bool = False,
          blink: Bool = False, err: Bool = False, e: Bool = False) -> None:
    """
    White.
...
@app.command()
def bblack(msg: Text = str(), bold: Bool = False, underline: Bool = False,
           blink: Bool = False, err: Bool = False, e: Bool = False) -> None:
    """
    Black.
...
@app.command()
def bred(msg: Text = str(), bold: Bool = False, underline: Bool = False,
         blink: Bool = False, err: Bool = True, e: Bool = True) -> None:
    """
    Bred.
...
@app.command()
def bgreen(msg: Text = str(), bold: Bool = False, underline: Bool = False,
           blink: Bool = False, err: Bool = False, e: Bool = False) -> None:
    """
    Bgreen.
...
@app.command()
def byellow(msg: Text = str(), bold: Bool = False, underline: Bool = False,
            blink: Bool = False, err: Bool = True, e: Bool = False) -> None:
    """
    Byellow.
...
@app.command()
def bblue(msg: Text = str(), bold: Bool = False, underline: Bool = False,
          blink: Bool = False, err: Bool = False, e: Bool = False) -> None:
    """
    Bblue.
...
@app.command()
def bmagenta(msg: Text = str(), bold: Bool = False, underline: Bool = False,
             blink: Bool = False, err: Bool = False, e: Bool = False) -> None:
    """
    Bmagenta.
...
@app.command()
def bcyan(msg: Text = str(), bold: Bool = False, underline: Bool = False,
          blink: Bool = False, err: Bool = False, e: Bool = False) -> None:
    """
    Bcyan.
...
@app.command()
def bwhite(msg: Text = str(), bold: Bool = False, underline: Bool = False,
           blink: Bool = False, err: Bool = False, e: Bool = False) -> None:
    """
    Bwhite.
...
def ask(msg: Text) -> Bool:
    ...
    Returns:
        Bool:
    """
    if rich.prompt.Prompt.ask(msg, choices=['Yes', 'No'], default='Yes') == 'Yes':
        return True
    return False
...
def context(variables=''):
    ...
    if variables:
        try:
            msg_dict = {var: '' for var in variables.split() if not context_dict_clean.get(var, None)}
...
@app.command()
def package_info(p: Text, stdout: Bool = False) -> TupleBoolTextText:
    """
...
@app.command()
def package_latest(p: Text, stdout: Bool = False) -> Text:
    """
...
@app.command()
def package_latest_search(p: Text = None, stdout: Bool = False) -> Text:
    """
...
@app.command()
def print_modules(p: Text, indent: Int = 0):
    # noinspection PydanticTypeChecker
    print('\t' * indent, mod_name(p), ':')
    indent += 1
    # noinspection PyUnresolvedReferences
    for mod in p._modules:
        if mod._isPackage:
            print_modules(mod, indent)
        else:
            # noinspection PyCallingNonCallable
            print('\t' * indent, mod_name(mod))
...
@app.command()
def upgrade_message(p: Text = None, out: Bool = False):
    """
...
class Frame:
    ...
    def __init__(self, index: Int = index, ctx: Int = ctx):
        info: FrameInfo = inspect.stack(ctx)[index]
        frame: FrameType = info.frame
        self.frame = frame
        self.code_context: List = info.code_context
        self.function: Text = info.function
        self.lineno: Int = info.lineno
        self.filename: Path = Path(info.filename).resolved
        ...
        self.f_lineno = info.frame.f_lineno
...
class GitHub:
    ...
    def __post_init__(self):
        ...
        self.url = Url.lumenbiomics(http=False, repo=self.path.name)
...
class Option:
    ...
    def option(function: Function = Function.__args__[0], msg: Text = None, default: Int = 0) -> typer.Option:
        ...
        Examples:
            ...
            >>> Option.option(Option.Function.__args__[1], 'Part of version to increase', 1)
...
def paths(file: Text) -> Path:
    ...
    p.attrs_set(
        ...
        log=Logr.get_log(name=p.name, p=work.mkdir_add('log') / f'{p.name}.log',
                         stream=Env().file.log_level(upper_prefix('LOG_LEVEL', prefix=prefix), Logr.Levels.ERROR.value),
                         installed=installed),
        scripts=[item for item in p.mkdir_add('scripts').iterdir()
                 if item.suffix == Path.Suffix.BASH.value or item.suffix == Path.Suffix.SH.value],
        scripts_relative=[item.relative_to(p.parent).text for item in p.mkdir_add('scripts').iterdir()
                          if item.suffix == Path.Suffix.BASH.value or item.suffix == Path.Suffix.SH.value],
        git=GitHub(p.parent)
    )
    return p
...
def system() -> None:
    """
    System: Distro, Executable, Machine and Py.

    Returns:
        None:
    """
    return console.print(System())
...
class Url(furl.furl):
    ...
    Examples:
        >>> if Url.email(os.environ.get('USER')) == Url.email(User.user):
        ...     True
        >>> Url.github()
        Url('https://github.com')
        >>> Url.wiki(number=183238657, page='Hola')
        Url('https://nferx.atlassian.net/wiki/spaces/DevOps/pages/183238657/Hola')
        >>> Url.lumenbiomics(http=False, repo='test')
        'org-4379404@github.com:lumenbiomics/test'
        >>> http_url = Url.lumenbiomics(repo='test', username=User.github_username, password='password')
    ...
    @classmethod
    @app.command()
    def email(cls, username: Text, stdout: Bool = False) -> Text:
        """
        User Email.
        ...
        """
        if stdout:
            console.print(f'{username}@{cls.nference().host}')
        return f'{username}@{cls.nference().host}'
    ...
    def github(cls) -> Url:
        ...
        Returns:
            Url:
        """
        return cls('github.com')

    @classmethod
    @app.command()
    def lumenbiomics(cls, http: Bool = True, username: Text = None, password: Text = None,
    ...
    @classmethod
    @app.command()
    def nference(cls, stdout: Bool = False) -> Url:
        """
        Nferx Url.
    ...
    def nferx(cls) -> Url:
        ...
        Returns:
            Url:
        """
        return cls('nferx.com')

    @classmethod
    def wiki(cls, space: Text = group, number: UnionTextInt = None, page: Text = None) -> Url:
        """
        Confluence Wiki Url.
        ...
        return cls('atlassian.net', name=name, path=p,)
...
class User:
    ...
    admin: Any = collections.namedtuple('UserNamed', 'username password', defaults=('admin', '1Aragon$.'))
    user: Any = admin(username, username)
...
def user() -> None:
    ...
    Returns:
        None:
    """
    return console.print(dataclasses.asdict(User()))
...
# ic(path.project.templates)

try:
    distribution = importlib_metadata.distribution(path.repo)
except importlib_metadata.PackageNotFoundError as exception:
    pass
