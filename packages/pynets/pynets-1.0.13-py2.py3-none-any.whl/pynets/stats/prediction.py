...
def bootstrapped_nested_cv(
    ...
    if remove_multi is True:
        ...
        to_drop = [column for column in upper.columns if any(upper[column] > alpha)]
        try:
...
class MakeXY(SimpleInterface):
    ...
    def _run_interface(self, runtime):
        ...
        if isinstance(X, pd.DataFrame):
            out_X = f"{runtime.cwd}/X_{self.inputs.target_var}_" f"{self.inputs.modality}_{self.inputs.alg}_" f"{'_'.join(str(v) for v in self.inputs.grid_param)}.csv"
...
class MakeDF(SimpleInterface):
    ...
    def _run_interface(self, runtime):
        ...
        out_df_summary = f"{runtime.cwd}/df_summary_" f"{self.inputs.target_var}_" f"{self.inputs.modality}_{self.inputs.alg}_" f"{'_'.join(str(v) for v in self.inputs.grid_param)}.csv"
...
def build_predict_workflow(args, retval):
    ...
    procmem = [int(nthreads), int(list(psutil.virtual_memory())[4] / 1000000000) - 2]
    plugin_args = {
...
def main():
    ...
    df = pd.read_csv(
        "/working/tuning_set/outputs_shaeffer/df_rum_persist_all.csv", index_col=False
    )
    ...
    args["embedding_type"] = embedding_types
