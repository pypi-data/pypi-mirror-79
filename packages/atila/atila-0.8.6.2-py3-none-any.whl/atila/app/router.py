...
class Router:
    ...
    def add_route (self, rule, func, **options):
        ...
        if s == -1:
            ...
            if s >= 0 and s_rule [-1] == ">" and len (rulenames) >= 1:
                if len (rulenames) == 1 and rulenames [0][0] in options.get ("defaults", {}):
                ...
                elif rulenames [-1][0] in options.get ("defaults", {}):
                    options_ = copy.copy (options)
                    options_ ["argspec"] = proto [2:5]
                    self.add_route (s_rule [:s_rule.rfind ('/<')], func, **options_)
    ...
    def funcfor (self, thing):
        if isinstance (thing, FunctionType):
            thing = self._function_names [id (thing)]
        fpath = self._function_names [thing]
        try:
            return self.optimal_proto (self.route_map, fpath [0], thing) [0]
        except KeyError:
            if len (fpath) == 2:
                return self.optimal_proto (self.route_map_fancy [fpath [0]], fpath [1], thing) [0]
