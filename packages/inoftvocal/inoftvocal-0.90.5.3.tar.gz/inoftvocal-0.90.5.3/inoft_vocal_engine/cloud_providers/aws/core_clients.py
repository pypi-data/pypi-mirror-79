...
from inoft_vocal_engine.cloud_providers.aws.policies import ATTACH_POLICY, ASSUME_POLICY
...
class AwsCoreClients:
    ...
    def __init__(self, clients_to_load: List[int]):
        self._boto_session, self._s3_client, self._lambda_client, self._dynamodb_client, \
        self._api_gateway_client, self._api_gateway_v2_client, self._events_client, \
        self._iam_client, self._iam_resource, self._sts_client = None, None, None, None, None, None, None, None, None, None
        ...
        if self.CLIENT_API_GATEWAY_V2 in clients_to_load:
            futures.append(self.pool.submit(self._create_api_gateway_v2_client))
        ...
        if self.CLIENT_STS in clients_to_load:
            futures.append(self.pool.submit(self._create_sts_client))
    ...
    def _create_api_gateway_client(self):
        self._api_gateway_client = boto3.client("apigateway")

    @property
    def api_gateway_v2_client(self) -> boto3.client:
        if self._api_gateway_v2_client is None:
            self._raise_client_not_loaded("CLIENT_API_GATEWAY_V2")
        return self._api_gateway_v2_client

    def _create_api_gateway_v2_client(self):
        self._api_gateway_v2_client = boto3.client("apigatewayv2")
    ...
    def sts_client(self) -> boto3.client:
        if self._sts_client is None:
            self._raise_client_not_loaded("STS_CLIENT")
