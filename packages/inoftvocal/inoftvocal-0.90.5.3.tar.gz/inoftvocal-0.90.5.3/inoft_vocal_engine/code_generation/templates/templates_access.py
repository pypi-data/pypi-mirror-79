...
class TemplatesAccess:
    file_loader = FileSystemLoader(os.path.join(get_inoft_vocal_engine_root_path(), "code_generation/templates"))
    # We cannot just set the templates folder name in the FileSystemLoader, otherwise jinja will try to descend into
    # all the folders it found until it finds a folder called templates. And of course, the engine has other folders
    # named templates (in particular, the templates folder in the web_interface) folder. So, we need to use a direct
    # path to the code_generation/templates folder.
    env = Environment(loader=file_loader, lstrip_blocks=True, trim_blocks=True)

    # todo: remove the staticness
    ...
    def _load_template(template_filepath: str) -> Template:
        from inoft_vocal_engine.code_generation.templates.jinja_filepath_loader import JinjaFilepathLoader
