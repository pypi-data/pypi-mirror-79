...
class AudioEditorProjectsDynamoDbClient(DynamoDbCoreAdapter):
    ...
    def save_project_data(self, account_project_id: str, project_data: dict) -> bool:
        try:
            item = Utils.python_to_dynamodb(project_data)
            item["projectId"] = account_project_id
    ...
    def get_project_data_by_account_project_id(self, account_project_id: str) -> (dict, bool):
        ...
        try:
            table = self.dynamodb.Table(self.table_name)
            response = table.get_item(Key={"projectId": account_project_id}, ConsistentRead=True)
            if "Item" in response:
