import struct, re, datetime, sys
import aiohglib
from aiohglib import error, util, templates, merge, context

from aiohglib.util import b, u, cmdbuilder, BytesIO, strtobytes, POSSIBLE_TIMEZONES
...
class revision(tuple):
    ...
    def __new__(cls, rev, node, tags, branch, author, desc, date, p1node, p2node, extras):
        source = b("0000000000000000000000000000000000000000")
        inter_source = b("0000000000000000000000000000000000000000")
        close = False
        for extra in extras.split(aiohglib.util.b("_;_")):
            if extra.startswith(aiohglib.util.b("source=")):
                source = extra.split(aiohglib.util.b("="))[-1]
            if extra.startswith(aiohglib.util.b("intermediate-source=")):
                inter_source = extra.split(aiohglib.util.b("="))[-1]
            if extra.startswith(aiohglib.util.b("close=1")):
                close = True
        return tuple.__new__(cls, (u(rev), u(node), u(tags), u(branch), u(author), u(desc), date, u(p1node), u(p2node), u(source), u(inter_source), close))
...
class hgclient(object):
    ...
    def __init__(self, path, encoding, configs):
        self._args = [aiohglib.HGPATH, 'serve', '--cmdserver', 'pipe',
    ...
    async def _readhello(self):
        """ read the hello message the server sends when started """
        ch, msg = await self._readchannel()
        assert ch == b('o')
    ...
    async def _readchannel(self):
        data = await self.server.stdout.read(hgclient.outputfmtsize)
        if not data:
        ...
        if channel in b('IL'):
            ...
            return channel, await self.server.stdout.read(length)
    ...
    def _parserevs(splitted):
        ...
        '''
        revs = []
        for rev in aiohglib.util.grouper(10, splitted):
            input_dt = datetime.datetime.strptime(u(rev[6][:19]), "%Y-%m-%d %H:%M:%S")
            # guess timezone
            for possible_tz in POSSIBLE_TIMEZONES:
                possible_dt = possible_tz.localize(input_dt)
                reverse_dt = u(possible_dt.strftime("%Y-%m-%d %H:%M:%S %z"))
                if reverse_dt == u(rev[6]):
                    correct_dt = possible_dt
                    break
            else:
                correct_dt = input_dt
            revs.append(revision(rev[0], rev[1], rev[2], rev[3],
                                 rev[4], rev[5], correct_dt, rev[7], rev[8], rev[9]))
        return revs

    async def runcommand(self, args, inchannels, outchannels):
        async def writeblock(data):
        ...
        await self.server.stdin.drain()
        await writeblock(b('\0').join(args))

        while True:
            channel, data = await self._readchannel()
            if self._protocoltracefn is not None:
                self._protocoltracefn('r', channel, data)

            # input channels
            if channel in inchannels:
                await writeblock(inchannels[channel](data))
            # output channels
    ...
    async def rawcommand(self, args, eh=None, prompt=None, input=None):
        """
        ...
        ret = await self.runcommand(args, inchannels, outchannels)
    ...
    async def open(self):
        ...
        self.server = await util.popen(self._args, self._env)
        try:
            await self._readhello()
    ...
    async def _close(self):
        if self.server.stdin.can_write_eof():
            self.server.stdin.write_eof()
        else:
            self.server.stdin.close()
        _sout, serr = await self.server.communicate()
        ret = self.server.returncode
        self.server = None
        return ret, serr

    async def add(self, files=[], dryrun=False, subrepos=False, include=None,
            exclude=None):
        """
        ...
        await self.rawcommand(args, eh=eh)

        return bool(eh)

    async def addremove(self, files=[], similarity=None, dryrun=False, include=None,
                  exclude=None):
    ...
    async def annotate(self, files, rev=None, nofollow=False, text=False, user=False,
        ...
        out = await self.rawcommand(args)

        for line in out.splitlines():
            yield tuple(line.split(b(': '), 1))

    async def archive(self, dest, rev=None, nodecode=False, prefix=None, type=None,
        ...
        await self.rawcommand(args)

    async def backout(self, rev, merge=False, parent=None, tool=None, message=None,
    ...
    async def bookmark(self, name, rev=None, force=False, delete=False,
    ...
    async def branch(self, name=None, clean=False, force=False):
        ...
        out = await self.rawcommand(args).rstrip()
    ...
    async def branches(self, active=False, closed=False):
        """
    ...
    async def bundle(self, file, destrepo=None, rev=[], branch=[], base=[], all=False,
    ...
    async def cat(self, files, rev=None, output=None):
    ...
    async def clone(self, source=b('.'), dest=None, branch=None, updaterev=None,
              revrange=None):
        """
    ...
    async def init(self, dest, ssh=None, remotecmd=None, insecure=False):
        args = util.cmdbuilder('init', dest, e=ssh, remotecmd=remotecmd,
                               insecure=insecure)
        await self.rawcommand(args)

    async def commit(self, message=None, logfile=None, addremove=False,
    ...
    async def config(self, names=[], untrusted=False, showsource=False):
    ...
    async def copy(self, source, dest, after=False, force=False, dryrun=False,
    ...
    async def diff(self, files=[], revs=[], change=None, text=False,
        ...
        return await self.rawcommand(args)

    async def export(self, revs, output=None, switchparent=False,
    ...
    async def forget(self, files, include=None, exclude=None):
    ...
    async def grep(self, pattern, files=[], all=False, text=False, follow=False,
        ...
        async def eh(ret, out, err):
            if ret != 1:
                raise error.CommandError(args, ret, out, err)
            return b('')

        out = await self.rawcommand(args, eh=eh).split(b('\0'))
    ...
    async def heads(self, rev=[], startrev=[], topological=False, closed=False):
        ...
        out = await self.rawcommand(args, eh=eh)
        out = out.split(b('\0'))[:-1]
        return self._parserevs(out)

    async def identify(self, rev=None, source=None, num=False, id=False, branch=False,
    ...
    async def import_(self, patches, strip=None, force=False, nocommit=False,
        ...
        if hasattr(patches, 'read') and hasattr(patches, 'readline'):
            ...
            async def readline(size, output):
        ...
        await self.rawcommand(args, prompt=prompt, input=input)

    async def incoming(self, revrange=None, path=None, force=False, newest=False,
    ...
    async def log(self, revrange=None, files=[], follow=False,
            ...
            include=None, exclude=None, safe_template=True):
        ...
        if safe_template:
            template = templates.changeset_safe
        else:
        ...
        try:
            result = self._parserevs(out)
        except ValueError as er:
            raise Exception("Cannot parse revs from template")

        return result

    async def manifest(self, rev=None, all=False):
    ...
    async def merge(self, rev=None, force=False, tool=None, cb=merge.handlers.abort):
        ...
        await self.rawcommand(args, prompt=prompt)

    async def move(self, source, dest, after=False, force=False, dryrun=False,
             include=None, exclude=None):
    ...
    async def outgoing(self, revrange=None, path=None, force=False, newest=False,
    ...
    async def parents(self, rev=None, file=None):
    ...
    async def paths(self, name=None):
        """
        ...
        """
        if not name:
            out = await self.rawcommand([b('paths')])
            if not out:
                return {}
    ...
    async def pull(self, source=None, rev=None, update=False, force=False,
    ...
    async def push(self, dest=None, rev=None, force=False, bookmark=None, branch=None,
    ...
    async def remove(self, files, after=False, force=False, include=None,
               exclude=None):
    ...
    async def resolve(self, file=[], all=False, listfiles=False, mark=False,
    ...
    async def revert(self, files, rev=None, all=False, date=None, nobackup=False,
    ...
    async def root(self):
        ...
        """
        return await self.rawcommand([b('root')]).rstrip()

    async def status(self, rev=None, change=None, all=False, modified=False,
    ...
    async def tag(self, names, rev=None, message=None, force=False, local=False,
    ...
    async def phase(self, revs=(), secret=False, draft=False, public=False,
              force=False):
    ...
    async def summary(self, remote=False):
        """
        ...
        out = await self.rawcommand(args).splitlines()
    ...
    async def update(self, rev=None, clean=False, check=False, date=None):
        """
    ...
    async def version(self):
        ...
        if self._version is None:
            v = await self.rawcommand(cmdbuilder(b('version'), q=True))
