...
def project_load_from_workbooks(workbooks, **kwargs):
    ...
    try:
        ...
        duplicate = True
        while duplicate:
            duplicate, project_sheets = remove_dups(project_sheets)
        x=0
        compiled = (
            pd.concat(project_sheets[::-1], sort=False)
                .set_index("Topic ID")
...
def remove_dups(project_sheets):
    duplicate = False
    for index, df in enumerate(project_sheets):
        if df.columns.duplicated().any():
            cols = pd.Series(df.columns)
            for dup in cols[cols.duplicated()].unique():
                cols[cols[cols == dup].index.values.tolist()] = [dup + '- DUPLICATED COLUMN -' + str(i) if i != 0 else dup for i in
                                                                 range(sum(cols == dup))]
            df.columns = cols
            log.info(
                f"Suspected duplication event: Topic ID: {df.get('Topic ID')}"
            )
            # project_sheets.pop(index)
            duplicate = True
    return duplicate, project_sheets
