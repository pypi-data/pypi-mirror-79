...
def prepare_dataset(para):
    ...
    read_para_ob = para["ob_data"]["read_para"]
    creat_ob_dataset(hdf_file,para["ob_data"]["dir_ob"],station,"ob",end_time = end_time,day_num=day_num,
                     read_method=para["ob_data"]["read_method"], read_para = read_para_ob,reasonable_value = para["ob_data"]["reasonable_value"],
                     operation=para["ob_data"]["operation"],operation_para = para["ob_data"]["operation_para"])
    models = para["fo_data"].keys()
    if para["dtime"] is not None:
        dtime = np.arange(para["dtime"][0],para["dtime"][1]+1,para["dtime"][2]).astype(dtype = 'int32')
    else:
        dtime = para["dtime"]
    for model in models:
        hdf_file = para["fo_data"][model]["hdf_dir"] + "/" + para["hdf_file_name"]
        hdf_file_list.append(hdf_file)
        read_para_fo = para["fo_data"][model]["read_para"]
        creat_fo_dataset(hdf_file,para["fo_data"][model]["dir_fo"],station,model,
                         end_time = end_time,day_num = day_num,dtime = dtime,
                         read_method=para["fo_data"][model]["read_method"], read_para=read_para_fo,
                         operation = para["fo_data"][model]["operation"],
                         operation_para = para["fo_data"][model]["operation_para"],interp = para["interp"],move_fo_time = para["fo_data"][model]["move_fo_time"])
...
def creat_fo_dataset(hdf_path, dir_fo, station,data_name,day_num,end_time = None,dtime = None,read_method= pd.read_hdf,read_para ={},interp = None,
                     operation=None, operation_para={},move_fo_time = 0):
    data0 = None
...
def creat_ob_dataset(hdf_path, dir_ob,station,data_name,day_num,end_time = None,read_method = pd.read_hdf,read_para = {}, interp = None,reasonable_value = None,operation=None, operation_para={}):
    data0 = None
    if os.path.exists(hdf_path):
        data0 = pd.read_hdf(hdf_path, "df")
    # print(data0)
