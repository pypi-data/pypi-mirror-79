...
def start_ssh_session(
    ...
    try:
        ...
        print("ERR", err, err.stderr, file=open("/docker_sym_logs/foo.txt", "a"))
...
def ensure_ssh_key(client, instance: str, port: int):
    ...
    print("CONNECTING", last_connect, file=open("/docker_sym_logs/foo.txt", "a"))
    if last_connect and datetime.now() - last_connect < timedelta(days=1):
        print("SKIPPING", file=open("/docker_sym_logs/foo.txt", "a"))
        client.dprint(f"Skipping remote SSH key check for {instance}")
        return

    try:
        print("COMM2", file=open("/docker_sym_logs/foo.txt", "a"))
        _start_background_ssh_session(
            client, instance, port, "exit", capture_output_=True
        )
    except CalledProcessError as err:
        print("ERROR", err, file=open("/docker_sym_logs/foo.txt", "a"))
        client.dprint(f"SSH Check Error: {err.stderr}")
        if not MissingPublicKeyPattern.search(err.stderr):
            raise
        send_ssh_key(client, instance, ssh_key)

    print("SUCC", file=open("/docker_sym_logs/foo.txt", "a"))
...
def start_tunnel(client, instance: str, port: int):
    def handler(signum, frame):
        ...
        print("ERR3", instance, file=open("/docker_sym_logs/foo.txt", "a"))
        Config.touch_instance(instance, error=True)

    signal(SIGPIPE, handler)

    print("CONN3", instance, file=open("/docker_sym_logs/foo.txt", "a"))
    out = client.exec(
        "aws",
        "ssm",
        "start-session",
    ...
    print("DONE3", instance, out, file=open("/docker_sym_logs/foo.txt", "a"))
