...
class SAMLClient(ABC):
    ...
    def _creds_expiring(self, buffer=timedelta(minutes=15)):
        if (expires := self.get_creds().get("AWS_CREDENTIAL_EXPIRATION")) :
            parsed = datetime.fromisoformat(expires)
            return datetime.now(parsed.tzinfo) + buffer < parsed
        return False
