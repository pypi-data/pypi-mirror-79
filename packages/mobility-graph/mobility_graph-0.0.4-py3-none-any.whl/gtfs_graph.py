...
def add_weight_to_edge_nx(G, from_id, to_id):
    ...
    weight = utils.compute_HS(float(pos1[from_id][1]),float(pos1[from_id][0]),float(pos2[to_id][1]),float(pos2[to_id][0]))
    return weight

def add_weight_to_edge(from_loc, to_loc):
    weight = utils.compute_HS(float(from_loc[1]),float(from_loc[0]),float(to_loc[1]),float(to_loc[0]))
    return weight
