...
DataSegmentClientBase, DataSegmentClient, FusionDataSegmentClient
"""
...
class DataSegmentClientBase:
    ...
    def upload_description(self, description: Optional[str] = None) -> None:
        ...
        self._client.dataset_post("createOrUpdateSegment", post_data)
    ...
    def _get_upload_permission(self) -> Dict[str, Any]:
        with self._permission_lock:
            if int(time.time()) >= self._permission["expireAt"]:
                ...
                self._permission = self._client.dataset_post("getPutPermission", post_data)
...
class DataSegmentClient(DataSegmentClientBase):
    """DataSegmentClient has only one sensor, supporting upload_data method."""
    ...
    def delete_data(self, remote_paths: Union[str, List[str]]) -> None:
        ...
        self._client.dataset_post("deleteObjects", post_data)
    ...
    def list_data(self) -> List[str]:
        ...
        data = self._client.dataset_post("listObjects", post_data)
        return [self._name_wrapper.get_remote_path(object_path) for object_path in data["objects"]]


class FusionDataSegmentClient(DataSegmentClientBase):
    ...
    def upload_sensor_object(self, sensor: T) -> None:
        ...
        self._client.dataset_post("createOrUpdateSensor", post_data)
    ...
    def delete_sensors(self, sensor_names: Union[str, List[str]]) -> None:
        ...
        self._client.dataset_post("deleteSensors", post_data)
    ...
    def _list_sensor_summaries(self) -> List[Dict[str, Any]]:
        ...
        data = self._client.dataset_post("listSensors", post_data)
    ...
    def _list_frames(self, offset: int = 0, page_size: int = -1) -> List[List[Dict[str, Any]]]:
        ...
        data = self._client.dataset_post("listFrames", post_data)
    ...
    def _delete_frames(self, frame_id: Union[str, List[str]]) -> None:
        ...
        self._client.dataset_post("deleteFrames", post_data)
...
class DatasetClientBase:
    ...
    def __init__(self, dataset_id: str, client: Client) -> None:
        self._dataset_id = dataset_id
        self._client = client
    ...
    def upload_description(
        ...
        post_data["contentSetId"] = self._dataset_id
        self._client.dataset_post("updateContentSet", post_data)
    ...
    def is_published(self) -> bool:
        ...
        return status == DatasetClientBase._PUBLISH_STATUS  # type: ignore[no-any-return]
    ...
    def _list_segments(self, segment_names: Optional[List[str]] = None) -> List[str]:
        ...
        segments_info = self._client.dataset_post("listSegments", post_data)["segments"]
    ...
    def delete_segments(
        ...
        self._client.dataset_post("deleteSegments", post_data)


class DatasetClient(DatasetClientBase):
    """dataset has only one sensor, supporting create segment."""

    def get_or_create_segment(self, name: str = "") -> DataSegmentClient:
        ...
        if name and not self._list_segments([name]):
            ...
            self._client.dataset_post("createOrUpdateSegment", post_data)
        return DataSegmentClient(name, self._dataset_id, self._client)

    def get_segment(self, name: str = "") -> DataSegmentClient:
        ...
        *,
        ...
        if skip_uploaded_files:
            ...
            segment_filter = filter(
                lambda data: data.remote_path
                if data.remote_path
                else os.path.basename(data.fileuri) not in done_set,
                segment,
            )
        else:
...
class FusionDatasetClient(DatasetClientBase):
    ...
    def get_or_create_segment(self, name: str = "") -> FusionDataSegmentClient:
        ...
        if name and not self._list_segments([name]):
            post_data = {"contentSetId": self._dataset_id, "name": name}
            self._client.dataset_post("createOrUpdateSegment", post_data)
        return FusionDataSegmentClient(name, self._dataset_id, self._client)

    def get_segment(self, name: str = "") -> FusionDataSegmentClient:
    ...
    def _existing_frame_generator(
        segment_client: FusionDataSegmentClient,
        segment: FusionSegment,
