...
class GAS:
    ...
    def _create_dataset(
        ...
        return ReturnType(dataset_id, self._client)
    ...
    def create_labelset(
        ...
        labelset_id = self._create_labelset(dataset_id, label_table, remote_paths)
        return LabelsetClient(labelset_id, self._client)
    ...
    def create_fusion_labelset(
        ...
        labelset_id = self._create_labelset(dataset_id, label_table, remote_paths)
        return FusionLabelsetClient(labelset_id, self._client)

    def _get_labelset_type(self, labelset_id: str) -> bool:
        if not labelset_id:
            raise GASLabelsetError(labelset_id)

        post_data = {
            "id": labelset_id,
            "projection": {"contentSetType": 1},
        }
        ...
        return bool(summaries[0]["contentSetType"])
    ...
    def get_labelset(self, labelset_id: str) -> LabelsetClient:
        ...
        """
        is_fusion = self._get_labelset_type(labelset_id)
        if is_fusion:
