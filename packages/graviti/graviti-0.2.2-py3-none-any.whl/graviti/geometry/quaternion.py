...
T = TypeVar("T", bound="Quaternion")  # pylint: disable=invalid-name
...
class Quaternion:
    ...
    1. Init from keyword-only argument: "loads"
        >>> Quaternion(loads={"w": 1, "x": 0, "y": 0, "z": 0})
        >>> Quaternion(1.0, 0.0, 0.0, 0.0)

    2. Init from w, x, y, z components
        >>> Quaternion(1)
        >>> Quaternion(1.0, 0.0, 0.0, 0.0)
    ...
    3. Init from a 3*3 or 3*4 or 4*4 sequence/numpy array
    ...
    5. Init from rotation vector
        >>> Quaternion(rotation_vector=[0, 0, 0])
        >>> Quaternion(1.0, 0.0, 0.0, 0.0)

    6. Init from axis angle
    ...
    def __init__(
        ...
        **kwargs: KwargsType,
        ...
        if loads:
            self._data = self._quaternion_from_wxyz(loads)

        if kwargs:
            self._data = self._quaternion_from_kwargs(kwargs)
            if self._data:
                return

        arg = args[0] if len(args) == 1 else args
        if isinstance(arg, collections.abc.Sequence) and arg:
    ...
    @overload
    def __mul__(self: T, other: np.ndarray) -> Vector3D:
        ...
    ...
    def __mul__(self: T, other: Union[T, np.ndarray, Sequence[float]]) -> Union[T, Vector3D]:
        if isinstance(other, Quaternion):
            return self._create(self._data.__mul__(other._data))

        if isinstance(other, (collections.abc.Sequence, np.ndarray)):
            return self.rotate(other)

        return NotImplemented

    @classmethod
    def _create(cls: Type[T], data: quaternion.quaternion) -> T:
