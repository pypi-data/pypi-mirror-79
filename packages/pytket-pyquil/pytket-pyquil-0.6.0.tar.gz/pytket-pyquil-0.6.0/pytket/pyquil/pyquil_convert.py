    ...
    if len(ppi.free_symbols) == 0:
        ...
        def to_pyquil(e: Expr) -> Union[float, Expression]:
            if isinstance(e, Number):
                ...
                args = Pow_(to_pyquil(e.base), to_pyquil(e.exp))
            elif e == pi:
                return math.pi
            else:
...
def pyquil_to_tk(prog: Program) -> Circuit:
    ...
    for i in prog.instructions:
        if isinstance(i, Gate):
            ...
            params = [param_from_pyquil(p) for p in i.params]
...
def process_characterisation(qc: QuantumComputer) -> dict:
    """Convert a :py:class:`pyquil.api.QuantumComputer` to a dictionary containing Rigetti device Characteristics
    ...
    device_node_fidelities = specs.f1QRBs()
    device_link_fidelities = specs.fCZs()
    device_fROs = specs.fROs()
    device_t1s = specs.T1s()
    device_t2s = specs.T2s()

    for index in qc.qubits():
        error_cont = QubitErrorContainer({OpType.Rx, OpType.Rz})
        error_cont.add_readout(1 - device_fROs[index])
        t1_times_dict[index] = device_t1s[index]
        t2_times_dict[index] = device_t2s[index]
        error_cont.add_error((OpType.Rx, 1 - device_node_fidelities[index]))
    ...
    for (a, b), fid in device_link_fidelities.items():
        ...
        error_cont.add_error((OpType.CZ, 1 - fid))
