...
class AutosaveSection(CollapsibleSection):
    ...
    def __init__(self, dialog):
        self.widget = None
        super(AutosaveSection, self).__init__(dialog, "Autosave", collapse=True)

    def build(self):
        self.widget = HidableTextFieldGrp(self.dialog, "Autosave Filename", extra_checkbox_label="Cleanup")
    ...
    def on_plugin_message(self, widget_id, msg):
        ...
        elif widget_id ==  self.dialog.section("UploadOptionsSection").use_daemon_widget:
            self._disable_on_use_daemon()
    ...
    def _disable_on_use_daemon(self):
        use_daemon = self.dialog.section("UploadOptionsSection").use_daemon_widget.get_value()
