...
class User(Endpoint):
    ...
    def manuscripts(self, identifier: Union[str, None] = None) -> List[Dict[str, Any]]:
        ...
        manuscripts = self._request(path)
        if 'manuscripts' in manuscripts:
            return manuscripts['manuscripts']
        else:
            return [{
                'status': manuscripts['status'],
                'error': f'No manuscripts found for request: {path}.'
            }]

    def worlds(self, identifier: Union[str, None] = None) -> List[Dict[str, Any]]:
