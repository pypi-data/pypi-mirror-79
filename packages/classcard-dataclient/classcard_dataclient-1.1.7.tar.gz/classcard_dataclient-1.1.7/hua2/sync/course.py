...
class CourseSync(BaseSync):
    ...
    def extract_course(self):
        ...
        sql = "SELECT id, coursename, sectionid, classroomid, coursedate, weekday, userid, isteacher, deptname " \
              ...
              "WHERE updateflag!='3' and coursedate > '{}' and coursedate <= '{}' and deptname!='学生' " \
        ...
        for row in rows:
            ...
            if current_course_table[0] in ["M", "U"]:
                print(current_course_table)
                ...
                if is_teacher:
                    ...
                    if class_name and class_name in self.class_name_num:
                        ...
                        if exist_class_sign and current_course_table != exist_class_sign:
                            ...
                            continue
                        if "E" in current_course_table and "小学" not in class_name:
                            print("该班级{}已在{}课表出现，请检查数据".format(class_name, current_course_table))
                            print(attend_id)
                            continue
                        if "M" in current_course_table and "初中" not in class_name:
                            print("该班级{}已在{}课表出现，请检查数据".format(class_name, current_course_table))
                            print(attend_id)
                            continue
                        if "U" in current_course_table and "高中" not in class_name:
                            print("该班级{}已在{}课表出现，请检查数据".format(class_name, current_course_table))
                            print(attend_id)
                            continue
