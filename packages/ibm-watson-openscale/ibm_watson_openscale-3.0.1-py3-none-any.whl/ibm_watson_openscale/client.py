...
class WatsonOpenScaleV2Adapter(WatsonOpenScaleV2):
    ...
    def __init__(self,
        ...
        if authenticator is not None:
            ...
            if type(authenticator) is CloudPakForDataAuthenticator:
                super().set_disable_ssl_verification(True)    
            ...
            if isinstance(authenticator, BearerTokenAuthenticator) and service_url is None:
                # Service URL is mandatory to be provided for the token path
                raise MissingValue("service_url", "Service URL is mandatory to be provided with BearerTokenAuthenticator.")
            ...
            self.service_instance_id = get_instance_guid(self.authenticator, service_url) if service_instance_id is None else service_instance_id
    ...
    def _generate_service_url(self, service_url: str = None, instance_id: Optional[str] = None) -> None:
        ...
        if instance_id is None:
            instance_id = get_instance_guid(self.authenticator, service_url)
