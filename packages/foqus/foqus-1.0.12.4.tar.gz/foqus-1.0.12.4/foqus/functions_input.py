...
def delta_retrieve_json(customer_name, customer_type, project_name):
    ...
    if before_last_json[customer_name + "_" + customer_type] != [] and difference != {} \
        ...
        for key_cat, cat in difference[customer_name + '_' + customer_type].items():
            logger("======== key_cat and cat:" + key_cat + cat)
                        ...
                        for kchanges, changes in photos.items():
                            if str(kchanges) == '$insert':
                                for change in changes:
                                    ...
                                    logger.info("added in: " + str(block_image_n) + " kcat " + str(key_cat))
                                    last_json[customer_name + "_" + customer_type][key_cat]['Photos'][int(block_image_n)]["Status"] = 1
                            elif str(kchanges) == '$delete':
                                for change in changes:
                                    block_image_n = change
                                    logger.info("deleted in: " + str(block_image_n))
                                    delete_block = before_last_json[customer_name + "_" + customer_type][key_cat]['Photos'][block_image_n]
                                    delete_block["Status"] = 0
                                    last_json[customer_name + "_" + customer_type][key_cat]['Photos'].append(delete_block)
