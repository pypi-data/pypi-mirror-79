...
class VAEF(VAE):
    ...
    def sample_from_posterior_z(self, x, y=None, mode="scRNA"):
        ...
        y :
            tensor of cell-types labels with shape ``(batch_size, n_labels)`` (Default value = None)
        mode :
            string that indicates the type of data we analyse (Default value = "scRNA")
    ...
    def get_sample_rate_fish(self, x, y=None):
        ...
        ----------
        x :
            tensor of values with shape ``(batch_size, n_input_fish)``
        y :
            tensor of cell-types labels with shape ``(batch_size, n_labels)`` (Default value = None)
        ...
        -------
        py:class:`torch.Tensor`
            tensor of means of the negative binomial distribution with shape ``(batch_size, n_input_fish)``
        library = torch.log(torch.sum(x[:, self.indexes_to_keep], dim=1)).view(-1, 1)
        batch_index = torch.ones_like(library)
    ...
    def classify(self, x, mode="scRNA"):
        ...
        -------
        py:class:`torch.Tensor`
            tensor of probabilities with shape``(batch_size, n_labels)``
    ...
    def forward(self, x, local_l_mean, local_l_var, batch_index=None, y=None, mode="scRNA", weighting=1):
        ...
        mode :
            string that indicates the type of data we analyse (Default value = "scRNA")
        weighting :
            used in none of these methods (Default value = 1)
