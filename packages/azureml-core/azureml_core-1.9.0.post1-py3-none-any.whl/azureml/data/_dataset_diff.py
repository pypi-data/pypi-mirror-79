...
class ClassifierDriftDetector:
    ...
    @staticmethod
    def _is_model_valid(pred_output):
    ...
    def get_classifier_drift_detector_results(self, x1, x2, learner=None):
        ...
        for train, test in sss.split(data, target):
            ...
            if not ClassifierDriftDetector._is_model_valid(all_preds):
                raise Exception("Trained model for drift calculation is invalid. "
                                "This is due to data of insufficient size for baseline or target dataset "
