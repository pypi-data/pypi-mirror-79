...
from azureml._base_sdk_common.workspace.operations import WorkspacesOperations, WorkspaceConnectionsOperations
...
def delete_workspace_private_endpoint_connection(auth, private_endpoint_connection_resource_id):
    # resource id format = "/subscriptions/ID/resourceGroups/rg/providers/Microsoft.MachineLearningServices/workspaces
    # /ws_name/privateEndpointConnections/connection_name"
    subscription_id = private_endpoint_connection_resource_id.split("/")[2]
    return WorkspacesOperations.delete_private_endpoint_connection(
        auth._get_service_client(AzureMachineLearningWorkspaces, subscription_id).workspaces,
