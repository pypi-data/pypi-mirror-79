...
class QuestionAnswering(Module):
    ...
    def update_question(self, question_data: dict):
        ...
        self.caller.update_question(question_data)
        if("lq" in question_data.keys()):
          lq_list = self.caller.query_corpus(question_id)
          lq_id_list = []
          for lq in lq_list:
              lq_id_list.append(lq['id'])
          self.caller.delete_corpus_list(question_id,lq_id_list)
          lq_list = question_data.get("lq")
          self.caller.add_corpus_list(question_id,lq_list)
