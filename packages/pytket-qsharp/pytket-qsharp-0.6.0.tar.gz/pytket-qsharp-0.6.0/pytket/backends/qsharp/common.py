...
class _QsharpBaseBackend(Backend):
    ...
    def default_compilation_pass(self, optimisation_level: int = 1) -> BasePass:
        ...
        if optimisation_level == 0:
            return SequencePass([DecomposeBoxes(), qs_compilation_pass()])
        elif optimisation_level == 1:
            return SequencePass(
                [DecomposeBoxes(), SynthesiseIBM(), qs_compilation_pass()]
            )
        else:
            return SequencePass(
                [DecomposeBoxes(), FullPeepholeOptimise(), qs_compilation_pass()]
            )
