...
path = site.getsitepackages()[1]+'\\melke_t\\'
...
if os.path.exists(path+names[0]) :
    ...
    if os.path.isfile(path+'ent_disambig_doc2vec.model') :
...
classpath = os.pathsep.join([site.getsitepackages()[1]+'\\melke_t\\lib\\'+j
                             for j in [i for i in os.listdir(site.getsitepackages()[1]+'\\melke_t\\lib') if '.jar' in i]])
os.chdir(site.getsitepackages()[1]+'\\melke_t')
