...
class PlotHandler(object):
    ...
    def __init__(self,
        ...
        if not include_timeline:
            ...
            red_patch = ptch.Patch(color='red', label='$v_x$')
            blue_patch = ptch.Patch(color='blue', label=r'$\kappa$')
            green_patch = ptch.Patch(color='green', label=r'$\psi$')
    ...
    def plot_time_rel_line(self,
        ...
        for group_idx, group_data in enumerate(line_coords_list):
            for idx, line_coords in enumerate(reversed(group_data)):
                ...
                fade_in_clr = 1 - ((1 + idx) / len(group_data))
                color = (np.array(color_masks[group_idx])
                         + (1 - np.array(color_masks[group_idx])) * np.array([fade_in_clr] * 3))

                # plot the spline
                line_coords = np.array(line_coords)
                temp_handle, = group_axes[group_idx].plot(line_coords[:, 0], line_coords[:, 1], color=color,
                                                          linewidth=1.4, label="Local Path", zorder=99)
                # append handle to array
