...
class IntrusionVA(VAService):
    ...
    def doClassification(self,image_by_ch,classification_images,movingBoxes_ch):
        ...
        for n in range(0,totalBoxSize,MAX_BATCH_SIZE):
            ...
            tmp_total_classification_result.extend(rr4)
    ...
    def getMaxDiffValueWhenNotFound(self, ch_id):
        ...
        if(queue is None):
            return 0
        if len(queue)> 10 :
            # print(len(queue) ,' np.mean(queue) ' , np.mean(queue) ,' std ' ,np.std(queue)   , 'var ' ,np.var(queue)  )
    ...
    def _execute(self, sc):
        ...
        for n, row in enumerate(image_by_ch):
            ...
            # if   (self.isObjectDetectorEfficient(ch_id,threthold=0.6) or _useBackgroundSubtractor == False) :
        ...
        #classification
        t3=time.time()
        new_inf_result=self.doClassification(image_by_ch,classification_images,movingBoxes_ch)
        ...
        for (ch_id, _, ch_uuid, cfg_json), inference in zip(image_by_ch, new_inf_result):
