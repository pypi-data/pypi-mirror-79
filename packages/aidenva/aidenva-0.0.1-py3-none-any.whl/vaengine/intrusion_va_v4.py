...
class IntrusionVA(VAService):
    ...
    def classifyBoxes2(self, movingBoxes, _image):
        height, width = _image.shape[:2]
    ...
    def filterDetectionResult(self,result,filter):
        boxes=result[0]
        scores=result[1]
        classname=result[2]
        ...
        for n in range(len(boxes)):
            if filter(boxes[n],scores[n],classname[n]):
                boxes2.append(boxes[n])
                scores2.append(scores[n])
                classname2.append(classname[n])
        return (boxes2,scores2,classname2)


    def filter1(self,box,score,classname):
        if classname!='others':
            return True
        else:
            return False

    def filter2(self,box,score,classname):
        if classname=='others':
            return True
        else:
            return False
    ...
    def _execute(self, sc):
        ...
        if self.USE_BGS and len(diffScores) > 0:
        # if len(diffScores) > 0:
            for n, row in enumerate(image_by_ch):
                _image = row[1]
                ...
                tmp1=movingBoxes
                tempDetectionBoxes = [ (int(_box[1] * _width)
                                      ,int(_box[0] * _height)
                                      ,int((_box[3]-_box[1])*_width)
                                      ,int((_box[2]-_box[0])*_height)) for _box in detectionBoxes]
                tmp1.extend(tempDetectionBoxes)
                tmpClassificationResult=self.classifyBoxes2(tmp1, _image)


                detection_inf_result[n]=self.filterDetectionResult((tmpClassificationResult[0][:len(detectionBoxes)]
                                            ,tmpClassificationResult[1][:len(detectionBoxes)]
                                            ,tmpClassificationResult[2][:len(detectionBoxes)]), self.filter1);
                ...
                bg_result=self.filterDetectionResult((tmpClassificationResult[0][len(detectionBoxes):]
                                                                        ,tmpClassificationResult[1][len(detectionBoxes):]
                                                                        ,tmpClassificationResult[2][len(detectionBoxes):]), self.filter1);

                bg_result_invalid=self.filterDetectionResult((tmpClassificationResult[0][len(detectionBoxes):]
                                                       ,tmpClassificationResult[1][len(detectionBoxes):]
                                                       ,tmpClassificationResult[2][len(detectionBoxes):]), self.filter2);
        ...
        tracelogger.debug('Intrusion elapsed time [%.2f]', time.time() - start)
