...
class FiresmokeVA(VAService):
    ...
    def _execute(self, sc):
        ...
        for (seq, firesmoke, ch_id, cfg_json) in image_by_ch:
            ...
            if ch_id in debug_dict:
                ...
                sc.set_out_by_ch(self.va_type, seq, self.aggregate_classify_result(crop_box_np[idx:idx+num], inf_results[idx:idx+num], self.estimator[ch_id]))
            idx = idx + num
        return sc
    ...
    def expend_box_n_resize(self, image_np, detects):
        ...
        for idx in range(len(boxes)):
            ...
            crop, box = self.__crop_expend_ares(image_np, box_t, ratio=CROP_EXPEND_RATIO)
