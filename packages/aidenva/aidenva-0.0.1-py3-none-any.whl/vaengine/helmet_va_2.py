...
class HelmetVA(VAService):
    ...
    def __init__(self, config, va_type):
        ...
        self.min_width = config.getvalue(self.conf_prefix + 'min_width', 40)
        self.min_height = config.getvalue(self.conf_prefix + 'min_height', 80)
    ...
    def _execute(self, sc):
        ...
        for (ch, image, _, cfg_json) in images_by_ch:
            ...
            if cfg_json is not None and const.HELMET_VA_NAME.upper() in cfg_json:
                if 'min_width' in cfg_json[const.HELMET_VA_NAME.upper()]:
                    try:
                        self.min_width = cfg_json[const.HELMET_VA_NAME.upper()]['min_width']
                    except Exception:
                        cfg_json_val = json.loads(cfg_json[const.HELMET_VA_NAME.upper()])
                        self.min_width = cfg_json_val['min_width']
                if 'min_height' in cfg_json[const.HELMET_VA_NAME.upper()]:
                    try:
                        self.min_height = cfg_json[const.HELMET_VA_NAME.upper()]['min_height']
                    except Exception:
                        cfg_json_val = json.loads(cfg_json[const.HELMET_VA_NAME.upper()])
                        self.min_height = cfg_json_val['min_height']

            detect = detect_by_ch[ch]
            boxes, helmet_crop_np_list = self.expend_box_n_padding_zero(image, detect)
            #print(boxes)
            # self.createCropImages(self.count, image, boxes, corp_np_list)
           # print(len(corp_np_list))
            if (self.BATCH_BY_CHANNEL == True and len(helmet_crop_np_list) > 0) or len(
                    helmet_crop_np_list) > self.CLASSIFICATION_BATCH_SIZE:
                total_inf_result.extend(self.classify._inference(helmet_crop_np_list))
                helmet_crop_np_list = []

        if len(helmet_crop_np_list) > 0:
            total_inf_result.extend(self.classify._inference(helmet_crop_np_list))
    ...
    def createCropImages(self,_count,image,boxes,inputlist):
        ...
        for box in boxes:
            ...
            tlx = int(box[1] * width)  if int(box[1] * width) > 0 else 0
            tly = int(box[0] * height) if int(box[0] * height) > 0 else 0
            brx = int(box[3] * width)  if int(box[3] * width) > 0 else 0
            bry = int(box[2] * height) if int(box[2] * height) > 0 else 0
    ...
    def expend_box_n_padding_zero(self, image_np, person_detect):
        ...
        helmet_crop_list = list()
        helmet_boxes = list()

        # temporary fix - 20.01.13
        # min_width = 40
        # min_height = 80
        ...
        for idx in range(len(boxes)):
            ...
            if (box_t[3]-box_t[1])*width >= self.min_width and (box_t[2]-box_t[0])*height >= self.min_height:
                crop, box = self.__crop_expend_ares(image_np, box_t)
                crop = self.__resize_and_padding_zero(crop, self.default_image_size)
                helmet_crop_list.append(crop)
                helmet_boxes.append(boxes[idx])

        return helmet_boxes, helmet_crop_list
    ...
    def __resize_and_padding_zero(self, image, desired_size=299):
        ...
        color = [0, 0, 0]
        return cv2.copyMakeBorder(image, top, bottom, left, right, cv2.BORDER_CONSTANT, value=color) / 255
