...
class MotionDetection:
    ...
    def detectByBackgroundSubtractor(self, ch_id, diffScore=3, isDebug=True):
        ...
        brightness = self._brightnessMap[ch_id]
        ...
        maxDiffScore=35 if isDay else 70;
        if diffScore > maxDiffScore:
            return []

        diffScore = cv2.mean(fgmask)[0]
        if diffScore > (12 if isDay else 17):
        # if diffScore > (3 if isDay else 6):
            ...
            return []
            # return self.doModtionDetectByDiff(ch_id,isDebug)
        ...
        # if isDay==False:
        #     # lightMarker=cv2.threshold((currentGray if isResize==False else cv2.resize(currentGray,(self.BG_SUB_WIDTH,self.BG_SUB_HEIGHT))),150, 200, cv2.THRESH_BINARY_INV)[1] # 불
        #     # lightMarker=cv2.threshold((currentGray if isResize==False else cv2.resize(currentGray,(self.BG_SUB_WIDTH,self.BG_SUB_HEIGHT))),brightness+60, 255, cv2.THRESH_BINARY_INV)[1] # 불
        #     # lightMarker=cv2.threshold((currentGray if isResize==False else cv2.resize(currentGray,(self.BG_SUB_WIDTH,self.BG_SUB_HEIGHT))),200, 255, cv2.THRESH_BINARY_INV)[1] # 불
        #     lightMarker=self.detectByBackgroundSubtractorNightMarker(ch_id)
        #     thresh=lightMarker
        #     # thresh=cv2.bitwise_and(thresh,lightMarker)
        #     if isDebug:
        #         cv2.imshow('inverted', lightMarker)
        ...
        if isDebug:
            ...
            cv2.putText(fgmask, str(diffScore)[0:3] + ": detection count :" + str(len(boxes)), (50, 50),
    ...
    def _isValidBoxes(self, rect, width, height, isDay):
        ...
        if w > min_width and h > min_height \
                ...
                and w < width * self.MAX_WIDTH_RATE and h < height * self.MAX_HEIGHT_RATE:
            return True
        else:
            return False
