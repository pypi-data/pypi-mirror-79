...
Usage:
    command_basename get [-s] [-p AWS_PROFILE]
    command_basename set FILE [-d] [-p AWS_PROFILE]
...
def do_set():
    ...
    for key, value in parameters.items():
        if key.startswith("-"):
            ...
            k = key[1:]
            if k.startswith("*"):
                create_parameter(k[1:], value, "SecureString")
            else:
                create_parameter(k, value)
        elif key.startswith("*"):
...
if __name__ == "__main__":
    ...
    try:
        if cmdargs["-p"]:
            session = boto3.session.Session(profile_name=cmdargs["-p"])
