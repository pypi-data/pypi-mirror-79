...
macro(DISSECT_VERSION)
  ...
  string(REGEX REPLACE "^([0-9]+).*" "\\1" PYFLANN_VERSION_MAJOR
                       "${PYFLANN_VERSION}")
  string(REGEX REPLACE "^[0-9]+\\.([0-9]+).*" "\\1" PYFLANN_VERSION_MINOR
                       "${PYFLANN_VERSION}")
...
macro(find_hdf5)
  ...
  foreach(_dir ${HDF5_INCLUDE_DIRS})
    if(EXISTS "${_dir}/H5pubconf.h")
      file(STRINGS "${_dir}/H5pubconf.h" HDF5_HAVE_PARALLEL_DEFINE
...
macro(flann_add_gtest exe)
  ...
  add_custom_target(
    test_${_testname}
    COMMAND ${exe} ARGS --gtest_print_time
    DEPENDS ${exe}
...
macro(pyflann_download_test_data _name _md5)
  ...
  add_custom_target(
    ${_dataset_name}
    COMMAND
      ${PYTHON_EXECUTABLE} ${PROJECT_SOURCE_DIR}/bin/download_checkmd5.py
      http://people.cs.ubc.ca/~mariusm/uploads/FLANN/datasets/${_name}
      ${TEST_OUTPUT_PATH}/${_name} ${_md5}
    VERBATIM)
