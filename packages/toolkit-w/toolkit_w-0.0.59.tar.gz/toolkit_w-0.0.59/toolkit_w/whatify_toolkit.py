...
class MyWhatify:
    ...
    def train_model(cls, data_name, dataset_id, target_metric=TargetMetric.RECALL_MACRO, training_time_in_minutes=6,
                    cv_folds=3, ensemble_size=5, interpt_level=Interpt.EXPLAINABLE, wait=True, skip_if_exists=True):
        with allure.step('On data_name: ' + data_name + ' ID: ' + str(dataset_id)
            ...
            try:
                task = my_toolkit.Dataset.train(
                    ...
                    timeout=training_time_in_minutes * 60, wait=wait, skip_if_exists=skip_if_exists
                )
                task_id = task['id']
