...
def store_from_context(context):
    ...
    if mongo:
        ...
        mongo.buffer_metrics = context.buffer_metrics
...
class StoreV2():
    ...
    def __init__(self, storage_creds=None, config=None, max_retries=25, run_cache_dir=None, feedback_enabled=True,  mongo_conn_str=None, 
        ...
        self.mongo = mongo_v2.MongoDB2(mongo_conn_str, run_cache_dir=run_cache_dir, store=self) if mongo_conn_str else None
    ...
    def start_child_run(self, ws_name, parent_run_name, exper_name=None, cmd_line_args=None, all_args=None, description=None, 
        ...
        self.start_run_core(ws_name, child_name, exper_name=exper_name, description=description, username=username,
            ...
            sku=sku, search_style=search_style, is_child=True)           
    ...
    def start_run_core(self, ws_name, run_name, exper_name=None, description=None, username=None,
            ...
            sku=None, search_style=None, is_child=False):
    ...
    def end_run(self, ws_name, run_name, status, exit_code, hparams_dict, metrics_rollup_dict, end_time=None, 
        ...
        self.log_run_event(ws_name, run_name, "ended", {"status": status, "exit_code": exit_code, \
            "restarts": restarts, "metrics_rollup": metrics_rollup_dict}, \
                event_time=end_time, is_aml=is_aml)
    ...
    def rollup_and_end_run(self, ws_name, run_name, aggregate_dest, dest_name, status, exit_code, primary_metric, maximize_metric, 
        ...
        if self.mongo:
            self.mongo.update_run_at_end(ws_name, run_name, status, exit_code, restarts, end_time, log_records, hparams, metrics)
    ...
    def update_run_at_end(self, ws_name, run_name, status, exit_code, restarts, end_time, log_records, hparams, metrics):
        ...
        if self.mongo:
            result = self.mongo.update_run_at_end(ws_name, run_name, status, exit_code, restarts, end_time, log_records, hparams, metrics)
    ...
    def get_log_records_for_runs(self, ws_name, id_list):
        ...
        num_workers = 20
        print("  reading log_records from storage ({} runs, {} worker processes): ".format(len(id_list), num_workers), 
            end="")

        pool = Pool(processes=num_workers) 

        started = time.time()
        results = pool.map(worker_read_log_records, work_items)
        elapsed = time.time() - started

        print(" {:.0f} secs elapsed".format(elapsed))
