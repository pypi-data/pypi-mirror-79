...
class MongoDB2():
    ...
    def __init__(self, mongo_cs, db_name=None, ws_name=None, run_cache_dir=None, 
        direct_mode=True, store=None, buffer_metrics=False):
        ...
        self.name = "mongo_v2"
        self.store = store
        self.buffer_metrics = buffer_metrics
        self.metrics = {}

        # default controls for mongo stats and logging
    ...
    def get_collection_records(self, collection, ws_name, filter, fields=None):
        ...
        # experimental code: failover to storage if records not found
        if not records and collection == "log_records":
            id_list = filter["key"]["$in"]
            records = self.store.get_log_records_for_runs(ws_name, id_list)
    ...
    def process_run_event(self, ws_name, run_name, event, orig_dd, record_dict):
        ...
        if event == "created":
            ...
            if self.buffer_metrics:
                #console.print("buffering metrics: {}".format(dd))
                self.metrics.update(dd)
                #console.print("after metrics update: self.metrics=", self.metrics)
