...
run_stats_props = {"status": 1, "last_time": 1, "exit_code": 1, "restarts": 1, "end_time": 1, "end_id": 1, "tags": 1}
...
def expand_job_entry(store, mongo, workspace, run_list, job_term):
    ...
    records = mongo.get_info_for_runs(workspace, run_filter, {"_id": 1})
    if records:
...
def remove_run_stats(dd: dict):
    run_stat_props = ["status", "last_time", "exit_code", "restarts", "end_time", "end_id", 
        "metric_names", "start_time", "duration", "run_duration", "queue_duration"]
    run_stats = {}

    for prop in run_stat_props:
        if prop in dd:
            value = dd[prop]
...
def get_run_property_dicts():
    ...
    user_to_actual = {"box": "box_name", "created": "create_time", "child": "is_child", "cluster": "cluster",
        ...
        "status": "status", "target": "compute", "username": "username", "vc": "vc", "workspace": "ws_name", "xt_build": "xt_build", 
...
def get_filtered_sorted_limit_runs(store, config, show_gathering, col_dict=None, preserve_order=False, 
    ...
    records, using_default_last, last = builder.get_mongo_records(mongo, filter_dict, workspace, which="runs", 
        ...
        flatten_records=flatten_records, args=args)
