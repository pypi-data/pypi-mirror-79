...
class OperetoClient(object):
    ...
    def __init__(self, **kwargs):
        ...
        def _get_agent_credentials():
            ...
            ## Temp: supports Opereto 2.0
            agent_version = os.environ.get('agent_version', None)
            if agent_version:
                agent_config_file = '/etc/default/opereto-agent'
                if os.path.exists(agent_config_file):
                    with open(agent_config_file, 'r') as af:
                        config = af.readlines()
                        for item in config:
                            (key,val) = item.strip().split('=')
                            if key=='OPERETO_HOST':
                                self.input.update({'opereto_host': val})
                            elif key=='OPERETO_USER':
                                self.input.update({'opereto_user': val})
                            elif key=='OPERETO_PASS':
                                self.input.update({'opereto_password': val})
