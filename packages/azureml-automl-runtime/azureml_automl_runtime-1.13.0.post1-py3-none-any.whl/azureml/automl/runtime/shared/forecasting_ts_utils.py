...
from azureml.automl.core.shared.exceptions import (
    AllLabelsMissingException, InsufficientDataException, ValidationException
)
...
def get_stl_decomposition(ts_values: np.ndarray, seasonality: int = 1) \
    ...
    if np.any(pd.isnull(ts_values)):
        raise AllLabelsMissingException(
            'Cannot calculate STL decomposition on an array with NA values.', has_pii=False)
