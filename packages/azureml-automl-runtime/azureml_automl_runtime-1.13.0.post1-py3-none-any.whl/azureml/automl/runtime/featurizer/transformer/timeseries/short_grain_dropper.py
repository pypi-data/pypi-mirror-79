...
class ShortGrainDropper(AzureMLForecastTransformerBase):
    ...
    DROPPING_GRAIN_TEMPL = ("Dropping series with identifier {}. Reason: "
                            "the series was not present in the training set "
                            "or was too short.")
    ...
    def fit(self, X: TimeSeriesDataFrame, y: Any = None) -> 'ShortGrainDropper':
        ...
        if not self._grains_to_keep:
            raise InsufficientDataException(
                "All time series are too short please check the data or decrease target_lags, "
    ...
    def transform(self, X: TimeSeriesDataFrame, y: Any = None) -> TimeSeriesDataFrame:
        ...
        if(result.shape[0] == 0):
            raise InsufficientDataException(
                "All time series were removed because "
