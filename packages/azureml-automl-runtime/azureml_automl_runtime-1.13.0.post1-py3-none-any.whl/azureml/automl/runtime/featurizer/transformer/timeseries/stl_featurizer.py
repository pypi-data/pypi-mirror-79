...
from azureml.automl.core.shared.exceptions import (
    InsufficientDataException, ConfigException, ClientException, DataException, TransformException)
            ...
            if grain not in self._last_observation_dates.keys() or \
               grain not in self._first_observation_dates.keys():
                raise DataException._with_error(AzureMLError.create(
                    GrainAbsent, target="grain", grain=grain,
                    reference_code=ReferenceCodes._STL_GRAIN_ABSENT)
                )
