...
from azureml.automl.core.shared.exceptions import (
    DataException, InsufficientDataException, ResourceException, ValidationException)
...
def validate_timeseries_training_data(automl_settings: AutoMLBaseSettings,
    ...
    if automl_settings.time_column_name in grain_set:
        raise GrainAndTimeOverlapException(
            ...
            "Please remove it from time series identifiers list.", has_pii=False)
...
def _check_grain_min_points(number_of_data_points: int,
    ...
    :raises: InsufficientDataException
    """
    ...
    if number_of_data_points < min_points:
        ...
        if grain_name is None:
            pii_message = (
                ...
                "reducing max_horizon, the number of cross validations, lags or rolling window size.".format(
                    automl_settings.n_cross_validations,
                    automl_settings.max_horizon, lags, window_size,
        ...
        pii_message = (
            "The data provided is insufficient for training series: [{}] for a valid training with cv {}, "
            ...
            "dropping that particular series.".format(
                ", ".join(
                    [
                        str(grain) for grain in grain_name]),
                automl_settings.n_cross_validations,
    ...
    if duplicateRowsDF.shape[0] > 0:
        if grain_column_names is not None and len(grain_column_names) > 0:
            message = ("Found duplicated rows for {0} and {1} combinations. "
                       ...
                       format([time_column_name], grain_column_names, constants.ErrorLinks.DUPLICATED_INDEX.value))
            ...
            message = ("The specified time column, '{}', contains rows "
                       ...
                       "For more information on time series identifiers and forecasting "
                       "configurations visit {}.".
                       format([time_column_name], constants.ErrorLinks.DUPLICATED_INDEX.value))
