...
from azureml.automl.core.shared.exceptions import DataException, ConfigException, InvalidDataTypeException
    ...
    def _convert_input_to_onnx_compatible_types(self, X):
        ...
        for column in X:
            ...
            if col_type == NumericalDtype.Floating or col_type == NumericalDtype.MixedIntegerFloat:
                ...
                raise InvalidDataTypeException(msg.format(col_type)).with_generic_msg(msg.format('[Masked]'))
    ...
    def predict(self, X: pd.DataFrame, with_prob: bool = True) -> Tuple[Any, Any]:
        ...
        if _onnxrt_present:
            if not isinstance(X, pd.DataFrame):
                raise InvalidDataTypeException(
                    'The inference helper only supports the X with pandas DataFrame type.',
                    has_pii=False)
