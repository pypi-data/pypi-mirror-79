...
from azureml.automl.core.shared._diagnostics.automl_error_definitions import InvalidArgumentType, \
    InvalidDampingSettings, InvalidSTLFeaturizerForMultiplicativeModel, ConflictingValueForArguments
...
from azureml.automl.core.shared.exceptions import (
    InsufficientDataException, ConfigException, ClientException, EmptyDataException, TransformException)
...
class STLFeaturizer(AzureMLForecastTransformerBase):
        ...
        for grain, df_one in X.groupby_grain():
            if grain not in self._last_observation_dates.keys() or \
               grain not in self._first_observation_dates.keys():
                error_msg = 'The time series with identifier {} is not in the training set.'
    ...
    def _fit_one_grain(self,
        ...
        if df_one.shape[0] < 2:
            error_msg = ('STL featurizer requires at least two data points in each time series. \
                    Series {} contains only one point. Please remove this series from the dataset.')
        ...
        if self.seasonality >= len(df_one):
            error_msg = "Must have at least a full season of data for each grain. \
                                  The series with identifier {} has {} observations with seasonality={}"
            raise InsufficientDataException(
