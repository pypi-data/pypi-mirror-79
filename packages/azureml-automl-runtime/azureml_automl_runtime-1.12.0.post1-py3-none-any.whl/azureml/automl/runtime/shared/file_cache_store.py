            ...
            for k, v in zip(keys, values):
                try:
                    ...
                    if e.errno == errno.ENOSPC:
                        ...
                        msg = "Failed to write to cache store path for key {}. "\
                            "Exception type: OSError {} {} {}".format(
                                k, e.errno, errno.errorcode.get(e.errno, 'Unknown'), os.strerror(e.errno))
                        raise CacheStoreCorruptedException.from_exception(e, msg=msg).with_generic_msg(msg)
                    msg = 'Failed to add key {} to cache. Exception type: OSError {} {} {}'.format(
                        k, e.errno, errno.errorcode.get(e.errno, 'Unknown'), os.strerror(e.errno))
                    raise ResourceException.from_exception(e, msg)
        ...
        with self.log_activity():
            for key in keys:
                try:
                    ...
                    logging_utilities.log_traceback(e, logger, is_critical=False)
                    msg = "Failed to access the cache store path for key {}. Exception type: {}".format(
                        key, e.__class__.__name__)
                    raise CacheStoreCorruptedException.from_exception(e).with_generic_msg(msg)
                except Exception as e:
                    logging_utilities.log_traceback(e, logger, is_critical=False)
