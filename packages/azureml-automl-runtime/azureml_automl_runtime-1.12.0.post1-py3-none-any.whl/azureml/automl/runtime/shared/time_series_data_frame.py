...
from azureml.automl.core.shared._diagnostics.automl_error_definitions import GrainContainsEmptyValues
...
def _fill_datetime_gap_single_slice_key(df, grain_level, freq, origin=None,
    ...
    if not set(time_index).issubset(onfreq_time):
        error_message = ('In time series identifier(s) {}, '
                         'one or more date(s) do not align with the desired frequency {}')
...
class TimeSeriesDataFrame(pd.DataFrame):
    ...
    GRAIN_ABSENT_TEMPL = 'time series id %s is not found in dataframe.'
    ...
    DUPLICATED_INDEX_ERROR = ("The data contains more than one value for a "
                              "`time_index` and time series identifier combination. "
                              "Please ensure that time series identifier columns and time"
            ...
            if self.time_colname is not None \
                ...
                if not np.all(all_equal.values):
                    ...
                    error_message_no_client_info = (
                        ...
                        "which shares the same time series identifier column names"
                        "and time_colname value,"
