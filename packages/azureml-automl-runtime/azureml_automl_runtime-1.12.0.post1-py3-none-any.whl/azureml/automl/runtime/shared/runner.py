...
class ClientRunner:
    ...
    def _run_train_full(self, dataset, pipeline_spec,
        ...
        with log_utils.log_activity(logger,
            ...
            score_full = self._compute_metrics(X_full, y_full, X_full, y_full,
                                               pipeline, dataset,
            ...
            with dataset.open_dataset():
                ...
                if constants.TrainingType.TrainValidateTest in sets_to_run:
                    ...
                    try:
                        ...
                        X_train, y_train, sample_weight_train = dataset.get_train_set()
                        scores = self._compute_metrics(X_train, y_train, X_train, y_train,
                                                       fit_model, dataset,
                        ...
                        scores = self._compute_metrics(X_test, y_test, X_train, y_train,
                                                       fit_model, dataset,
                ...
                if constants.TrainingType.TrainFull in sets_to_run:
                    ...
                    try:
                        ...
                        if dataset.has_test_set():
                            X_test, y_test, sample_weight_test = dataset.get_test_set()
                            scores = self._compute_metrics(X_test, y_test, X_full, y_full,
                                                           fit_model, dataset,
                                                           sample_weight=sample_weight_test)
    ...
    def _compute_metrics(
        ...
        with log_utils.log_activity(logger,
            ...
            Contract.assert_true(
                self.task in [Tasks.CLASSIFICATION, Tasks.REGRESSION],
