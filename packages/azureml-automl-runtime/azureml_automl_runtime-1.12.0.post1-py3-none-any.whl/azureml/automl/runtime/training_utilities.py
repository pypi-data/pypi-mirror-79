...
from azureml.automl.core.shared._diagnostics.automl_error_definitions import (
    AllTargetsUnique, AllTargetsOverlapping,
    ConflictingValueForArguments, ExperimentTimeoutForDataSize,
    FeatureUnsupportedForIncompatibleArguments, FeaturizationConfigColumnMissing,
    InsufficientMemoryWithHeuristics, InvalidArgumentWithSupportedValues,
...
from azureml.automl.runtime import dataprep_utilities, frequency_fixer, _time_series_training_utilities
...
def check_x_y(x: DataInputType,
    ...
    if not automl_settings.enable_streaming:
        _column_value_number_check(y, automl_settings.task_type)
...
def check_sample_weight(x: DataInputType,
    ...
    if sample_weight_min < 0:
        raise ValidationException._with_error(
            AzureMLError.create(
                ArgumentOutOfRange, target="check_sample_weight",
                argument_name="check_sample_weight ({})".format(str(sample_weight_min)), min=0, max="inf",
...
def validate_timeseries_training_data(automl_settings: AutoMLBaseSettings,
    ...
    if automl_settings.time_column_name in grain_set:
        raise GrainAndTimeOverlapException(
            "Time column name is present in the time series identifier columns. "
    ...
    if automl_settings.n_cross_validations is None and X_valid is None:
        ...
        X = _time_series_training_utilities._get_df_or_raise(X, x_raw_column_names)
        ...
        tsdf_valid = None
        if X_valid is not None:
            X_valid = _time_series_training_utilities._get_df_or_raise(X_valid, x_raw_column_names)
    ...
    if data_points < min_points:
        ...
        if grain_names is None:
            ...
            pii_message = (
                ...
                "max_horizon {} lags {} and rolling window size {}. The current series has only {} points. "
                "Please consider reducing max_horizon, n_cross_validations, or lags, rolling window size or "
                "dropping that particular series.". format(
                    ",".join(
                        [
                    ...
                    data_points))
            raise InsufficientDataException(
                pii_message).with_generic_msg("The data provided is insufficient for some training series.")
...
def _check_timeseries_input_and_get_tsdf(
    ...
    if not is_validation_data:
        _column_value_number_check(y, constants.Subtasks.FORECASTING)
    ...
    if automl_settings.grain_column_names is not None:
        ...
        try:
            ...
            for data_tuple in tsdf.groupby_grain():
                ...
                err_msg = ("Time series frequency cannot be inferred for time series identifier(s) [{}]. "
                    ...
                    if tsdf_grain.shape[0] != 1:
                        ...
                        if freq is None:
                            raise DataFrameFrequencyException(err_msg).with_generic_msg(
                                "Frequency cannot be inferred for the [masked] time series identifier.")
            ...
            raise DataFrameFrequencyException(
                "A non-specific error occurred checking frequencies across series.", has_pii=False)
    ...
    if duplicateRowsDF.shape[0] > 0:
        if grain_column_names is not None and len(grain_column_names) > 0:
            message = ("Found duplicated rows for {0} and {1} combinations. "
                       ...
                       "before passing to AutoML. For more information on time series identifiers and forecasting "
                       "configurations visit {2}.".
                       format([time_column_name], grain_column_names, ErrorLinks.DUPLICATED_INDEX.value))
            raise DuplicatedIndexException(exception_message="Duplicates in time and "
                                           "time series identifier combinations.",
                                           pii_message=message)
        else:
            message = ("The specified time column, '{}', contains rows "
                       "with duplicate timestamps. If your data contains "
                       "multiple time series, review the time series identifier column setting "
                       "to define the time series identifiers for your data. "
                       "For more information on time series identifier s and forecasting "
...
def _check_timeseries_alignment_single_grain(grain_level: Any, tsdf: TimeSeriesDataFrame,
    ...
    if not set(time_index).issubset(onfreq_time):
        error_message = (
            ...
            'data for the following time series identifier(s) [{}]. The expected frequency '
            'is a data point every \'{}\'. Review the time series identifier(s) and ensure '
...
def _check_columns_present(df: pd.DataFrame, timeseries_param_dict: Dict[str, str]) -> None:
    ...
    if len(missing_col_names) != 0:
        raise DataFrameMissingColumnException(
            pii_message=msg.format(constants.TimeSeries.TIME_SERIES_ID_COLUMN_NAMES),
