"Defines all functions within the rds (relational database service) module."
...
from pandas import DataFrame, read_sql_query, to_datetime, to_numeric, \
    StringDtype, BooleanDtype, Int64Dtype, Int32Dtype


def config(file_path: str = "/home/ec2-user/SageMaker/.config/database.ini",
    ...
    imported in to the main python script. This file was adapted from 
    ...
    if parser.has_section(section):  # Checks to see (postgresql) parser exists
...
def import_data(sql_query: str, db_cred: dict, df_meta: DataFrame = None,
                post_rename: dict = None, convert_dtypes: bool = True,
                query_name: str = None) -> DataFrame:
    ...
    try:
        ...
        if (df_meta is not None) & (convert_dtypes):
            for col, data_dtype in df_.dtypes.iteritems():
                ...
                # Check schema
                df_meta_sub = df_meta.loc[df_meta.name == col, ]
                ...
                if df_meta_sub.empty is False:
                    ...
                    if df_meta_sub.type.nunique() > 1:
                        ...
                        logger.warning("\t Warning:'%s' has multiple dtypes "
                                       "in schema. Using most popular dtype "
                                       " %s for conversion", col, schema_dtype)
                    else:
                    ...
                    # Float64s
                        ...
                        df_[col] = to_numeric(df_[col],
                                              errors="coerce",
                                              downcast="float")
                    # Float64s
                    elif bool(match(r"NUMERIC\([0-9]{1,6},\s[0-9]{1,6}\)",
                                    schema_dtype)):
                        df_[col] = to_numeric(df_[col],
                                              errors="coerce",
                                              downcast="float")
                    ...
                    # Dates and timestamps
                        ...
                        if data_dtype == dtype('O'):
                            df_[col] = to_datetime(df_[col],
                                                   ...
                                                   errors="coerce")
                    # Strings 
                    elif (schema_dtype in ["TEXT"] 
                          or bool(match(r"VARCHAR\([0-9]{1,6}\)",
                                        schema_dtype))):
                        df_[col] = df_[col].astype(StringDtype())
            ...
            logger.warning("""\tNo dtype conversion was executed. Meta data 
        ...
        logger.info("\tOK! Query executed in %f [s]. Shape = %s""",
                    time() - start,
                    df_.shape)

    except Exception:
        traceback_str = format_exc()
        logger.critical("Failed to execute query (%s)", traceback_str)
        raise Exception
...
def import_meta_data(db_cred: dict, 
                     like_str: str = "maximoworker") -> DataFrame:
    ...
    try:
        ...
        df_meta = DataFrame(
            data=[{"table": t, "name": c.name, "type": str(c.type),
                   "python_type": c.type.python_type,
                   "nullable": c.nullable} for t in tables for c in
                  Table(t, metadata, autoload=True,
                        autoload_with=engine).columns])
        
        # Set dtypes
        df_meta = df_meta.astype({"table": StringDtype(),
                                  "name": StringDtype(),
                                  "type": StringDtype(), 
                                  "nullable": bool})
        # Log results
        logger.info("""\tOK! Query executed in %.3f [s].
                    Shape = %s""", (time() - start), df_meta.shape)
