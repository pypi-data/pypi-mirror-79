...
def import_file(bucket: str, s3key: str, obj_class: str = "pandas", **kwargs):
    ...
    logger.info("""Load settings:\n\tobj_class=%s\n\t
        bucket=%s\n\ts3key=%s""", obj_class, bucket, s3key)
...
def list_files_in_path(bucket, prefix, drop_sub_folders=True, 
                       remove_prefix=True, subset_ext=None, remove_ext=False,):
