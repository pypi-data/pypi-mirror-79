...
class Configuration:
    ...
    def __init__(self, config_file_path=None):
        ...
        if not isinstance(content, dict):
            raise ValueError('Configuration file "{}" does not contain a dictionary'.format(content))
    ...
    def __is_path(self, path):
        """
        Funzione che controlla se la stringa passata descrive un path
        Una chiave è un path se il suo valore è a sua volta un dizionario
        Ritorna True se è un path
        :param path:            stringa da verificare
        :throw ValueError:      se la chiave non ha un formato valido
        :return:                True se la chiave esiste, False se la chiave non esiste
        """

        # come prima cosa controllo che la chiave abbia il formato corretto
        if not self.__is_a_valid_key(path):
            raise ValueError('Searched key does not respect key standards (dots separated strings).')

        # ottengo la lista di chiavi
        keys_list = str.split(path, '.')
        ...
        # se la chiave è stata trovata verifico se il suo valore è valido
        return not self.__is_a_valid_value(content)
