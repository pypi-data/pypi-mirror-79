...
class SpacyFactor(object):
    def __init__(self, issue_type: str, simple_description: str, description: Optional[str] = None, debug=False):
    ...
    def __call__(
        ...
        learn_more = spacy_span._.learn_more if spacy_span.has_extension("learn_more") else ""
        paragraph = spacy_span._.paragraph if spacy_span.has_extension("paragraph") else False
        header = spacy_span._.header if spacy_span.has_extension("header") else ""
        meta_dict = spacy_span._.meta_dict if spacy_span.has_extension("meta_dict") else {}
        simple_description = spacy_span._.simple_description if spacy_span.has_extension("simple_description") else self.simple_description
