...
class SshTransport(Transport):  # pylint: disable=too-many-public-methods
    ...
    def _get_gss_deleg_creds_suggestion_string(cls, computer):
        ...
        return convert_to_bool(
            str(config.get('gssapidelegatecredentials', 'no')))
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._load_system_host_keys = kwargs.pop('load_system_host_keys',
                                                 False)
    ...
    def open(self):
        ...
        if 'key_filename' in connection_arguments and not connection_arguments[
                'key_filename']:
    ...
    def open_file_transport(self):
        ...
        try:
            ...
            raise InvalidOperation(
                'SFTP seems not functional. Try to use ssh_only transport instead ?'
            )
        self._is_open = True
    ...
    def close(self):
        ...
        if not self._is_open:
            raise InvalidOperation(
                'Cannot close the transport: it is already closed')
    ...
    def sshclient(self):
        if not self._is_open:
            raise TransportInternalError(
                'Error, ssh method called for SshTransport without opening the channel first'
            )
        return self._client
    ...
    def rmtree(self, path):
        ...
        if retval == 0:
            if stderr.strip():
                self.logger.warning(
                    'There was nonempty stderr in the rm command: {}'.format(
                        stderr))
            return True
    ...
    def put(self,
            ...
            dereference=True,
            overwrite=True,
            ignore_nonexisting=False):  # pylint: disable=too-many-arguments,too-many-branches,arguments-differ
        """
        ...
        if self.has_magic(localpath):
            if self.has_magic(remotepath):
                raise ValueError(
                    'Pathname patterns are not allowed in the destination')
            ...
            for file in to_copy_list:
                if os.path.isfile(file):
                    if rename_remote:  # copying more than one file in one directory
                        ...
                        remotefile = os.path.join(remotepath,
                                                  os.path.split(file)[1])
                        self.putfile(file, remotefile, callback, dereference,
                                     overwrite)
                        ...
                        self.putfile(file, remotepath, callback, dereference,
                                     overwrite)
                else:
                    self.puttree(file, remotepath, callback, dereference,
                                 overwrite)
            ...
            if os.path.isdir(localpath):
                ...
                if self.isdir(remotepath):
                    remote = os.path.join(remotepath,
                                          os.path.split(localpath)[1])
                    self.putfile(localpath, remote, callback, dereference,
                                 overwrite)
                else:
                ...
                if not ignore_nonexisting:
                    raise OSError(
                        'The local path {} does not exist'.format(localpath))
    ...
    def putfile(self,
                ...
                callback=None,
                dereference=True,
                overwrite=True):  # pylint: disable=arguments-differ
        """
    ...
    def puttree(self,
                ...
                callback=None,
                dereference=True,
                overwrite=True):  # pylint: disable=too-many-branches,arguments-differ,unused-argument
        """
    ...
    def get(self,
            ...
            dereference=True,
            overwrite=True,
            ignore_nonexisting=False):  # pylint: disable=too-many-branches,arguments-differ,too-many-arguments
        """
        ...
        if self.has_magic(remotepath):
            ...
            for file in to_copy_list:
                if self.isfile(file):
                    if rename_local:  # copying more than one file in one directory
                        # here is the case isfile and more than one file
                        remote = os.path.join(localpath,
                                              os.path.split(file)[1])
                        self.getfile(file, remote, callback, dereference,
                                     overwrite)
                    else:  # one file to copy on one file
                        self.getfile(file, localpath, callback, dereference,
                                     overwrite)
                else:
                    self.gettree(file, localpath, callback, dereference,
                                 overwrite)
            ...
            if self.isdir(remotepath):
                ...
                if os.path.isdir(localpath):
                    ...
                    self.getfile(remotepath, remote, callback, dereference,
                                 overwrite)
                else:
                ...
                if ignore_nonexisting:
                    ...
                    raise IOError(
                        'The remote path {} does not exist'.format(remotepath))
    ...
    def gettree(self,
                ...
                callback=None,
                dereference=True,
                overwrite=True):  # pylint: disable=arguments-differ,unused-argument
        """
        ...
        if not os.path.isdir(
            ...
            os.makedirs(localpath,
                        exist_ok=True)  # and make a directory at its place
        ...
        for item in item_list:
            ...
            if self.isdir(os.path.join(remotepath, item)):
                self.gettree(os.path.join(remotepath, item),
                             os.path.join(dest, item))
            else:
                self.getfile(os.path.join(remotepath, item),
                             os.path.join(dest, item))
    ...
    def copy(self,
        ...
        if not remotedestination:
            raise ValueError('Input to copy() must be a non empty string. ' +
                             'Found instead %s as remotedestination' %
                             remotedestination)
        ...
        if self.has_magic(remotesource):
            ...
            if len(to_copy_list) > 1:
                if not self.path_exists(remotedestination) or self.isfile(
                        remotedestination):
                    raise OSError(
                        "Can't copy more than one file in the same destination file"
                    )
    ...
    def _exec_cp(self, cp_exe, cp_flags, src, dst):
        ...
        if retval == 0:
            if stderr.strip():
                self.logger.warning(
                    'There was nonempty stderr in the cp command: {}'.format(
                        stderr))
        else:
            self.logger.error(
    ...
    def rename(self, oldpath, newpath):
        ...
        if not newpath:
            raise ValueError(
                'Destination {} is not a valid string'.format(newpath))
    ...
    def _exec_command_internal(self, command, combine_stderr=False,
                               bufsize=-1):  # pylint: disable=arguments-differ
        """
        ...
        channel.exec_command('bash -l -c ' +
                             escape_for_bash(command_to_execute))
    ...
    def exec_command_wait(self,
        ...
        if stdin is not None:
            ...
            try:
                ...
                raise ValueError(
                    'stdin can only be either a string of a file-like object!')
        ...
        stderr.channel.settimeout(
            0.1)  # Maybe redundant, as this could be the same channel.

        while True:
            chunk_exists = False

            if stdout.channel.recv_ready(
            ):  # True means that the next .read call will at least receive 1 byte
                chunk_exists = True
                try:
        ...
        return (retval, b''.join(stdout_bytes).decode('utf-8'),
                b''.join(stderr_bytes).decode('utf-8'))
    ...
    def gotocomputer_command(self, remotedir):
        ...
        if 'key_filename' in self._connect_args and self._connect_args[
                'key_filename']:
        ...
        connect_string = (
            ...
            .format(further_params=further_params_str,
    ...
    def symlink(self, remotesource, remotedestination):
        ...
        if self.has_magic(source):
            ...
            for this_source in self.glob(source):
                ...
                this_dest = os.path.join(remotedestination,
                                         os.path.split(this_source)[-1])
