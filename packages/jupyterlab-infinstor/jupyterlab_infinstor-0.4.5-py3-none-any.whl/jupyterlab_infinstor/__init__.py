...
class AddXformHandler(APIHandler):  # pylint: disable=abstract-method
    def process_xformcode(self, xfin):
        ...
        quoted_dockerfile = quote(dockerfile)
        return quoted_clean_code, quoted_dockerfile
    ...
    def post(self, path=""):
        ...
        cleaned_xformcode, cleaned_dockerfile = self.process_xformcode(xformcode)

        if (len(cleaned_dockerfile) > 0):
            payload = "xformname=" + xformname + "&xformcode=" + cleaned_xformcode\
                + "&dockerfile=" + cleaned_dockerfile
        else:
            payload = "xformname=" + xformname + "&xformcode=" + cleaned_xformcode
        for retry in range(2):
            headers = {
