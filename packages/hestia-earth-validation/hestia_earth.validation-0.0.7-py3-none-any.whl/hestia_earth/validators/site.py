...
from hestia_earth.gadm import exec_request
...
def validate_region(site: dict):
    ...
    return region_id[0:8] == country.get('@id') or {
        'level': 'error',
        'dataPath': '.region',
        'message': f"must be within {country.get('name')}"
    }
...
def validate_coordinates(site: dict):
    ...
    res = exec_request(gadm_id, latitude=latitude, longitude=longitude)
    id = res.get('id')
...
def validate_site(site: dict):
    return [
        ...
        validate_coordinates(site) if 'latitude' in site and 'longitude' in site else True,
