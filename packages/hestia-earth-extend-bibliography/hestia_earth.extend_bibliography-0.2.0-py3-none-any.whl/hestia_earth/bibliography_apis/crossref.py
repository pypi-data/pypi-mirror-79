...
from .utils import MAXIMUM_DISTANCE, find_closest_result, extend_bibliography, remove_empty_values
...
def search(queries, cr, title):
    [item, distance] = find_closest_result(queries, title, exec_search(cr))
...
def extend_crossref(titles, **kwargs):
    ...
    for title in titles:
        if len(title) > 0:
            (biblio, authors) = search(queries, cr, title)
