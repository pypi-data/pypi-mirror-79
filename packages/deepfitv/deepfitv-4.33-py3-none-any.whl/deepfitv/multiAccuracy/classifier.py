...
#         net.bias_output = tf.Variable(tf.compat.v1.random_normal([2]),name='9')
        ...
        if (self.group=="race"):
            ...
            errs.append(100 * np.mean((probs[idxs]>0.5)!=y_test.iloc[idxs,0]))
        elif(self.group=="socioeconomic_category"):
            groups=['all','socioeconomic_status_High','socioeconomic_status_Low','socioeconomic_status_Middle']
            #groups = ['all', 'F', 'M', 'B', 'N', 'BF', 'BM', 'NF', 'NM']
            errs = []
            idxs = list(range(0,1600))
            errs.append(100 * np.mean((probs[idxs]>0.5)!=y_test.iloc[idxs,0]))
            idxs = np.where((X_test['socioeconomic_status_High']==1))[0]
            #print(idxs)
            errs.append(100 * np.mean((probs[idxs]>0.5)!=y_test.iloc[idxs,0]))
            idxs = np.where((X_test['socioeconomic_status_Low']==1) )[0]
            errs.append(100 * np.mean((probs[idxs]>0.5)!=y_test.iloc[idxs,0]))
            idxs = np.where((X_test['socioeconomic_status_Middle']==1) )[0]
            errs.append(100 * np.mean((probs[idxs]>0.5)!=y_test.iloc[idxs,0]))
            ...
            idxs = list(range(0,1600))
            errs.append(100 * np.mean((probs[idxs]>0.5)!=y_test.iloc[idxs,0]))
            idxs = np.where((X_test[X_test['gender']==0]))[0]
            #print(idxs)
            errs.append(100 * np.mean((probs[idxs]>0.5)!=y_test.iloc[idxs,0]))
            idxs = np.where((X_test[X_test['gender']==1]))[0]
            errs.append(100 * np.mean((probs[idxs]>0.5)!=y_test.iloc[idxs,0]))
