...
def batch_result_impl(**kwargs):
    ...
    if not kwargs.get("esn", None) and not kwargs.get("ip", None) and not kwargs.get("serial", None):
        logger.critical("No device identifier was set. Batch ID requires a device ID (--esn, --ip, or --serial) and a batch ID. "
                        "You can get more general status with the status command.")
        sys.exit(1)
