...
@click.option("--eyepatch", is_flag=True, help="Remove EyePatch tests")
@click.option(
    "--save-to",
    "result_file",
    type=click.File("w", encoding="utf-8"),
...
def filter_(*args, **kwargs):  # filter is a reserved word in python. careful!
    ...
    Examples:
        ...
        cat plan.json | nts filter ACT-004-TC11 AUDIO-001-TC1 > filtered.json

        nts get-plan | nts filter --eyepatch > filtered.json

        nts filter --testplan plan.json | nts run
    """
...
def print_upgrade_command():
    print("pip3 install -U \"ntscli-client<1.0.0\"")
