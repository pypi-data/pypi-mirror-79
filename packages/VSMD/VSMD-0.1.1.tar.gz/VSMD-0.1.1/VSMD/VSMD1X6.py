...
class CommonCMD(object):
    ...
    def move_dis(device: str, dis: int):
        tar = CommonCMD._get_tar_device(device)
        src = DeviceTable.Pi
        cmd = CommonCMD.__easy_cmd(tar=tar, src=src, cw=CWTable.CMD, cmd0reg=CMDTable.RMV, data=2)
        return cmd
...
# print(CommonCMD.save("X0"))

# distance =2
#
# for cmd in [CommonCMD.enable_motor("ALL"), CommonCMD.disable_motor("Y"),
#             CommonCMD.disable_motor("Z"), CommonCMD.move_motor("ALL", 1.0), CommonCMD.stop_motor("All")]:
#     print(cmd)
