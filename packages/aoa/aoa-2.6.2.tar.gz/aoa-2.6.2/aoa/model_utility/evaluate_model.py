...
class EvaluateModel(BaseModel):
    ...
    def evaluate_model_local(self, model_id: str = None, base_path: str = None, data_conf: dict = None):
        ...
        if os.path.exists("{}/output".format(model_artefacts_path)):
    ...
    def __input_boolean(self, name, tooltip='', called_from_test=False):

        if tooltip != '':
            print(tooltip)

        value = input("Enter (y)es or (n)o: ")
        if value == '' or value not in ('y', 'n'):
            print('Please select y or n.')
            print('You may cancel at anytime by pressing Ctrl+C')
            print("")
            return self.__input_boolean(name, tooltip, called_from_test)
