...
from django_grpc_secure.utils import add_servicers, extract_handlers, create_server
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('--host', type=str, default='[::]', help="manufacture set Host")
        parser.add_argument('--secure', action='store_true', default=False, help="manufacture set Host")
    ...
    def _serve(self, max_workers, port, *args, **kwargs):
        ...
        server = create_server(max_workers, port, kwargs["host"], kwargs["secure"])
        server.start()

        self.stdout.write("Server is listening %s:%s" % (kwargs['host'], port))
