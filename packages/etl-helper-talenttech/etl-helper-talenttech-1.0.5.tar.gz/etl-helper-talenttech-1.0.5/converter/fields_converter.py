...
class FieldsConverter:
    ...
    def __get_fields_definition(self, table_name, field_name, field_type, attributes={}):
        """
        ...
        if "text_sizes" in attributes and field_name in attributes["text_sizes"]:
            if field_type == "long varchar" and attributes["text_sizes"][field_name] < 65000:
                field_type = "varchar"
            text_size = "(" + str(attributes["text_sizes"][field_name]) + ")"
        else:
            text_size = ""
        if "constrained_columns" in attributes and field_name in attributes["constrained_columns"]:
            constrained = " " + base_converter_params[self.to_db]["constrained_columns_str"].format(table_name=table_name)
        else:
            constrained = ""
    ...
    def get_inner_ddl(self, table_name):
        """
        get ddl by standart queries in
        :param table_name:
        :return:
        """
        if self.to_db == "vertica":
            sql = """SELECT 'select export_tables('''', ''' || table_schema || '.' || table_name || ''');'
                     FROM v_catalog.tables
                     where table_schema = '{schema}' and table_name='{table_name}';""".format(
                schema=self.db_worker_to.schema, table_name=table_name
            )
            res = self.db_worker_to.conn.execute(sql)
            get_ddl_sql = [r for r in res][0]
            res = self.db_worker_to.conn.execute(get_ddl_sql[0])
            return [r for r in res][0]
            # get_ddl_sql = [r for r in res][0]
            # cur.execute(get_ddl_sql[0])
            # row = cur.fetchone()
            #ddl = [x + ";" for x in row[0].replace("\n", "").split(";") if x != ""]
    ...
    def generate_create(self, fields_new, table_name):
        ...
        if len(self.advanced_features) > 0:
            self.log("advanced ddl creator. get additional attrs")
    ...
    def create_list_of_tables(
            self, tables, to_create=True, dir_ddl=None
    ):
        result_list = []
        for table in tables:
        ...
        if to_create:  # refit meta
            self.db_worker_to.init_meta()
        return result_list
