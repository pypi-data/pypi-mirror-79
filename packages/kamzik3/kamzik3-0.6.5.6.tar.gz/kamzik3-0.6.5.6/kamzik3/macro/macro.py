...
class Macro(Common, Device):
    ...
    def __init__(self, device_id, chain=None, repeat_count=0, wait_after=units.Quantity(0, "s"), retry_count=0,
                 trigger_log=True, comment=None):
        self.logger = logging.getLogger("Macro.{}".format(device_id))
        self.comment = comment
        self.chain = chain
        Common.__init__(self, device_id, repeat_count, wait_after, retry_count, trigger_log)
    ...
    def _init_attributes(self):
        ...
        self.create_attribute(ATTR_STATE, default_vlaue=IDLE, readonly=True, default_type=np.uint8)
        self.create_attribute(ATTR_POINTS_COUNT, default_vlaue=1, readonly=True, default_type=np.uint64)
        self.create_attribute(ATTR_POINT_INDEX, default_vlaue=0, readonly=True, default_type=np.uint64)
        self.create_attribute(ATTR_PROGRESS, default_vlaue=0, readonly=True, default_type=np.float16, unit="%",
                              min_value=0, max_value=100, decimals=2)
        self.create_attribute(ATTR_CHAIN_LINK, default_vlaue=0, readonly=True, default_type=np.uint64)
        self.create_attribute(ATTR_CHAIN_LINK_OUTPUT, default_vlaue="", readonly=True)
        self.create_attribute(ATTR_RUNNING_TIME, default_vlaue=0, readonly=True)
        self.create_attribute(ATTR_ESTIMATED_TIME, default_vlaue=0, readonly=True)
        self.create_attribute(ATTR_ESTIMATED_END, default_vlaue=0, readonly=True)
        self.create_attribute(ATTR_REMAINING_TIME, default_vlaue=0, readonly=True)
        self.create_attribute(ATTR_STARTED_AT, default_vlaue=None, readonly=True)
        self.create_attribute(ATTR_OUTPUT_FILENAME, default_vlaue=None, readonly=True)
