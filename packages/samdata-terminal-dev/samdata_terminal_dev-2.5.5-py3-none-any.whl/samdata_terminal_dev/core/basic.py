...
class SAMTraderEngine(object):
    def __init__(self,userid,strategyid,symbol_list,time_type,cash,
        ...
        if (self.mode == EModeType.BACKTEST):
            ...
            context.backtestid = self.__get_backtest_id()
            msg = 'start backtest, backtestid为: ' + context.backtestid
            LogHelper.info(msg)
        else:
    ...
    def place_order(self,  symbol, direction, order_time, quantity, open_or_close, order_type, fillprice=0, limit_price=0, order_id= None):
        ...
        if self.mode == EModeType.LIVE:
            ...
            try:
                ...
                LogHelper.error("创建订单时，出错", traceback.print_exc())
    ...
    def client_close(self,):
        ...
        try:
            ...
            LogHelper.error("client_close出现异常", traceback.format_exc())
    ...
    def __create_back_test(self):
        ...
        try:
            ...
            value = (context.backtestid, self.strategyid, now_time, now_time, now_time, self.starttime, self.endtime,
            ...
            value = (context.backtestid, data, now_time, now_time)
            MySqLHelper().insertone(sql, value)
        except Exception as ex:
            LogHelper.error("更新回测交易信息失败", traceback.format_exc())
    ...
    def __create_paper_trade(self):
        ...
        try:
            ...
            LogHelper.error("更新仿真交易信息失败", traceback.format_exc())
...
class SAMTrader(object):
    ...
    def start(self):
        ...
        try:
            ...
            LogHelper.error("开启回测服务时出错", traceback.format_exc())
        finally:
    ...
    def stop(self):
        ...
        if self.e.mode == EModeType.BACKTEST:
            ...
            if len(ibtradeengine.unknow_order_dict.keys()) != 0:
                for item in ibtradeengine.unknow_order_dict.keys():
                    order, is_update = ibtradeengine.get_order_status(item)

                    if is_update:
                        ibtradeengine.update_order(order)
            """
