...
class HolgerUserTokenAuthentication(BaseAuthentication):
    def authenticate(self, request):
        ...
        try:
            ...
            key = getattr(settings, 'Holger_JWT_KEY', settings.SECRET_KEY)
...
class HolgerTokenAuthentication(BaseAuthentication):
    def authenticate(self, request):
        User = get_user_model()
        user = AnonymousUser()
        try:
