...
def transitions(digr):
    ...
    the form `range(n)`. The output dictionary maps each edge label to a List of sets:
    the image of edge label `x` is a list indexed by the vertex set, where
    the `v`-entry is the set of end vertices of `x`-labeled edges out of `v`, or
...
def is_deterministic(digr):
    ...
    ``digr`` is expected to be a ``DiGraph`` with labeled edges and with vertex set of
    the form `range(n)`. It is said to be deterministic if for each vertex `v` and each
...
def transition_function(digr):
    ...
    ``digr`` is expected to be a deterministic ``DiGraph`` (a ``ValueError`` is raised otherwise), with edge labels
    positive integers. The output dictionary maps each edge labels to a list:
...
def DiGraph_to_list_of_PartialInjection(G):
    ...
    ``G`` is expected to be a folded ``DiGraph`` (a ``ValueError`` is raised otherwise) with edge labels in `[1..r]` and 
    vertex set `[0..n-1]` (`r` and `n` not given). Folded means that the graph
    is deterministic and co-deterministic or, equivalently, that every edge label
    defines a partial injection on the vertex set. The list returned has size `r`, and
    represents the partial injections (from the class ``PartialInjection``) induced
    by edge labels `1, 2, \dots, r`, respectively.
    ...
    if G.edges():
        ...
        r = 0
    n = len(G.vertices())
    pinj = [[] for _ in range(r)]
    for a in range(r):
        pinj[a] = [None for _ in range(n)]
    for (p,q,i) in G.edges():
        pinj[i - 1][p] = q
    pinj = [PartialInjection(x) for x in pinj]
    return pinj
...
def image_of_word(G,w,qinitial=0,trace=False):
    ...
    (``None``, ``length_read``, ``last_vertex_visited``) where ``length_read`` is the length of the longest
    prefix `u` of `w` which can be read in `G` starting at ``qinitial`` and ``last_vertex_visited``
    is the vertex reached after reading `u`.
...
def are_equal_as_rooted_unlabeled(G, H, certificate=False, verbose=False):
    ...
    # and the same stallings_graph_size. A common reason for not being identical subgroups
    # is that the partial injections cannot be translated to one another because they
...
def relabeling(G, R):
    ...
    ``G`` is expected to a ``DiGraph`` with vertices labeled `0,\dots,n-1`. ``R`` is expected to be a permutation
    of `0,\dots,n-1`. (No verification is made of that fact.) The output ``DiGraph`` is obtained from ``G`` by
    relabeling the vertices of ``G`` using the permutation ``R``. 
...
def exchange_labels(G,i,j):
    ...
    u = min(i,j)
    v = max(i,j)
    R = range(u) + [v] + range(u + 1,v) + [u] + range(v + 1,len(G.vertices()))
...
##################################################
### Finding a spanning tree, rooted at vertex 0
### along with the words labeling the paths in the
### tree to each vertex
##################################################
...
def basis_from_spanning_tree(G, T, D, root=0):
    r"""
    ...
    EXAMPLES::
        ...
        sage: from stallings_graphs.about_automata import bouquet, spanning_tree_and_paths, basis_from_spanning_tree
        sage: from stallings_graphs.about_folding import NT_fold
...
def express_Word_in_basis_from_spanning_tree(G, T, w, root=0, alphabet_type = 'abc'):
    r"""
    Return the valuea word the basis (of the space of loops of ``G``at the ``root`` vertex) specified by the spanning tree ``T``.

    ``G`` is expected to be a folded ``DiGraph`` with numerical edge labels. ``T`` (also a ``DiGraph``) is expected to be
    a spanning tree of ``G``. ``w`` is expected to be a Word labeling a loop at ``root`` in ``G``. The output is a 
    numerical Word which is the translation of ``w`` in the alphabet of the basis defined by ``T``.
    ...
    EXAMPLES::
        ...
        sage: from stallings_graphs.about_automata import bouquet, spanning_tree_and_paths, basis_from_spanning_tree, express_Word_in_basis_from_spanning_tree
        sage: from stallings_graphs.about_folding import NT_fold
    ...
    for a in u:
        if a > 0:
            ...
            raise ValueError('the 3rd argument is not a proper numerical Word (contains a non acceptable character')
        current_state = next_state
    if current_state != root:
        raise ValueError('the 3rd argument does not label a loop at the root')
    ...
    if len(G.vertices()) > 1:
        while list_of_leaves:
            ...
            if len(G.neighbors_in(v)) + len(G.neighbors_out(v)) == 1:
                ...
                G.delete_vertex(v)
                T.delete_vertex(v)
                if w != 0 and len(G.neighbors_in(w)) + len(G.neighbors_out(w)) == 1:
...
def cyclic_reduction(G,trace=False):
    ...
    ``G`` is expected to be ``DiGraph`` with numerical edge labels and vertex set of the form `[0..n]`.
