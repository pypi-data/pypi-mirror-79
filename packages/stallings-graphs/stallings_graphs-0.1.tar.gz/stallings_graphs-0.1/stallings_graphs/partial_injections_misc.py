...
def is_valid_partial_injection(L):
    ...
    either ``None`` or in `range(n)`, where `n` is the length of ``L``, and if none of the integer entries is repeated.
...
#    q = [p[i] for i in range(len(p))]
    q.sort()
    return all((i < n) and (0 <= i) for i in q if not(i == None)) and all((q[i] == None) or not (q[i] == q[i+1]) for i in range(n-1))
...
def number_of_partial_injections_list(n):
    ...
    ALGORITHM:
        ...
        The algorithm implements a recurrence relation described in [BNW2008] F. Bassino,
        C. Nicaud, P. Weil. Random generation of finitely generated subgroups of a free
        group, International Journal of Algebra and Computation 18 (2008) 1-31.
