...
class CosS3Client(object):
    ...
    def head_object(self, Bucket, Key, **kwargs):
        ...
        if 'versionid' in headers:
            ...
            del headers['versionid']
        url = self._conf.uri(bucket=Bucket, path=Key)
        logger.debug("head object, url=:{url} ,headers=:{headers}".format(
            url=url,
            headers=headers))
        rt = self.send_request(
            method='HEAD',
    ...
    def create_multipart_upload(self, Bucket, Key, **kwargs):
        ...
        """
        headers = mapped(kwargs)
        headers['Host'] = self._conf.get_host(Bucket)
        params = {'uploads': ''}
        params = format_values(params)
        url = self._conf.uri(bucket=Bucket, path=Key)
        logger.debug("create multipart upload, url=:{url} ,headers=:{headers}".format(
            url=url,
            headers=headers))
        rt = self.send_request(
                method='POST',
    ...
    def complete_multipart_upload(self, Bucket, Key, UploadId, MultipartUpload={}, **kwargs):
        ...
        params = format_values(params)
        url = self._conf.uri(bucket=Bucket, path=Key)
        logger.debug("complete multipart upload, url=:{url} ,headers=:{headers}".format(
            url=url,
            headers=headers))
        rt = self.send_request(
                method='POST',
