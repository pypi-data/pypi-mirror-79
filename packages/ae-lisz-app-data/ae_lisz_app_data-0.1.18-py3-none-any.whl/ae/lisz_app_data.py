...
class LiszDataMixin(ABC):
    ...
    def current_item_or_node_repr(self) -> str:
    ...
    def edit_validate(self, old_idx: int, new_id: Optional[str] = None, want_node: Optional[bool] = None) -> str:
        ...
        if new_id is not None:
            if not new_id:
                # on empty id string cancel edit if add_item, else request confirmation from user for item del
                return "" if add_item else 'request_delete_confirmation_for_item'
            if not check_item_id(new_id):
                return get_f_string(
                    "invalid item id {new_id} containing '{FLOW_PATH_TEXT_SEP}' or is equal '{FLOW_PATH_ROOT_ID}'")
