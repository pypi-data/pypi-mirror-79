...
class AliyunOss(object):
    ...
    def find_prefix_oss(self, ctx, oss_file_path, isrm=False):
        ...
        if not isrm:
            file_iter = oss2.ObjectIterator(bucket, prefix=oss_file_path, delimiter='/')
        else:
        ...
        for i in range(retry_count):  # 最多重试三次，由于网络不稳定等问题
            try:
                ...
                if i < retry_count-1:
                    ...
                    BMOBJ.clog(ctx, f"{local_file} download oss finally fail: {e}")
                    raise Et(2, f"download fail {td_file} {local_file}")

    def download_oss(self, ctx, local_file_path, oss_file_path, max_workers=200):
        """
        ...
        # 下载
        subfiles = self.find_prefix_oss(ctx, oss_file_path, isrm=False)
        if subfiles:
        ...
        for i in range(retry_count):  # 最多重试三次，由于网络不稳定等问题
            try:
                ...
                if i < retry_count-1:
                    ...
                    BMOBJ.clog(ctx, f"{local_file} upload oss finally fail: {e}")
                    raise Et(2, f"upload fail {td_file} {local_file}")

    def upload_oss(self, ctx, local_file_path, oss_file_path, add_success=False, add_empty=False, max_workers=200):
        """
