...
class GridHandler():
    ...
    @staticmethod
    def submit(config_input:str, n_jobs=1, site=None, time=-1, mode='grid'):
        config = GridHandler.preprocess_config(config_input)
        if mode == 'grid':
            GridHandler.submit_grid(config, n_jobs=n_jobs, site=site, time=time)
        elif mode == 'idds':
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            with helper.cd(project_path):
                ...
                for _ in range(n_jobs):
                    os.system(command)
    @staticmethod
    def submit_grid(config, n_jobs=1, site=None, time=-1):
        project_path = helper.get_project_path(config['project_name'])
        print('INFO: Submitting {} grid job(s)'.format(n_jobs))
        command = GridHandler.format_grid_command(config, site=site, time=time)
        with helper.cd(project_path):
            # submit jobs
            for _ in range(n_jobs):
                os.system(command)
    ...
    @staticmethod
    def format_grid_command(config, site=None, time=-1):
        options = {'forceStaged': '',
                   'useSandbox': '',
                   'noBuild': '',
                   'alrb': ''}
        ...
        options['exec'] = '"pip install --upgrade hpogrid && '+\
                          'hpogrid run {} --mode grid"'.format(proj_name)
