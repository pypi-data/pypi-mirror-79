...
class JobBuilder():
    ...
    def run(self) -> None:
        ...
        try:
            extracted_files = []
            if environment_settings.is_grid_job():
                datadir = environment_settings.get_datadir()
        ...
        metadata_path = os.path.join(hpogrid.get_workdir(), kGridSiteMetadataFileName)
        # save metadata
        with open(metadata_path,'w') as output:
            print('INFO: Saved job metadata at {}'.format(metadata_path))
            json.dump(metadata, output, cls=helper.NpEncoder)
        
        # save idds output
        if environment_settings.is_idds_job():
            idds_utils.save_idds_output_from_metadata(metadata)

        # reset environment
        environment_settings.reset()
    ...
    def from_input(cls, config_input: [Dict, str],
        ...
        environment_settings.setup(mode)
        ...
        if (not search_points) and environment_settings.is_idds_job():
