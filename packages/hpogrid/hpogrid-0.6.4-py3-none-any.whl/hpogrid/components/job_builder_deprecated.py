...
class JobBuilder():
    ...
    def run(self) -> None:
        if not helper.is_grid_job_running():
            helper.local_setup()
        ...
        model = JobBuilder.get_model(self.model_script, self.model_name, self.scripts_path)
    ...
    def from_project(cls, proj_name:str, idds_job=False):
        ...
        scripts_path = helper.get_scripts_path(proj_name)
        ...
        return JobBuilder.from_config(config, scripts_path=scripts_path)
    
    
    
    @classmethod
    def from_config(cls, config_input: [Dict, str], scripts_path=None):
        config = helper.load_configuration(config_input)
        
        job = JobBuilder()
        job.project_name = config['project_name']
        job.scripts_path = config['scripts_path'] if scripts_path is None else scripts_path
    ...
    def from_config_for_idds(cls, config:Dict, input_point:Dict):
        ...
        job.project_name = config['project_name']
        job.scripts_path = config['scripts_path']
...
if __name__ == '__main__':
    ...
    parser.add_argument('config_file', help='Name of configuration file in either '
                        'json or yaml format')    
    config_file = parser.parse_args().config_file
    job = JobBuilder.from_config(config_file)
    job.run()
