...
class RandomApply(object):
    ...
    def __init__(self, transform, p=0.5,random_params=None,*args,**kwargs):
        self.transform=transform
        self.p = p
        self.args=args
        self.kwargs=kwargs
        self.random_params=random_params or {}
    def __call__(self, img):
        if self.p < random.random():
            return img
        params={}
        for k,v in self.random_params.items():
            params[k]=v()
        img=self.transform(img,*self.args,**self.kwargs,**params)
        return img
