...
def assert_custom_model_common(model, model_json):
    ...
    assert (model.target_type == TARGET_TYPE.BINARY) == model_json["supportsBinaryClassification"]
    assert (model.target_type == TARGET_TYPE.REGRESSION) == model_json["supportsRegression"]
...
class TestCustomInferenceModel(object):
    ...
    def test_create(self, request, make_models_url, model_data):
        ...
        model = CustomInferenceModel.create(
            model_data["name"], TARGET_TYPE.REGRESSION, model_data["targetName"],
        )
        assert_inference_model(model, model_data)
