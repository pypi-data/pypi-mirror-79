...
def mocked_blueprints(mocked_training_model, mocked_environment):
    return {
        ...
        "data": [
            {
                "blueprintId": "5cf4d3f5f930e26daac18a1a",
            ...
            {
                "blueprintId": "5cf4d3f5f930e26daac18a2b",
...
def assert_blueprint(blueprint, blueprint_json):
    assert blueprint.id == blueprint_json["blueprintId"]

    cm = blueprint_json["customModel"]
    assert blueprint.custom_model["id"] == cm["id"]
    assert blueprint.custom_model["name"] == cm["name"]
    ...
    exec_env = blueprint_json["executionEnvironment"]
    assert blueprint.execution_environment["id"] == exec_env["id"]
    assert blueprint.execution_environment["name"] == exec_env["name"]

    exec_env_ver = blueprint_json["executionEnvironmentVersion"]
    assert blueprint.execution_environment_version["id"] == exec_env_ver["id"]
    assert blueprint.execution_environment_version["label"] == exec_env_ver["label"]
...
def test_create_blueprint(mocked_blueprint, make_blueprints_url, include_env):
    responses.add(
        responses.POST,
        make_blueprints_url(),
        status=200,
    ...
    kwargs = dict(
        custom_model_id=mocked_blueprint["customModel"]["id"],
        custom_model_version_id=mocked_blueprint["customModelVersion"]["id"],
    )
    if include_env:
        kwargs.update(
            dict(
                environment_id=mocked_blueprint["executionEnvironment"]["id"],
                environment_version_id=mocked_blueprint["executionEnvironmentVersion"]["id"],
            )
        )
...
def test_create_blueprint_use_latest_version(
    ...
    CustomTrainingBlueprint.create(
        mocked_blueprint["customModel"]["id"], mocked_blueprint["executionEnvironment"]["id"],
    )
...
@responses.activate
def test_get_blueprint(mocked_blueprint, make_blueprints_url):
    url = make_blueprints_url(mocked_blueprint["blueprintId"])
    mock_get_response(url, mocked_blueprint)

    blueprint = CustomTrainingBlueprint.get(mocked_blueprint["blueprintId"])
    assert_blueprint(blueprint, mocked_blueprint)
...
@responses.activate
def test_change_blueprint_trainingcode(
    mocked_blueprint, mocked_custom_model_version, make_blueprints_url
):
    envdir = tempfile.mkdtemp()
    codedir = os.path.abspath(tempfile.mkdtemp())
    bfile = os.path.join(codedir, "bfile")
    ...
    responses.add(
        responses.POST,
        "https://host_name.com/customModels/{}/versions/".format(
            mocked_blueprint["customModel"]["id"]
        ),
        body=json.dumps(mocked_custom_model_version),
        status=200,
        content_type="application/json",
    ...
    blueprint = CustomTrainingBlueprint.from_server_data(mocked_blueprint)
    blueprint.with_updated_code([bfile, cfile])
    assert (
        json.loads(responses.calls[-1].request.body)["customModelVersionId"]
        == mocked_custom_model_version["id"]
    )
...
def test_from_dropin(
    ...
    assert blueprint.id == mocked_blueprint["blueprintId"]
