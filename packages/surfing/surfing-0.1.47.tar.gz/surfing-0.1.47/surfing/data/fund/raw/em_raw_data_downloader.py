...
class EmRawDataDownloader:
    ...
    def em_fund_nav(self, start_date, end_date, fund_id_list: Tuple[str] = ()):
        try:
            ...
            if trade_date_df is None:
                print('[em_fund_nav] failed to get em trade dates for em_fund_nav')
                return False
            if not fund_id_list:
                fund_id_list = self._em_sector_wrapper('507013', end_date)
                if fund_id_list is None:
                    print(f'[em_fund_nav] failed to get fund id list')
                    return False
            df_list: List[pd.DataFrame] = []
            for date in trade_date_df.TRADEDATES:
                # 基金 万份基金单位收益 7日年化收益率
                data1 = self._em_css_wrapper(fund_id_list, "10KUNITYIELD,YIELDOF7DAYS", f"EndDate={date},Ispandas=1")
                if data1 is None:
                    print('[em_fund_nav] failed to get fund nav data1')
                    return False
                # 基金 单位净值(原始披露) 累计单位净值(原始披露) 复权单位净值
                data2 = self._em_css_wrapper(fund_id_list, "ORIGINALUNIT,ORIGINALNAVACCUM,NAVADJ", f"TradeDate={date},Ispandas=1")
                if data2 is None:
                    print('[em_fund_nav] failed to get fund nava data2')
                    return False
    ...
    def em_fund_status(self, end_date: str):
        try:
            ...
            df = df.reset_index()
            df['DATES'] = end_date
            df = df.astype({'FUNDSCALE': 'float64', 'LPLIMIT': 'float64', 'OTCLPLIMITJG': 'float64', 'OTCLPLIMITGR': 'float64'})
            # 获取当前的fund status
            ...
            if now_df is not None:
                ...
                df = df.merge(now_df.drop(columns=['_update_time']), how='left', on=['CODES', 'FUNDSCALE', 'LPLIMIT', 'OTCLPLIMITJG', 'OTCLPLIMITGR', 'PURCHSTATUS', 'REDEMSTATUS', 'TRADESTATUS'], indicator=True, validate='one_to_many')
    ...
    def em_stock_info(self, end_date: str):
        try:
            ...
            new_stock_code = list(set(stock_list).difference(set(stock_info_df.stock_id.to_list())))
    ...
    def em_stock_fin_fac(self, end_date: str):
        ...
        df_list: List[pd.DataFrame] = []
        # 获取无参数指标
        df_no_param = self._em_sector_wrapper(stock_list,
        ...
        if df_no_param is None:
            print('failed to get stock financial factors')
        else:
            df_list.append(df_no_param)

        # 获取主营收入构成(按产品)
        df_mb_sales_cons = self._em_sector_wrapper(stock_list, "MBSALESCONS", f'ReportDate={end_date},type=2,Ispandas=1')
        ...
        df_with_param_1 = self._em_sector_wrapper(stock_list, "DEDUCTEDINCOME", f'ReportDate={end_date},DataAdjustType=1,Ispandas=1')
        ...
        df_with_param_2 = self._em_sector_wrapper(stock_list, "DEDUCTEDINCOME", f'ReportDate={end_date},DataAdjustType=2,Ispandas=1')
        ...
        # 获取TTM指标
        df_ttm = self._em_sector_wrapper(stock_list, 'GRTTMR,GCTTMR,ORTTMR,OCTTMR,EXPENSETTMR,GROSSMARGINTTMR,OPERATEEXPENSETTMR,ADMINEXPENSETTMR,FINAEXPENSETTMR,IMPAIRMENTTTMR,\
        ...
        try:
            ...
            for row in index_info[~index_info['em_plate_id'].isna()].loc[:, ['index_id', 'em_id', 'em_plate_id']].itertuples(index=False):
                for date in date_list:
                    stock_list = self._em_sector_wrapper(row.em_plate_id, end_date)
                    if stock_list is None:
    ...
    def em_fund_info(self, fund_id_list: Set[str]):
        if not fund_id_list:
            return True
        try:
            data = self._em_css_wrapper(','.join(fund_id_list), 'NAME,FNAME,FUNDTYPE,FOUNDDATE,MATURITYDATE,FIRSTINVESTTYPE,SECONDINVESTTYPE,TRADECUR,STRUCFUNDORNOT,GFCODEM,PREDFUNDMANAGER,MGRCOMP,CUSTODIANBANK,FOREIGNCUSTODIAN', 'Ispandas=1')
            if data is None:
    ...
    def em_fund_rate(self, end_date: str):
        try:
            fund_list = self._em_sector_wrapper('507013', end_date)
            if fund_list is None:
    ...
    def em_fund_asset(self, end_date: str):
        ...
        if fund_list_df is None:
            print(f'failed to get fund list for fund status')
            return False
        fund_list: List[str] = fund_list_df.all_live_fund_list.array[0].split(',')
        fund_list += fund_list_df.delisted_fund_list.array[0].split(',')
        ...
        for year in range(start_year, end_year+1):
            for date in self._quarter_update_dates:
                ...
                for rank in rank_list:
                    data = c.css(fund_list, "PRTKEYINDNAME,PRTKEYINDTOASSET", f"ReportDate={dt},Rank={rank},IndustryCode=A,NewOldType=1,Ispandas=1")
    ...
    def em_macroeconomic_daily(self):
        try:
            ...
            if now_df is not None:
                ...
                df = df.merge(now_df.drop(columns='_update_time'), how='left', on=['codes', 'datetime', 'value'], indicator=True, validate='one_to_one')
...
if __name__ == '__main__':
    ...
    # em.em_fund_benchmark_history('20160101', '20161231')
    ...
    # em.em_macroeconomic_monthly_history('20200914')
