    ...
    def init(self, index_list=None, score_pre_calc=True, print_time=False, use_weekly_monthly_indicators=False, is_normal_type=False, is_mix_type=False):
        if self._s3_retriever.is_activated:
            ...
            if self.try_to_init_from_s3([self.start_date == std_dm.start_date,
                                        ...
                                        is_normal_type is False,
                                        is_mix_type is False], score_pre_calc, use_weekly_monthly_indicators):
                # 从s3成功读取数据，直接返回
                return
        ...
        def fetch_table(session, view):
            ...
            return self._s3_retriever.fetch_table(query, view.__table__.name)
        ...
        def _fund_size_and_hold_rate_filter(query):
            def _fund_size_and_hold_rate_filter_etl(x):
                return x[x.fund_id.isin(self.dts.fund_list) & (x.datetime >= self.start_date - datetime.timedelta(days=200)) & (x.datetime <= self.end_date)].reset_index(drop=True)
        ...
        def _fund_indicator_filter(query):
        ...
        with BasicDatabaseConnector().managed_session() as quant_session:
            ...
            index_price_raw = self._s3_retriever.fetch_table(_index_query, IndexPrice.__table__.name, _index_price_filter_func)
            self.dts.index_price = index_price_raw.pivot_table(index='datetime', columns='index_id', values='close').reindex(trim_trading_days).fillna(method='ffill')
            ...
            _fund_nav_query, _fund_nav_filter_func = _fund_nav_filter(_fund_nav_query)
            _nav_df = self._s3_retriever.fetch_table(_fund_nav_query, FundNav.__table__.name, _fund_nav_filter_func)
            ...
            _fund_size_and_hold_rate_query, _fund_size_and_hold_rate_filter_func = _fund_size_and_hold_rate_filter(_fund_size_and_hold_rate_query)
            _size_and_hold_rate = self._s3_retriever.fetch_table(_fund_size_and_hold_rate_query, Fund_size_and_hold_rate.__table__.name, _fund_size_and_hold_rate_filter_func)
        ...
        with DerivedDatabaseConnector().managed_session() as derived_session:
            ...
            _fund_indicator_query, _fund_indicator_filter_func = _fund_indicator_filter(_fund_indicator_query)
            self.dts.fund_indicator = self._s3_retriever.fetch_table(_fund_indicator_query, FundIndicator.__table__.name, _fund_indicator_filter_func)
            ...
            self.dts.fund_indicator_annual = self._s3_retriever.fetch_table(_fund_indicator_query, FundIndicatorAnnual.__table__.name, _fund_indicator_filter_func)
            ...
            _index_pct_df = self._s3_retriever.fetch_table(_index_pct_query, IndexValuationLongTerm.__table__.name, _index_pct_filter_func).set_index('index_id')
            # 按照trading date 作为index, fillna
            date_list = self.dts.trading_days.datetime.tolist()
            res = []
            for index_id in pct_index_list:
                df = _index_pct_df[_index_pct_df.index == index_id].set_index('datetime').reindex(date_list).reset_index().fillna(method='ffill')
            ...
            if use_weekly_monthly_indicators:
                ...
                _fund_indicator_weekly = self._s3_retriever.fetch_table(_fund_indicator_weekly_query, FundIndicatorWeekly.__table__.name, _fund_indicator_weekly_filter_func)
