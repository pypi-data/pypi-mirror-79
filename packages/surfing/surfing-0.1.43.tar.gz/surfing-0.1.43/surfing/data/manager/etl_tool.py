...
from ..view.basic_models import FundInfo, TradingDayList, IndexInfo, IndexPrice, FundNav, Fund_size_and_hold_rate
from ..view.derived_models import FundIndicator, FundIndicatorAnnual, IndexValuationLongTerm, FundIndicatorWeekly, FundIndicatorMonthly, BarraCNE5FactorReturn
...
class SynchronizerS3ForDM(S3ETLTool):
    ...
    def _dump_misc(self) -> List[str]:
        ...
        # score manager部分
        for name in ('score_cache', 'score_raw_cache'):
            _t0 = time.time()
            try:
                pd.Series(self._score_manager.__dict__[name]).to_frame(name).to_parquet(f'{self._s3_bucket_uri}/_ndf_{name}.parquet', compression='gzip')
            except Exception as e:
                print(f'WARNING!! dump data {name} to s3 failed (err_msg){e}')
                failed_tables.append(name)
            _t1 = time.time()
...
class S3RetrieverForDM(S3ETLTool):
    ...
    def __init__(self, activation: bool = False):
        super().__init__()
        # 默认先不激活
    ...
    def fetch_table(self, query, table_name: str, post_processor=None):
        # 如果没有开启的话，直接从mysql读取
        ...
        try:
            ...
            print(f'WARNING!! fetch table {table_name} from s3 failed (err_msg){e}, try mysql')
            # TODO: 如果这里DB也失败，应该也需要适当的处理
            return pd.read_sql(query.statement, query.session.bind).drop(columns='_update_time', errors='ignore')

    def _retrieve_dts(self, data_tables: FundDataTables) -> List[str]:
        failed_tables: List[str] = []
        for field in dataclasses.fields(data_tables):
            ...
            if field.name == 'index_fund_indicator_pack':
                continue
            name = field.name
    ...
    def _retrieve_all_dts(self, data_tables: FundDataTables) -> List[str]:
        _t0 = time.time()
        failed_tables: List[str] = self._retrieve_dts(data_tables)
        _t1 = time.time()
        print(f'all dts total cost {_t1 - _t0}s (failed_tables){failed_tables}')
        return failed_tables

    def _retrieve_misc(self, dm) -> List[str]:
        failed_tables: List[str] = []
        for field in dataclasses.fields(dm.dts):
            ...
            try:
                ...
                if name in ('fund_list', 'index_list', 'index_date_list'):
                    data = set(df.to_dict(orient='list')[name])
                    if name == 'index_date_list':
                        data = sorted(data)
                elif name in ('fund_index_map'):
    ...
    def _retrieve_all_misc(self, dm) -> List[str]:
        _t0 = time.time()
        failed_tables: List[str] = self._retrieve_misc(dm)
        _t1 = time.time()
    ...
    def retrieve_fund_score(self, score_manager: FundScoreManager):
        ...
        for name in ('score_cache', 'score_raw_cache'):
            _t0 = time.time()
            try:
                df = pd.read_parquet(f'{self._s3_bucket_uri}/_ndf_{name}.parquet').to_dict(into=defaultdict(dict))[name]
                ...
                for v in df.values():
                    ...
                    for lk, lv in v.items():
                        if lv is None:
                            ...
                            to_delete_inner = []
                            for mk, mv in lv.items():
                                if mv is None:
                                    to_delete_inner.append(mk)
                            for one in to_delete_inner:
                                del lv[one]
                    for one in to_delete:
                        del v[one]
                score_manager.__dict__[name] = df
