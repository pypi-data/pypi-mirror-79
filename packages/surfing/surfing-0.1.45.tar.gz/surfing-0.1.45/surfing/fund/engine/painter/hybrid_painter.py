...
class HybridPainter(object):
    ...
    def plot_asset_price_ratio(fund_nav:pd.DataFrame, 
        ...
        for idx, begin_date in enumerate(rebalance_date[:-1]):
            ...
            for asset, num in proper_number_dict.items():
                ...
                _fund_list = fund_trade[(fund_trade['trade_date'] == begin_date) & (fund_trade['is_buy'])].fund_id.tolist()
                dic = {
                    'index_id':asset,
    ...
    @staticmethod
    def fund_rank_recent_analysis(fund_id,index_id,score_dict,fund_info,fund_indicator,fund_nav,index_price,score_func):
        ...
        for dt in dts_list:
            ...
            if index_id in _score_dict:
                _score_dict = _score_dict[index_id]
            else:
                continue
            _score_list = sorted(_score_dict.items(), key=lambda x:x[1], reverse=True)
            _fund_list = [i[0] for i in _score_list]
