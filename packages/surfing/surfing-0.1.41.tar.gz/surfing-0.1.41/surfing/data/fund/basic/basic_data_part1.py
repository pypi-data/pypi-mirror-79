...
class BasicDataPart1:
    ...
    def fund_size_hold_rate_history(self):
        ...
        fund_scale = fund_scale.pivot(index='DATES', columns='CODES', values='FUNDSCALE').fillna(method='ffill').iloc[[-1], :].stack()
        ...
        fund_scale['datetime'] = pd.to_datetime(start_date).date()
        fund_list = list(set(fund_scale.fund_id))
        res = []
        for fund_id in fund_list:
            fund_hold_i = fund_holding_rate[fund_holding_rate.fund_id == fund_id].drop(columns='fund_id').set_index('datetime')
            fund_scale_i = fund_scale[fund_scale.fund_id == fund_id].drop(columns='fund_id').set_index('datetime')
