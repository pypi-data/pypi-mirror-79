...
# from surfing.data.view.derived_models import FundScoreExtended
# from surfing.data.fund.derived.derived_data_helper import DerivedDataHelper, normalize, score_rescale, cont_rescale

from ...api.basic import BasicDataApi
from ...api.derived import DerivedDataApi
from ...view.derived_models import FundScoreNew
from .derived_data_helper import DerivedDataHelper, normalize, score_rescale, cont_rescale
...
class FundScoreGroupProcess:
    ...
    def init(self, start_date: str, end_date: str):
        ...
        self._fund_indicator_group: pd.DataFrame = DerivedDataApi().get_fund_indicator_group(start_date, end_date).drop(columns='_update_time').set_index('fund_id')
        # 过滤掉scale为0的fund
        self._fund_indicator_group = self._fund_indicator_group[self._fund_indicator_group.scale > 0]
    ...
    def _calc_score(self, x: pd.DataFrame, by_key: str) -> pd.DataFrame:
        ...
        # 第四个和第五个维度计算
        if wind_class in ('货币市场型基金', '被动指数型基金', '被动指数型债券基金'):
    ...
    def process(self, start_date: str, end_date: str, end_date_dt: datetime.date) -> List[str]:
        ...
        try:
            ...
            print(e)
            traceback.print_exc()
            failed_tasks.append('fund_score_group')
        return failed_tasks


if __name__ == '__main__':
    date = '20200824'
    fsg = FundScoreGroupProcess(DerivedDataHelper())
