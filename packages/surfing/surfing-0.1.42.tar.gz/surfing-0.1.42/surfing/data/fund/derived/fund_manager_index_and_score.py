...
class FundManagerIndexAndScore:
    ...
    @staticmethod
    def _filtered_by_class(info: pd.DataFrame, fund_kind: str) -> pd.DataFrame:
        if fund_kind == 'QDII':
    ...
    def calc(self, eval_date: str, eval_start_date: str):
        ...
        for class_name, index_id in self._REPLACE_DICT.items():
            ...
            fund = FundManagerIndexAndScore._filtered_by_class(self._info, class_name)['wind_id']
        ...
        for class_name, index_id in self._REPLACE_DICT.items():
            ...
            for i in cols:
                ...
                benchmark_stat_result.append(dataclasses.asdict(Calculator.get_benchmark_stat_result(df.index.array, df.array, bm.loc[df.index].array)))
