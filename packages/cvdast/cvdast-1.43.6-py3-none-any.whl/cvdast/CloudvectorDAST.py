...
fuzz_words_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'fuzzdb/attack')
fuzz_types = ["fuzzdb/attack"]#os.listdir(os.path.join(root, "fuzzdb/attack"))#["general", "injections", "vulns", "webservices", "stress", "others"]
...
class CloudvectorDAST(object):
    ...
    def __init__(self, APISpecOne, APISpecTwo, ce_host, ce_username, ce_password, config_file, cover_only_diff="n",
        ...
        if not ce_host and ".txt" in self.apispec_two_path:
    ...
    def _get_fuzz_details(self, fuzz_type):
        fuzz_values = []
        fuzz_values.append({"path": fuzz_type,
                            "fuzz_type": fuzz_type})
        return fuzz_values
    ...
    def generate_fuzz_report():
        with open(os.path.join(templates_dir, 'data/attack_info.json')) as file_:
        ...
        def get_metadata_for_fuzzinput(in_file):
            for type, subtype in attack_info.items():
                for name, info in subtype.items():
                    if name == "file_set":
                        if str(in_file).lower() in [str(_).lower() for _ in info]:
                            return subtype
            return None
        ...
        for test in report["report"]["tests"]:
            ...
            try:
                ...
                response_code = 0
            try:
                fuzz_sub_type = metadata.split("::")[3].split("/")[-1]
            except:
                fuzz_sub_type = "na"
            try:
                fuzz_type = metadata.split("::")[3].replace(fuzz_sub_type,"").replace("attack","").replace("/","")
                print(api_name, fuzz_type, fuzz_sub_type)
                #print(metadata)
            except:
                fuzz_type = "na"
            try:
        ...
        for k, v in fuzz_data.items():
            ...
            if v["apis_failed"] > 0:
                progress = 0
                fuzz_data_processed.insert(0, {"progress": progress, "fuzz_type": k.replace("_fuzz", ""),
                ...
                fuzz_data_processed.append(
                    {"progress": progress, "fuzz_type": k.replace("_fuzz", ""), "total_apis": v["apis_tested"],
        ...
        with open("cviast_report.html", "w+") as file_:
            file_.write(template.render(api_data=api_data, fuzz_data=fuzz_data_processed, total_apis=total_apis,
                                        total_vulnerabilities=vulnerabilities_found,
                                        test_time=datetime.datetime.fromtimestamp(time.time()).isoformat(),
                                        fuzz_types=fuzz_types, vulns_stats=vulns_stats,
                                        test_lapse_time=str(round(int(report["report"]["summary"]["duration"])/60))+" min(s)"))
        print("CVIAST report saved in " + str(os.getcwd()) + "/cviast_report.html")
...
def main():
    ...
    if sys.argv[1:]:
        if sys.argv[1:][0] == "--generate-report":
            CloudvectorDAST.generate_fuzz_report()
            return
