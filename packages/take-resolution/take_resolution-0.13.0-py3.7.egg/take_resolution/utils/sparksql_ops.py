...
def replace_missing_values(df: SDF, column: str, replacement: tp.Any) -> SDF:
    """Replace missing values in dataframe column by informed value.
    
    :param df: Pyspark dataframe to be replaced.
    :type df: ``pyspark.sql.DataFrame``
    :param column: Column to replace missing values.
    :type column: ``str``
    :param replacement: Value to replace missing values.
    :type replacement: ``any``
    :return: Dataframe with missing values replaced.
    :rtype: ``pyspark.sql.DataFrame``
    """
    return df.withColumn(column, f.coalesce(f.col(column), f.lit(replacement)))
