...
def get_ddl_path_default(schema):
    try_path = [
        os.getcwd().replace("/src", "/") + "db/vertica/{}".format(schema),
        "src/db/vertica/{}".format(schema),
        "db/vertica/{}".format(schema),
    ]
    for path in try_path:
        if not os.path.exists(path):
            logging.info(
                "Path {} not found, you have to create directory src/db/schema to upload file".format(
                    path
                )
            )
        else:
...
class TableLoader(VerticaConnector):
    ...
    def script_to_table(self, script):
        ...
        self.__exec_sqls([script_insert])
        self.reload_table()
