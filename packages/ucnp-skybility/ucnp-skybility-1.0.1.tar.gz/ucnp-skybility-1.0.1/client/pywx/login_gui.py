...
class LoginGui(LogUI):
    def __init__(self, pipe_data, connection, *args, **kwargs):
        ...
        conf_dir = os.path.join(user_home, 'ucnp_conf')
        if not os.path.exists(conf_dir):
            os.makedirs(conf_dir, exist_ok=True)

        self._connect_string_file = os.path.join(conf_dir,
                                                 'connect_string.txt')
        self._enable_save_pw_file = os.path.join(conf_dir,
...
class TaskBarIcon(wx.adv.TaskBarIcon):
    ...
    def show_log(self, event):
        content = ''
        file_path = os.path.join(user_home, 'ucnp_client', 'log', 'up_utils.log')
