...
def _iter_collection(path):
    ...
    with path.open("rt") as collection_stream:
        for did, text in logger.pbar(plaintext.read_tsv(collection_stream), desc='documents'):
...
class MsmarcoDataset(Dataset):
    ...
    SUBSETS=['train', 'train10', 'train_med', 'dev', 'minidev', 'judgeddev', 'eval', 'trec2019', 'judgedtrec2019']
    ...
    def _load_topics(self):
        return dict(self.logger.pbar(plaintext.read_tsv(self.assessed_topics.topics.path), desc='loading queries'))
    ...
    def prepare():
        ...
        def get(subset: str):
            ...
            return MsmarcoDataset(docstore=docstore, index=index, index_stem=index_stem, assessed_topics=assessed_topics)
    ...
    #     # TODO: A subset of dev that only contains queries that have relevance judgments
    ...
    #         med_qids = util.Lazy(lambda: {qid.strip() for qid in util.download_stream('', 'utf8', expected_md5="dc5199de7d4a872c361f89f08b1163ef")})
    ...
    # def _init_doctttttquery_iter(self): # TODO: add doctttt predictions for msmarco
