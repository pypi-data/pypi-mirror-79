...
def show(problem, field, variable, component, time_step=0, adaptive_step=0, width=600, height=400):
    temp_geometry_name = "{}.vtk".format(next(tempfile._get_candidate_names()))
    problem.geometry().export_vtk(temp_geometry_name)
    
    geometry = vtk_tools.vtk_geometry_actor(temp_geometry_name)    
    ...
    scalar = vtk_tools.vtk_scalar_actor(temp_post_name)
    contours = vtk_tools.vtk_contours_actor(temp_post_name)    
    
    fig = vtk_tools.vtk_figure(geometry=geometry, scalar=scalar, contours=contours, width=width, height=height)
