...
def start(*logs):
    def inner(func):
        def wrapper(*args, **kwargs):
            ...
            # print('Logs : ', logs[0].keys())
            # print('Args : ', args[2].keys())
...
def trace(func):
    def wrapper(*args, **kwargs):
        ...
        if not db.search('console', None) == b'not present':
            for key in db.search('console', None):
...
def latency(func):
    def wrapper(*args, **kwargs):
        ...
        if not start_time == b'not present' and not end_time == b'not present':
            db.delete(args[0], None)
