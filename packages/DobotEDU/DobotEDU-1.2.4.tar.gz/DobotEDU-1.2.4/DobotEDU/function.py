...
class Util(object):
    def get_image(self,
                  ...
                  timeout,
                  port,
                  flip=False):  # timeout为拍照时间,port相机端口，flip是否水平翻转标志
...
class Face(BaseAI):
    ...
    def detect(self, image, face_attributes=0, quality_detection=0):
        payload = {
            "data":
    ...
    def search(self, group_ids, image):
        payload = {
            "data":
    ...
    def create_library(self, group_name, group_id):
        payload = {
            "data":
