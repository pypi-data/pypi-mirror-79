...
import redis
import requests
from BaseColor.base_colors import blue, hcyan, green
...
def start_up_check(dont_ask=True):
    if os.path.exists("/etc/rc.d/init.d/nettrarec"):
        return
    local_path = os.path.split(os.path.abspath(__file__))[0]
    gen_server_script(local_path)
    os.popen(
        f'sudo cp {os.path.join(local_path, "nettrarec")} /etc/rc.d/init.d/ && sudo chmod +x /etc/rc.d/init.d/nettrarec'
    ).read()
    ...
    if do == 'y':
        print("adding nettrarec to system service")
        os.popen(
            'chkconfig --add /etc/rc.d/init.d/nettrarec && chkconfig /etc/rc.d/init.d/nettrarec on && systemctl start /etc/rc.d/init.d/nettrarec'
        ).read()
        print(green("\nnettrarec has add to system service and set to start automatically at system boot\n"))
        ...
        print(blue("2. Open target Redis to check if the data is correct\n"))
...
def _get_current_ip4(last_ip_info):
    ip_info = last_ip_info
    try:
        res = requests.get("http://meesee.top/ip").text
        ip_info['ip'] = re.findall(r"your ip: +([\d.]+)\n", res)[0]
        ip_info['city'] = ' '
        ip_info['country'] = ' '
    except Exception as E:
        print(f"Error in getting ip info: \n{E}")
