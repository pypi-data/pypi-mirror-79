...
def record_starter_server():
    ...
    try:
        ...
        args_str = f"-u {unit} -n {n_devices} -rf {refresh_rate} -pr {push_to_redis} -kp {key_params_mode}"
        run = f"python {os.path.join(os.path.abspath(os.path.dirname(__file__)), file_name)} {args_str}"
