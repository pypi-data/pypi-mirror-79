...
from NetRecorder.common_scheduler import ComThread
from NetRecorder.gear_for_nr import tell_the_datetime, convert_bytes, start_up_check, \
    _get_host_name, get_redis_cli, default_redis_params, get_redis_list_last, _get_current_ip4
...
def waiting(rw, head_str=None):
    while rw >= 0:
        m, s = divmod(rw, 60)
        h, m = divmod(m, 60)
        print(f'{head_str}: [ {red(f"{h}:{m}:{s}")} ]\r', end='') if head_str else print(f'[ {red(f"{h}:{m}:{s}")} ]\r',
                                                                                         end='')
        rw -= 1
        time.sleep(1)
    # print()
...
def start(ip_keys, return_only=True, unit="auto", refresh_rate="s", push_redis=False, target_redis_params=None, no_print_out=True):
    unit = unit.lower()
    if not return_only:
        ...
        while 1:
            try:
                ...
                if push_redis:
                    ...
                    ip_info = _get_current_ip4(ip_info)
                    ...
                    insert_obj = json.dumps({i_key: {"in": ne_in.get(i_key),
                                                     ...
                                                     "location": location_info.get(
                                                         'location') or f"{ip_info.get('city')} - {ip_info.get('country')}",
                    ...
                    for trp in target_redis_params:
                        ...
                        try:
                            ...
                            print(f"{red('Error in push data to server redis')}: \n{E}")
                            return
                if not no_print_out:
                    s_out = "; ".join(
...
def push_to_server(trp, insert_obj):
...
def record_starter():
    ...
    parser.add_argument("-ps", "--place_script", dest="place_script", action="store_true",
                        help=f'{da}y/n 做初始化检查，检查服务器脚本并复制\n')
    parser.add_argument("-pso", "--place_script_only", dest="place_script_only", action="store_true",
                        help=f'{da}y/n 只做初始化检查，检查服务器脚本并复制\n')
    parser.add_argument("-kp", "--key_params", type=str, dest="key_params", default='input',
                        help=f'{da}关键参数提供方式，input/local/now，\n'
                             ...
                             'now方式仅限测试\n'
                             f'默认input\n')
    parser.add_argument("-no", "--no_print_out", dest="no_print_out", action="store_true",
                        help=f'{da} 控制是否输出到终端，以服务方式启动时默认不输出\n')
    args = parser.parse_args()
    if args.place_script_only:
        return start_up_check(dont_ask=True)
    ...
    key_params = {}
    if push_to_redis:
        start_up_check(args.place_script)
    ...
    start(n_devices, return_only=True, unit=unit, refresh_rate=refresh_rate, push_redis=push_to_redis,
          target_redis_params=key_params, no_print_out=args.no_print_out)
