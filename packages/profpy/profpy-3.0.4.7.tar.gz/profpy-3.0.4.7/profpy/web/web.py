    ...
    def secured(self, any_roles=None, not_roles=None, all_roles=None, get_cas_user=False):
        ...
        def _secured(f):
            ...
            def wrap(*args, **kwargs):
                ...
                current_object = session.get("cas-object")
                if current_object is None:
                    after_login = f"{request.path}{_parse_query_string(quoted=False)}"
                    # if after_login.startswith("http://"):
                    #     after_login = after_login.replace("http://", "https://")

                    session["cas-after-login"] = after_login
                    response = _login(self.__cas_server_url)
                else:
                    cas = CasUser(current_object["user"], current_object["attributes"], db_session=self.db, user_table=self.users)
...
def _login(in_cas_url, db_session=None, security_user_table=None):
    ...
    cas_ticket = session.get("cas-ticket")
