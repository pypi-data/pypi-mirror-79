...
def sync_decode_indicators(pbindicators):
    return _decode_indicators(pbindicators)
...
async def decode_travel_times(pbtraveltimes):
    loop = asyncio.get_event_loop()
    with ProcessPoolExecutor() as executor:
        traveltimes = await loop.run_in_executor(executor, _decode_travel_times, pbtraveltimes.traveltimes)
    return traveltimes


def sync_decode_travel_times(pbtraveltimes):
    return _decode_travel_times(pbtraveltimes.traveltimes)


def _decode_travel_times(traveltimes):
