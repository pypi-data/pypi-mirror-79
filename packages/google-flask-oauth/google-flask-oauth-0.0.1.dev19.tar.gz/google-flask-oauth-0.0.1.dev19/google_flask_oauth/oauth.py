...
def oauth_exchange():
  ...
  current_app.logger.info(session.get('https://www.googleapis.com/userinfo/v2/me').json())
  ...
  return jsonify({
    'token': urlsafe_b64encode(json.dumps(assembled_credentials).encode()).decode()
  }), 200
