...
class SimfluxProcessor:
    ...
    def drift_correct(self, framesperbin=1, drift_fn=None, cuda=True, initializeWithRCC=False):
        if drift_fn is None:
            from photonpy.smlm.drift_estimate import minEntropy
            
            names = ['SMLM', 'SIMFLUX']
            self.g_drift,self.sf_drift = [
                minEntropy(ds.get_xyI()[:,:2], 
                  ds.get_frame_numbers(),
                  ds.get_crlb(), framesperbin=framesperbin,
                  pixelsize=self.pixelsize, imgshape=self.imgshape,
                  sigmaPrecise=ds.get_crlb().mean(0)[0],
                  dataname= names[i],
                  outputfn= self.resultprefix+names[i]+"-drift.png",
                  debugMode=self.debugMode,
                  useCuda=cuda,
                  initializeWithRCC=initializeWithRCC)[0]
