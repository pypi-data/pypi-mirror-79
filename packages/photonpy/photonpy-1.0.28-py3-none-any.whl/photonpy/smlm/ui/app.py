...
class Window(QDialog):
    ...
    def onDriftCorrectRCC(self):
        fpb = self.ui.driftbins.value()
        ds = self.datasets[-1].copy()
    ...
    def onDriftCorrectMinEntropy(self):
        fpb = self.ui.driftbins.value()
        ds = self.datasets[-1].copy()
        drift, prec = ds.estimateDriftMinEntropy(framesPerBin=fpb, maxdrift=5)
        ds.applyDrift(drift)
        path = os.path.splitext( ds['imagefile'])[0]+"_undrifted_mine.hdf5"
        ds.save(path)
        self.datasets.append(ds)
        self.updateList()
    ...
    def updateList(self):
        ...
        for d in self.datasets:
            item = QtGui.QStandardItem(f"{d['imagefile']} - {d.info()}")   
    ...
    def getConfig(self):
        ...
        cfg = {
                ...
                'maxchisq': self.ui.chiSquareThreshold.value(),
    ...
    def localize(self):
        ...
        def localize_thread():
            ...
            try:
                self.result = process_movie.localize(tiff_path, cfg, locs_fn, progress_update, est_sigma)
