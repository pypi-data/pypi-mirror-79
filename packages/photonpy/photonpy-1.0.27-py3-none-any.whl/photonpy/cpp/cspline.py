...
class CSpline_Calibration(ctypes.Structure):
    ...
    def from_file_nmeth(cls, filename):
        ...
        print(f"Voxels X:{coefs.shape[2]}, Y:{coefs.shape[1]}, Z:{coefs.shape[0]}")
        return cls(z_min, z_max, coefs)
...
class CSpline:
    ...
    def CreatePSF_XYZIBg(self, roisize, calib: CSpline_Calibration, cuda) -> CSplinePSF:
        inst = self._CSpline_CreatePSF(roisize, calib, None, cuda, self.ctx.inst)
    ...
    def CreatePSFFromFile(self, roisize, filename, cuda=True) -> CSplinePSF:
        
        if filename.endswith(".mat"):
            calib = CSpline_Calibration.from_file_nmeth(filename)
            return self.CreatePSF_XYZIBg(roisize, calib, cuda)

        elif filename.endswith(".pickle"):
            with open(filename, "rb") as f:
                import pickle
                d = pickle.load(f)
            return self.CreatePSFFromZStack(roisize, d['zstack'], d['zrange'], cuda)
            ...
            return self.CreatePSFFromZStack(roisize, zstack, zrange, cuda)
        else:
            raise RuntimeError(f"Unknown psf extension: {filename}")

    def CreatePSFFromZStack(self, roisize, zstack, zrange, cuda, border=0) -> CSplinePSF:
        spline_coeff = self.ComputeSplineCoeff(zstack, border)
        calib = CSpline_Calibration(zrange[0], zrange[-1], spline_coeff)
        calib.zstack = zstack
        return self.CreatePSF_XYZIBg(roisize, calib, cuda)
