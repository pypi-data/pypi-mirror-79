                ...
                if key == "range":
                    for field in filter_type[key]:
                        ...
                        if data_type == DataTypes.STRING:
                            print("field type error, range filter type should't be string")
                            return

                        name = StringToByteArray(field)
                        
                        if "lte" in filter_type[key][field]:
                            upper = filter_type[key][field]["lte"]
                            include_upper = 1
                        else:
                            upper = filter_type[key][field]["lt"]
                            include_upper = 0

                        if "gte" in filter_type[key][field]:
                            lower = filter_type[key][field]["gte"]
                            include_lower = 1
                        else:
                            lower = filter_type[key][field]["gt"]
                            include_lower = 0                        
