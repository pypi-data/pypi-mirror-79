...
def list_bots(auth, endpoint_url):
    r = api_session().get(urljoin(endpoint_url, BOT_API_PATH))
    return PlatformResponse.build_from_requests_result(r)
...
def delete_bot(auth, bot_id, endpoint_url):
    r = api_session().delete(urljoin(endpoint_url,  BOT_API_PATH + bot_id))
    return PlatformResponse.build_from_requests_result(r)
...
def get_api_key(auth, bot_id, api_key, endpoint_url):
    r = api_session().get(
        urljoin(endpoint_url, BOT_API_PATH + bot_id + '/api-keys/' + api_key))
    return PlatformResponse.build_from_requests_result(r)
