            ...
            if line[-2:] == '\r\n':
                ...
                if key == 'package':
                    if value.startswith('geovar-'):
                        ...
                        log.debug('WAPT/control {}: value do not start with geovar-'.format(key))

                elif key == 'name':
                    if not value:
                        log.debug('WAPT/control {}: value is empty'.format(key))
                    else:
                        ok[key] = True
                    ...
                    if value.endswith('-0'):
                        ...
                        log.debug('WAPT/control {}: value do not ends with -0'.format(key))

                elif key == 'architecture':
                    if re.match('^(all|x86|x64)$', value):
                        ok[key] = True
                    else:
                        log.debug('WAPT/control {}: invalid value {}, should be (all|x86|x64)'.format(key, value))

                elif key == 'section':
                    if value == 'base':
                        ok[key] = True
                    else:
                        log.debug('WAPT/control {}: invalid value {}, should be base'.format(key, value))

                elif key == 'maintainer':
                    if value == 'Geovariances IT Service (sysadmin@geovariances.com)':
                        ok[key] = True
                    else:
                        log.debug('WAPT/control {}: invalid value'.format(key))

                elif key.startswith('description'):
                    if not value:
                        log.debug('WAPT/control {}: empty value'.format(key))
                    else:
                        if value[-1:] == '.':
                            ok[key] = True
                        else:
                            log.debug('WAPT/control {}: do not end with a dot'.format(key))

                elif key == 'depends':
                    if package_name != 'geovar-wapt-tools' and 'geovar-wapt-tools' not in value:
                        log.debug('WAPT/control {}: should contains geovar-wapt-tools'.format(key))
                    else:
                        ok[key] = True
                    ...
                    if value != 'BETA':
                        log.debug('WAPT/control {}: should always be BETA'.format(key))
                    else:
                        ok[key] = True
                    ...
                    if value.startswith(repo_base_url):
                        ...
                        log.debug('WAPT/control {}: should starts with {}'.format(key, repo_base_url))

                elif key == 'installed_size':
                    if value.isdigit():
                        ok[key] = True
                    else:
                        log.debug('WAPT/control {}: should be an integer'.format(key))
                    ...
                    if value.startswith('https://'):
                        ...
                        log.debug('WAPT/control {}: should be an url'.format(key))

                else:
                    log.error('WAPT/control {}: unwanted key'.format(key))
                    check = False
