...
class Liuren():
    ...
    def result_html(self, num):
        ...
        layer_two =  "<tr>"+"".join(['''<td align="right">{} <br>'''.format([a[1] for a in fourke][g]) + '''{} <br>'''.format([b[0][0] for b in fourke] [g])+ '''{} <br>'''.format([c[0][1] for c in fourke][g])+"</td>"   for g in range(0,4)])+"</tr><tr>&emsp;</tr>" 
        layer_three = "<tr>"+"".join(['''<td style="vertical-align:bottom" align="right">'''+"".join([ '''{}<br>'''.format(k) for k in list(su[i])])+'''{}<br>'''.format(general[i]) + '''{}<br>'''.format(sky[i])+"</td>" for i in range(0,3)])+'''<td style="vertical-align:bottom" align="left">'''+"".join([ '''{}<br>'''.format(k) for k in list(su[4])])+'''{}<br>'''.format(general[4])+'''{}<br>'''.format(sky[4])+"</tr>"
        layer_four = '''<tr><td style="vertical-align:bottom" align="right">'''+"".join([ '''{}'''.format(k) for k in list(su[11])])+'''{}'''.format(general[11]) + '''{}'''.format(sky[11])+"</td><td>&emsp;</td><td>&emsp;</td>" +  '''<td style="vertical-align:bottom" align="left">'''+'''{}'''.format(sky[4]) + '''{}'''.format(general[4])+"".join([ '''{}'''.format(k) for k in list(su[4])])+"</td></tr>"
        layer_five = '''<tr><td style="vertical-align:bottom" align="right">'''+"".join([ '''{}'''.format(k) for k in list(su[10])])+'''{}'''.format(general[10]) + '''{}'''.format(sky[10])+"</td><td>&emsp;</td><td>&emsp;</td>" +  '''<td style="vertical-align:bottom" align="left">'''+'''{}'''.format(sky[5]) + '''{}'''.format(general[5])+"".join([ '''{}'''.format(k) for k in list(su[5])])+"</td></tr>"
        layer_six = "<tr>"+"".join(['''<td style="vertical-align:top" align="right">'''+'''{}<br>'''.format(sky[i]) + '''{}<br>'''.format(general[i])+"".join([ '''{}<br>'''.format(k) for k in list(su[i])])+"</td>" for i in range(5,8)])+'''<td style="vertical-align:bottom" align="left">'''+'''{}<br>'''.format(sky[9])+"".join([ '''{}<br>'''.format(k) for k in list(su[9])])+'''{}<br>'''.format(general[9])+"</tr>"
        return '''<table border="0" align="center">'''+layer_one+layer_two+layer_three+layer_four+layer_five+layer_six+"</table>"
