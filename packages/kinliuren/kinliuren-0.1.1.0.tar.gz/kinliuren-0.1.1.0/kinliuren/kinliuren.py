...
class Liuren():
    ...
    def result_html(self, num):
        ...
        layer_two =  "<tr>"+"".join(['''<td align="right">{} <br>'''.format([a[1] for a in fourke][g]) + '''{} <br>'''.format([b[0][0] for b in fourke] [g])+ '''{} <br>'''.format([c[0][1] for c in fourke][g])+"</td>"   for g in range(0,4)])+"</tr><tr></tr>"
        ...
        layer_six = "<tr>"+"".join(['''<td style="vertical-align:top" align="right">'''+'''{}<br>'''.format(sky[i]) + '''{}<br>'''.format(general[i])+"".join([ '''{}<br>'''.format(k) for k in list(su[i])])+"</td>" for i in range(5,8)])+'''<td style="vertical-align:bottom" align="left">'''+"".join([ '''{}<br>'''.format(k) for k in list(su[9])])+'''{}<br>'''.format(general[9])+'''{}<br>'''.format(sky[9])+"</tr>"
