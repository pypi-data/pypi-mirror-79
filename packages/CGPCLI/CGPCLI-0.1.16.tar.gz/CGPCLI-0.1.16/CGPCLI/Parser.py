...
from CGPCLI.Errors import NotValidCGPStringError, InfiniteParseLoopError
...
def parse_to_python_object(string, from_CLI=True):
    ...
    def parse_array(string):
        ...
        def brackets_check(el, pos, values, start=datetime.now()):
            lb_counter = el.count('(')
            rb_counter = el.count(')')
            
            while lb_counter != rb_counter:
                if (datetime.now() - start).seconds > BREAK_TIME:
                    raise InfiniteParseLoopError
    ...
    try:
        ...
        raise NotValidCGPStringError(e.message)