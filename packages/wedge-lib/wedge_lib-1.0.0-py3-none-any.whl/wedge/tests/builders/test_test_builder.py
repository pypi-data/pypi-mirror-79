...
from wedge.tests.fixtures.datasets.builders.builder_fake_django_model import (
    BuilderFakeDjangoModelWithForeignKey,
)
from wedge.tests.fixtures.datasets.builders.builder_with_auto_now import (
    BuilderWithAutoNow,
)
from wedge.tests.fixtures.datasets.builders.builder_with_internal_dependencies import (
    BuilderWithInternalDependencies,
    InternalDependencyOneTestBuilder,
)
from wedge.tests.fixtures.datasets.builders.builder_with_related_dependencies import (
    BuilderWithRelatedDependencies,
)
from wedge.tests.fixtures.datasets.builders.simple_builder import SimpleBuilder
from wedge.tests.mixins.testcase_mixin import TestCaseMixin


class TestTestBuilder(TestCaseMixin):
    ...
    def test_build_post_data_with_django_model(self):
        BuilderFakeDjangoModelWithForeignKey.reset_sequence(value=1)
        target = (
            "wedge.tests.builders.abstract_test_builder"
            ".AbstractTestBuilder._is_django_model"
        )
        with patch(
            target,
            side_effect=BuilderFakeDjangoModelWithForeignKey.mock_is_django_model,
        ):
            actual = BuilderFakeDjangoModelWithForeignKey().build_post_data()
        self.assert_equals_resultset(actual)
