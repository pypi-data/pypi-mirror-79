...
from wedge.tests.helpers import date_test_helper
from wedge.tests.mixins.factory_boy_mixin import FactoryBoyMixin
...
class AbstractTestBuilder(FactoryBoyMixin, ABC):
    ...
    def build_post_data(self, **attrs) -> dict:
        """ Build post data for main factory """
        import factory

        attrs = self._complete_attrs(attrs)
        built = factory.build(
            dict, FACTORY_CLASS=self.get_factory(self._main_factory_key), **attrs
        )
        result = {}
        for attr, value in built.items():
            # remove foreign key object
            if self._is_django_model(value):
                result[f"{attr}_id"] = value.pk
            else:
                result[attr] = value
