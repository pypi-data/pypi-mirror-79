...
USAGE = """
athena-run <my_program>

Example: athena-run python main.py

Flags:
--output $FILE  Dump Athena Trace output to the path indicated by $FILE. The trace is NOT sent to Athena's cloud service
--upload $FILE  Upload trace at the path indicated by $FILE
--headless Enable Headless mode. Dump all Athena Trace output (including infrastructure metrics) to a long-term archival directory. The data is NOT sent to Athena's cloud service
--output-dir In Headless Mode, specify the archive directory. When not specified, defaults to $PWD/athena_data. --output-dir is a no-op when --headless is not passed
-h, --help Show this help message

Example:
# Output trace to a file called trace.json
athena-run --output trace.json python main.py
# Upload trace at path "trace.json"
athena-run --upload trace.json

Example:
# Archive output in default archive directory
athena-run --headless python main.py
# Archive output in /var/log/athena
athena-run --headless --output-dir /var/log/athena  python main.py

Use --output and --upload for one-off sanity checks when you eventually intend to send data to Athena's cloud.

Use --headless and --output-dir for long-term setups where data will be archived outside of Athena's cloud.
"""
...
def main():
    ...
    parser = argparse.ArgumentParser(prog="athena-run", usage=USAGE)
    ...
    if args.headless:
        os.environ["ATHENA_DRY_RUN"] = "true"
        os.environ["ATHENA_HEADLESS_MODE"] = "true"
        if args.output_dir is not None:
            os.environ["ATHENA_HEADLESS_MODE_OUTPUT"] = args.output_dir
