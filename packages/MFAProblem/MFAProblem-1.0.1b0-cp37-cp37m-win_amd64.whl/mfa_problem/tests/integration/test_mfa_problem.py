...
file_names = listdir(os.path.join(cwd, 'ref_output'))
for file_name in file_names:
    try:
        if 'xlsx' in file_name:
            continue
        key = os.path.splitext(file_name)[0][len('expected_'):]
        file_name = os.path.join(cwd, 'ref_output', file_name)
...
class MFAProblemsTests(unittest.TestCase):
    ...
    def prepare_test(
        ...
        output_dir = os.path.join(cwd, 'ref_output')
    ...
    def tearDownClass(cls):
        if cls.generate_results:
            for name in cls.new_results:
                ...
                cwd = os.path.dirname(__file__)
                file_name = os.path.join(cwd, 'ref_output', 'expected_' + name + '.json')
