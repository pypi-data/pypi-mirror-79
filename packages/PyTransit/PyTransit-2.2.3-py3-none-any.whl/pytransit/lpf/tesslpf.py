...
class TESSLPF(BaseLPF):
    ...
    def __init__(self, name: str, dfile: Path = None, tic: int = None, zero_epoch: float = None, period: float = None,
                 nsamples: int = 2, trdur: float = 0.125, bldur: float = 0.3, use_pdc=True, remove_crowding: bool = False,
        ...
        if tic is not None:
            ...
            #if use_pdc and not remove_crowding:
            #    contamination = 1 - tb.meta['CROWDSAP']
            #    ts['flux'].data[:] = contamination + (1 - contamination) * ts['flux'].value
            self.contamination =  1 - tb.meta['CROWDSAP']
        else:
        ...
        fluxes = [self.contamination + (1-self.contamination)*f for f in fluxes]
