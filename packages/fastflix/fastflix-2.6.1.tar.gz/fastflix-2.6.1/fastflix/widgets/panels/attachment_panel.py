...
class Attachment(QtWidgets.QTabWidget):
    def __init__(self, parent, attachment, index, enabled=True, first=False):
        self.loading = True
        super(Attachment, self).__init__(parent)
        self.parent = parent
        self.index = index
        self.outdex = None
        self.attachment = Box(attachment, default_box=True)
        self.first = first
        self.last = False
        self.attachment_lang = attachment.get("tags", {}).get("language")

        self.widgets = Box(
            track_number=QtWidgets.QLabel(f"{self.index}:{self.outdex}" if enabled else "❌"),
            title=QtWidgets.QLabel(f"  {self.attachment.codec_long_name}"),
        ...
        for disposition, is_set in self.attachment.disposition.items():
            if is_set:
                try:
                    self.widgets.disposition.setCurrentIndex(dispositions.index(disposition))
                except ValueError:
                    pass  # TODO figure out all possible dispositions for attachments / log if issue
                break
    ...
    def init_language(self):
        ...
        try:
            self.widgets.language.setCurrentIndex(language_list.index(self.attachment_lang))
        except Exception:
...
class AttachmentList(FlixList):
    def __init__(self, parent, starting_pos=0):
        super().__init__(parent, "Attachments", starting_pos)
        self.main = parent.main

    def new_source(self, starting_pos=0):
        self.starting_pos = starting_pos
        self.tracks = []
        for index, track in enumerate(self.main.streams.attachment):
            new_item = Attachment(self, track, index=track.index, first=True if index == 0 else False)
    ...
    def get_settings(self):
        ...
        return Box(attachment_tracks=tracks)
