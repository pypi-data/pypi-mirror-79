...
def parQuadratureFixedGauss(func, lowlimit, uplimit, parname, n=5, weights=None, **kwargs):
    ...
    If function returns dataArray, only the .Y values are integrated and returned as dataArray.
...
def parQuadratureFixedGaussxD(func, lowlimit, uplimit, parnames, n=5,
    ...
    func : callable
        ...
        The return value should be 2 dimensional with first dimension along integration variable
    ...
    if isinstance(res[0], dA):
        Y = [r.Y for r in res]
        res[0].Y = vol * np.sum(weights * np.atleast_2d(Y).T, axis=-1)
        res[0].weightNormFactor = normfactor
        for na, me in zip(parnames, parmean):
            setattr(res[0], na, me)
        return res[0]
    else:
...
def parAdaptiveCubature(func, lowlimit, uplimit, parnames, fdim=None, adaptive='p',
    ...
    func : function
        ...
        The return value should be of shape [len(parnames), fdim]
    ...
    if iscomplex:
        return val.view(np.complex), np.abs(err.view(np.complex))
    else:
        return val, err
