...
class Fluorescence(Optics):
    ...
    def get(self, illuminated_volume, limits, **kwargs):
        ...
        for i, z in zip(index_iterator, z_iterator):
            ...
            output_image[:, :, 0] += field[:padded_volume.shape[0],
                                           :padded_volume.shape[1]]
...
class Brightfield(Optics):
    ...
    def get(self, illuminated_volume, limits, fields, **kwargs):
        ...
        pupils = (self._pupil(volume.shape[:2], defocus=[1], include_aberration=False, **kwargs) +
                  self._pupil(volume.shape[:2], defocus=[-z_limits[1]], include_aberration=True, **kwargs))
