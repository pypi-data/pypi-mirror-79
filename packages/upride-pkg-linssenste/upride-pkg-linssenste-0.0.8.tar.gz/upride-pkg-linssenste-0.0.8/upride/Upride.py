...
class Upride(): 
    ...
    def getTrackByID(self, trackID, geojson=False): 
        ...
        for pos in track_data['points']: 
            ...
            if not self.__crossPoint((startPos['lat'], startPos['lng']), (pos['lat'], pos['lng']), 500) and cutStartPos == -1:
                cutStartPos = indexPos
            
            if self.__crossPoint((endPos['lat'], endPos['lng']), (pos['lat'], pos['lng']), 500) and cutStopPos == -1: 
