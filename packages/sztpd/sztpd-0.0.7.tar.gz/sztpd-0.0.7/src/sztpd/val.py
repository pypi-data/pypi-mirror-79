...
class ValidationLayer:
	...
	async def handle_post_config_request(F,data_path,query_dict,request_body):
		...
		try:T=F.dm.parse_resource_id(H)
		...
		U,G=A.split(':');M=I.schema_node;J=M.get_child(G,U)
		...
		if isinstance(J,yangson.schemanode.SequenceNode):
			if type(J)==yangson.schemanode.ListNode:
				V=J.keys[0];W=V[0]
				if type(C[A])!=list:raise InvalidInputDocument(b+G+c)
				if len(C[A])!=1:raise InvalidInputDocument(d+G+e)
				N=C[A][0];Q=N[W]
			...
			else:E=G;K=H+_A+E+f+quote(Q,safe='')
			try:g=F.inst.raw_value();X=F.dm.parse_resource_id(K)
			...
			for O in B:
				...
				if O==_B:
					try:Y=F.dm.parse_resource_id(B[_B])
			...
			try:
				...
				else:P=I.put_member(G,C[A],raw=_D).up()
