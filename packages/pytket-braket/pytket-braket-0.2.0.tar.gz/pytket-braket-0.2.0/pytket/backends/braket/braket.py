...
from pytket.braket.braket_convert import tk_to_braket
...
from pytket.routing import Architecture, FullyConnected, NoiseAwarePlacement
...
def _obs_from_qps(pauli: QubitPauliString) -> (Observable, QubitSet):
    pdict = pauli.to_dict()
    obs, qbs = [], []
...
class BraketBackend(Backend):
    ...
    def __init__(
        ...
        if self._device_type == _DeviceType.QPU:
            ...
            if connectivity["fullyConnected"]:
                self._all_qubits = range(n_qubits)
            else:
        ...
        if self._device_type == _DeviceType.QPU:
            ...
            if schema == IONQ_SCHEMA:
                ...
                for node in arch.nodes:
                    ...
                    err_1q_cont = QubitErrorContainer(self._singleqs)
                    for optype in self._singleqs:
                        err_1q_cont.add_error((optype, 1 - nodespecs["f1QRB"]))
                    err_1q_cont.add_readout(nodespecs["fRO"])
                    node_errs[node] = err_1q_cont
                for coupling in arch.coupling:
                    ...
                    err_2q_cont.add_error((OpType.CZ, 1 - couplingspecs["fCZ"]))
                    edge_errs[coupling] = err_2q_cont
    ...
    def _run(
        self, bkcirc: braket.circuits.Circuit, n_shots: int = 0, **kwargs
    ...
    def process_circuits(
        ...
        **kwargs: KwargTypes,
        ...
        want_state = n_shots is None or n_shots == 0
        if want_state:
            n_shots = 0
        elif n_shots < self._sample_min_shots or n_shots > self._sample_max_shots:
            raise ValueError(
    ...
    def _get_expectation_value(
        ...
        **kwargs: KwargTypes,
        ...
        if (
            ...
            raise ValueError(
                f"n_shots must be between {self._expectation_min_shots} and {self._expectation_max_shots}"
            )
        ...
        res = task.result()
        return res.get_value_by_result_type(restype)
    ...
    def _get_variance(
        ...
        **kwargs: KwargTypes,
        ...
        if n_shots < self._variance_min_shots or n_shots > self._variance_max_shots:
            raise ValueError(
                f"n_shots must be between {self._variance_min_shots} and {self._variance_max_shots}"
            )
    ...
    def get_probabilities(
        ...
        **kwargs: KwargTypes,
        ...
        if (
            ...
            raise ValueError(
                f"n_shots must be between {self._probability_min_shots} and {self._probability_max_shots}"
            )
        if valid_check:
