...
def query(query, experiment_id, host):
    ...
    """
    import json
    resp = requests.get(f"https://{host}:1880/query?query={json.dumps(query)}&experiment_id={experiment_id}")
...
class experiment:
    ...
    def query(self, query):
        ...
        """
        import json
        resp = requests.get(f"https://{self.host}:1880/query?query={json.dumps(query)}&experiment_id={self.experiment_id}")
        return json.loads(resp.text)
    ...
    def listener(self, func=None):
        ...
        if func is None:
            ...
            func = default_func
        subscribe.callback(func, f"MDML_DEBUG/{self.experiment_id}", hostname=self.host,
            auth={'username': self.username, 'password': self.password})
