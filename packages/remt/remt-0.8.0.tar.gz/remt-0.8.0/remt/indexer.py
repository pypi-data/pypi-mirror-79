...
from .util import split, to_point_x, to_point_y
...
def page_text(page: Poppler.Page, selections: tp.List[TextSelection]) -> tp.List[str]:
    """
...
def is_horizontal_line(
        y_min: float, y_max: float, ys: float, width: float
    ) -> bool:
    """
    ...
    """
    return y_min >= ys - width and ys + width >= y_max
...
def merge_line_selection_item(acc: tp.List[TextSelection], ls: TextSelection) \
    ...
    to_merge = acc and acc[-1].overlaps(ls, PEN_WIDTH_HIGHLIGHTER / 2)
