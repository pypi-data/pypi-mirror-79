...
from .. import indexer as remt_idx
...
def test_page_text():
    ...
    """
    page = mock.MagicMock()
    with mock.patch.object(remt_idx, 'pdf_text') as mock_pdf_text:
        mock_pdf_text.side_effect = ['a', '', 'b', 'c']

        # note: number of strokes the same as number of above side effects
        result = remt_idx.page_text(page, [1, 2, 3, 4])
...
def test_fmt_header():
    ...
    result = remt_idx.fmt_header(page)
    print(result)
    assert 'Page IX (10)\n============' == result
...
def test_fmt_text():
    ...
    # page_text function
    texts = ['abc', 'xyz\nzyx']
    result = remt_idx.fmt_text(texts)
    expected = """\
::
...
def test_is_highlighter():
    ...
    """
    assert not remt_idx.is_highlighter(Page(1))
    assert not remt_idx.is_highlighter(Stroke(1, Pen.FINELINER_1, 1, 2, []))
    assert remt_idx.is_highlighter(Stroke(1, Pen.HIGHLIGHTER_1, 1, 2, []))

def test_is_horizontal_line():
    """
    Test checking if a candidate is a horizontal line.
    """
    assert not remt_idx.is_horizontal_line(1, 10, 5, 3)
    assert remt_idx.is_horizontal_line(1, 10, 5, 5)
...
def test_to_line_selection():
    ...
    result = remt_idx.to_line_selection(stroke)
