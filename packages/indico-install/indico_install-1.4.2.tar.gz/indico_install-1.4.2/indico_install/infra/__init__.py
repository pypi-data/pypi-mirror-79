...
def generate_saml(ctx, host=None, metadata_url=None, custom_saml=None):
    ...
    """
    if custom_saml:
        with open(custom_saml) as settings:
            settings_content = json.load(settings)
        run_cmd(
            f'''kubectl create secret generic indico-sso-secrets --from-literal=settings.json="{settings_content}"'''
        )
        ctx.invoke(
            restart, services=["noct"], contains=False, wait="2m"
        )
        return
    
    # not using custom settings
    if not (host and metadata_url):
        click.secho("Please provide a host and metadata url if not using a custom settings.json file", fg="red")
        return
    try:
        obj = untangle.parse(metadata_url)
    except HTTPError:
        click.secho(
            "Federation URL not found. Please check that this is the correct URL",
            fg="red",
        )
        return
    ...
    settings_content = convertb64(json.dumps(stg, indent=4))
    run_cmd(
        """kubectl patch secret indico-sso-secrets -p '{"data":{"settings.json":"""
        f'"{settings_content}"'
        "}}'"
    )
    ctx.invoke(
