...
def exc(ctx, user_email):
    ...
    """
    noct_pod = run_cmd("kubectl get pods | grep noct | head -n 1 | awk '{print $1}'", silent=True)
    run_cmd(f"kubectl exec -it {noct_pod} -- python3 /noct/alembic/migrations/setup_user_access.py {user_email}")
