    ...
    def set3rdNotifyInfo(self, notify):
        ...
        if notify.getType() is not None:
            ...
            if notify.getIntent() is not None:
                ...
                #放开intent为空限制
                if notify.getType() == gt_req_pb2.NotifyInfo._intent:
                    notifyInfo.intent = ""
            if notify.getUrl() is not None:
                notifyInfo.url = notify.getUrl()
        else:
            setPayloadInfo(notifyInfo,notify)
        if extKv is not None:
            for valExt in extKv:
                ext = notifyInfo.extKV.add()
                ext.key = valExt.key
                ext.value = valExt.value
                ext.constains = valExt.constains
        self.getPushInfo().notifyInfo.CopyFrom(notifyInfo)
        self.getPushInfo().validNotify = True

    #厂商通道关闭
    def setBlackThirdParty(self,blackThirdParty):
        blackThirdParty = list(set(blackThirdParty))
        self.pushInfo.blackThirdparty.extend(blackThirdParty)
