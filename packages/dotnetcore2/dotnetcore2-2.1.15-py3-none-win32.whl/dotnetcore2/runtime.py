...
def log_event(name, error=None, **kwargs):
    """
    Log Success/Failure events via azureml-telemetry if it is installed and enabled.
    """
    try:
        from azureml.telemetry import get_event_logger
        from azureml.telemetry.contracts import Event, RequiredFields, StandardFields
        event_logger = get_event_logger()
        prop_names = ['ErrorMsg', 'distro']
        prop_names.extend(list(kwargs.keys()))
        event_logger.log_event(Event(
            name='dotnetcore2.{}'.format(name),
            required_fields=RequiredFields(component_name='dotnetcore2'),
            standard_fields=StandardFields(task_result='Failure' if error else 'Success'),
            extension_fields={
                'ErrorMsg': '{}'.format(error),
                'distro': str(distro.info()),
                **kwargs
            }
        ), white_listed_properties=prop_names)
        event_logger.flush()
...
__version__ = '2.1.15'   # {major dotnet version}.{minor dotnet version}.{revision}
            ...
            try:
                ...
                if e.errno != errno.EEXIST:
                    ...
                    if e.errno == errno.EACCES or e.errno == errno.EPERM:
                        ...
                        log_event('FileLock.acquire', error=err_msg)
...
def ensure_dependencies() -> Optional[str]:
            ...
            def attemp_get_deps():
                ...
                try:
                    ...
                    if e.code == 404:
                        ...
                        log_event('ensure_dependencies', error=err_msg, missing_pkgs=list(missing_pkgs))
    ...
    log_event('ensure_dependencies', could_write=could_write, missing_pkgs=list(missing_pkgs))
    return deps_path
