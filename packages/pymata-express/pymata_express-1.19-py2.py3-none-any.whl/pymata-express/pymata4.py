...
class Pymata4:
    ...
    def __init__(self, com_port=None, baud_rate=115200, arduino_wait=4,
        ...
        """
        print('Instaniating PymataExpress...')
        self.express = PymataExpress()
        self.loop = asyncio.get_event_loop()
        # atexit.register(self.express.shut_it_down)
    ...
    def analog_read(self, pin):
        ...
        """
        task = asyncio.ensure_future(self.express.analog_read(pin))
    ...
    def analog_write(self, pin, value):
        ...
        # asyncio.run(self.express.analog_write(pin, value))
        self.loop.run_until_complete(self.express.analog_write(pin, value))
    ...
    def digital_read(self, pin):
        ...
        """
        task = asyncio.ensure_future(self.express.digital_read(pin))
        value = self.loop.run_until_complete(task)
        return value
    ...
    def digital_pin_write(self, pin, value=0):
        ...
        # self.loop.run_until_complete(task)
        asyncio.run(self.express.digital_pin_write(pin, value))
    ...
    def digital_write(self, pin, value=0):
        ...
        :returns: No return value
        """
        self.loop.run_until_complete(self.express.digital_write(pin, value))
        # task = asyncio.ensure_future(self.express.digital_write(pin, value))
        # self.loop.run_until_complete(task)
        #self.loop.run_until_complete(task)
    ...
    def disable_analog_reporting(self, pin):
        ...
        :returns: No return value
        """
        task = asyncio.ensure_future(self.express.disable_analog_reporting(pin))
        self.loop.run_until_complete(task)
    ...
    def disable_digital_reporting(self, pin):
        ...
        :returns: No return value
        """
        task = asyncio.ensure_future(self.express.disable_digital_reporting(pin))
        self.loop.run_until_complete(task)
    ...
    def encoder_config(self, pin_a, pin_b, cb=None, cb_type=None,
        ...
        :returns: No return value
        """
        task = asyncio.ensure_future(self.express.encoder_config(pin_a, pin_b,
                                                              cb, cb_type,
                                                              hall_encoder))
        self.loop.run_until_complete(task)
    ...
    def encoder_read(self, pin):
        ...
        """
        try:
            task = asyncio.ensure_future(self.express.encoder_read(pin))
            value = self.loop.run_until_complete(task)
            return value
        except RuntimeError:
            self.shutdown()
    ...
    def enable_analog_reporting(self, pin):
        ...
        :returns: No return value
        """
        task = asyncio.ensure_future(self.express.enable_analog_reporting(pin))
        self.loop.run_until_complete(task)
    ...
    def enable_digital_reporting(self, pin):
        ...
        :returns: No return value
        """
        task = asyncio.ensure_future(self.express.enable_digital_reporting(pin))
        self.loop.run_until_complete(task)
    ...
    def extended_analog(self, pin, data):
        ...
        :returns: No return value
        """
        task = asyncio.ensure_future(self.express.extended_analog(pin, data))
        self.loop.run_until_complete(task)
    ...
    def get_analog_latch_data(self, pin):
        ...
        """
        task = asyncio.ensure_future(self.express.get_analog_latch_data(pin))
    ...
    def get_analog_map(self, cb=None):
        ...
        """
        task = asyncio.ensure_future(self.express.get_analog_map())
    ...
    def get_capability_report(self, raw=True, cb=None):
        ...
        """
        task = asyncio.ensure_future(self.express.get_capability_report())
        report = self.loop.run_until_complete(task)
        if raw:
            if cb:
                cb(report)
            ...
            self.express._format_capability_report(report)
    ...
    def get_digital_latch_data(self, pin):
        ...
        :returns:  [latched_state, threshold_type, threshold_value,
                    latched_data, time_stamp]
        """
        task = asyncio.ensure_future(self.express.get_digital_latch_data(pin))
        l_data = self.loop.run_until_complete(task)
        return l_data
    ...
    def get_firmware_version(self, cb=None):
        ...
        """
        task = asyncio.ensure_future(self.express.get_firmware_version())
    ...
    def get_protocol_version(self, cb=None):
        ...
        :returns:If no callback is specified, the firmware version
        """
        task = asyncio.ensure_future(self.express.get_protocol_version())
        version = self.loop.run_until_complete(task)
    ...
    def get_pin_state(self, pin, cb=None):
        ...
        """
        task = asyncio.ensure_future(self.express.get_pin_state(pin))
        report = self.loop.run_until_complete(task)
    ...
    def get_pymata_version(self):
        ...
        """
        task = asyncio.ensure_future(self.express.get_pymata_version())
        self.loop.run_until_complete(task)
    ...
    def i2c_config(self, read_delay_time=0):
        ...
        :returns: No return value
        """
        task = asyncio.ensure_future(self.express.i2c_config(read_delay_time))
        self.loop.run_until_complete(task)
    ...
    def i2c_read_data(self, address):
        ...
        """
        task = asyncio.ensure_future(self.express.i2c_read_data(address))
        value = self.loop.run_until_complete(task)
        return value
    ...
    def i2c_read_request(self, address, register, number_of_bytes, read_type,
        ...
        task = asyncio.ensure_future(self.express.i2c_read_request(address, register,
                                                                number_of_bytes,
                                                                read_type,
                                                                cb,
                                                                cb_type))
    ...
    def i2c_write_request(self, address, args):
        ...
        :returns: No return value
        """
        task = asyncio.ensure_future(self.express.i2c_write_request(address, args))
        self.loop.run_until_complete(task)
    ...
    def keep_alive(self, period=1, margin=.3):
        ...
        :returns: No return value
        """
        asyncio.ensure_future(self.express.keep_alive(period, margin))
    ...
    def play_tone(self, pin, tone_command, frequency, duration=None):
        ...
        :returns: No return value
        """
        task = asyncio.ensure_future(self.express.play_tone(pin, tone_command,
                                                         frequency, duration))
        self.loop.run_until_complete(task)
    ...
    def send_reset(self):
        ...
        :returns: No return value
        """
        task = asyncio.ensure_future(self.express.send_reset())
        self.loop.run_until_complete(task)
    ...
    def servo_config(self, pin, min_pulse=544, max_pulse=2400):
        ...
        :returns: No return value
        """
        task = asyncio.ensure_future(self.express.servo_config(pin, min_pulse,
                                                            max_pulse))
        self.loop.run_until_complete(task)
    ...
    def set_analog_latch(self, pin, threshold_type, threshold_value,
        ...
        task = asyncio.ensure_future(self.express.set_analog_latch(pin, threshold_type, threshold_value, cb, cb_type))
    ...
    def set_digital_latch(self, pin, threshold_value, cb=None, cb_type=None):
        ...
        :returns: True if successful, False if parameter data is invalid
        """
        task = asyncio.ensure_future(self.express.set_digital_latch(pin, threshold_value, cb, cb_type))
        result = self.loop.run_until_complete(task)
        return result
    ...
    def set_pin_mode(self, pin_number, pin_state, callback=None, cb_type=None):
        ...
        :returns: No return value
        """
        asyncio.run(self.express.set_pin_mode(pin_number, pin_state, callback))
    ...
    def set_sampling_interval(self, interval):
        ...
        :returns: No return value
        """
        task = asyncio.ensure_future(self.express.set_sampling_interval(interval))
        self.loop.run_until_complete(task)
    ...
    def sleep(self, time):
        ...
        """
        try:
            self.loop.run_until_complete(asyncio.sleep(time))
    ...
    def shutdown(self):
        ...
        :returns: No return value
        """
        # task = asyncio.ensure_future(self.express.shutdown())
        self.loop.run_until_complete(self.express.shutdown())
    ...
    def sonar_data_retrieve(self, trigger_pin):
        ...
        """
        task = asyncio.ensure_future(self.express.sonar_data_retrieve(trigger_pin))
    ...
    def sonar_config(self, trigger_pin, echo_pin, cb=None, ping_interval=50,
        ...
        :returns: No return value
        """
        task = asyncio.ensure_future(self.express.sonar_config(trigger_pin,
                                                            echo_pin, cb,
                                                            ping_interval,
    ...
    def stepper_config(self, steps_per_revolution, stepper_pins):
        ...
        """
        task = asyncio.ensure_future(self.express.stepper_config(steps_per_revolution,
                                                              stepper_pins))
        self.loop.run_until_complete(task)
    ...
    def stepper_step(self, motor_speed, number_of_steps):
        ...
        """
        task = asyncio.ensure_future(self.express.stepper_step(motor_speed,
                                                            number_of_steps))
        self.loop.run_until_complete(task)
    ...
    def pixy_init(self, max_blocks=5, cb=None, cb_type=None):
        ...
        :returns: No return value.
        """
        task = asyncio.ensure_future(self.express.pixy_init(max_blocks, cb, cb_type))
        self.loop.run_until_complete(task)
    ...
    def pixy_get_blocks(self):
        ...
        :returns: Pixy data
        """
        return self.express.pixy_blocks
    ...
    def pixy_set_servos(self, s0, s1):
        ...
        :returns: No return value.
        """
        task = asyncio.ensure_future(self.express.pixy_set_servos(s0, s1))
        self.loop.run_until_complete(task)
    ...
    def pixy_set_brightness(self, brightness):
        ...
        :returns: No return value.
        """
        task = asyncio.ensure_future(self.express.pixy_set_brightness(brightness))
        self.loop.run_until_complete(task)
    ...
    def pixy_set_led(self, r, g, b):
        ...
        :returns: No return value.
        """
        task = asyncio.ensure_future(self.express.pixy_set_led(r, g, b))
        self.loop.run_until_complete(task)
