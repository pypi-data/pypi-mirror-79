...
def massbank(frag_list, minsimilarity=500):
    ...
    for result in json_result:
        ...
        try:
            formula = [i for i in cpd_meta if i['name'] == 'molecular formula'
                       and i['category'] == 'none'][0]['value']
        except Exception:
            formula = "NA"
        try:
        ...
        try:
            instrument = [i for i in ms_meta if
                          i['name'] == 'instrument type'][0]['value']
        except Exception:
            instrument = "NA"
        mode = [i for i in ms_meta if i['name'] ==
                'ionization mode'][0]['value']
        try:
            energy = [i for i in ms_meta if i['name'] ==
                      'collision energy'][0]['value']
        except Exception:
            energy = "NA"
        try:
            p_type = [i for i in ms_meta if i['name'] ==
                      'precursor type'][0]['value']
        except Exception:
            p_type = "NA"
        score = result['score']
