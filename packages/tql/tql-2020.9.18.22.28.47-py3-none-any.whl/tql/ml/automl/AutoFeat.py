...
class AutoFeat(object):
    ...
    def dfs(self, target_entity=None, max_depth=1, features_only=True, n_jobs=1, chunk_size=None,
            ignore_variables=['uid']):
    ...
    def run_dfs(self, es, target_entity, max_depth, chunk_size, n_jobs, features_only, ignore_variables):
        _ = ft.dfs(
            entityset=es,
            target_entity=target_entity,  # 具有唯一ID: 不重复id的base_es或者normalize_entity生成的唯一id es
            max_depth=max_depth,
            verbose=1,
