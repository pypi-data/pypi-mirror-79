...
__REGEX_SECTION_NUMBERS__ = [re.compile(r"^[0-9]{1,}(\.[0-9]+)?\.$"),
                             re.compile(r"^[0-9]{1,}(\.[0-9]+)?\.-$")]  # "1.0", "4.2", "02.-" etc
...
class SentenceSplit():
    ...
    def _concat_broken_phrases(self, df):
        continuation_terms = ["no", "na", "nos", "nas", "de", "do", "da", "dos", "das", ",", "sua", "pela", "para",
                              "pelo"]
        ...
        for i in range(len(df['final_text'])):
            ...
            if last_index_with_text != None:
                ...
                starts_money_simbol = current_text.startswith(tuple(["(", "R$"])) and \
                                      last_text_processed.endswith(__ENDING_PUNCTUATION_TUPLE__)
                is_section_number = any(regex.match(last_text_processed) for regex in __REGEX_SECTION_NUMBERS__)

                if starts_money_simbol or is_section_number:
