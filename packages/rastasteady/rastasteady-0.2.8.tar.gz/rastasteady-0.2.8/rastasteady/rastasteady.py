...
class RastaSteady:
   ...
   def dual(self):
      if not self.tmpfileexists('dual.mp4'):
          ...
          command = 'ffmpeg -y -i ' + self.inputfile + ' -i ' + self.tmpfile(video) + ' -filter_complex hstack ' + self.tmpfile('dual.mp4')
          os.system(command)
   ...
   def stabilize(self):
      ...
      if not self.tmpfileexists('stabilized.mp4'):
          command2 = 'ffmpeg -y -i ' + self.inputfile + ' -vf vidstabtransform=zoom=-5:input="' + self.tmpfile('transforms.trf') + ':zoom=0:smoothing=5,unsharp=5:5:0.8:3:3:0.4" -acodec copy -vcodec libx264 -level 50 ' + self.tmpfile('stabilized.mp4')
          os.system(command2)
