...
class FlowRequest(object):
    ...
    Args
    ----
    - names (list[str]): The names of flow objects to return results for. An empty list will return results for all flows
    ...
    - drilldown_column_names (list[str]): Any configured Flow.Pattern.drilldown_column_name unique names to be included in the results. If the name is not configured then it will be excluded from the Result.Flow.columns and Result.Flow.rows
    """
    def __init__(self, names=[], columns=[], drilldown_column_names=[]):
        if isinstance(names, (list, type(None))) is True:
            self.names = names
        else:
            raise TypeError('names must be an instance of (list, type(None))')
        if isinstance(columns, (list, type(None))) is True:
            ...
            raise TypeError('columns must be an instance of (list, type(None))')
        if isinstance(drilldown_column_names, (list, type(None))) is True:
            self.drilldown_column_names = drilldown_column_names
        else:
            raise TypeError('drilldown_column_names must be an instance of (list, type(None))')
