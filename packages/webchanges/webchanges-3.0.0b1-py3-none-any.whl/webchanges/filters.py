...
class BeautifyFilter(FilterBase):
    ...
    def filter(self, data, subfilter):
        try:
            ...
            raise ImportError('Python package "BeautifulSoup" is not installed; cannot use the "beautify" filter')
        ...
        try:
            ...
            logger.info('Python package "jsbeautifier" is not installed; will not beautify <script> tags')
            jsbeautifier = None
    ...
    def filter(self, data, subfilter):
        ...
        if method in ('html2text', 'pyhtml2text'):  # pythtml2text for backward compatibility
            if method == 'pyhtml2text':
                logger.warning("'html2text: pyhtml2text' is deprecated (now default): replace with 'html2text:'")
            ...
            try:
                ...
                raise ImportError('Python package "BeautifulSoup" is not installed; cannot use the "html2text: bs4"'
                                  ' filter')
            ...
            if method == 're':
                logger.warning("'html2text: re' is deprecated: replace with 'html2text: strip_tags'")
            stripped_tags = re.sub(r'<[^>]*>', '', data)
            d = '\n'.join((line.rstrip() for line in stripped_tags.splitlines() if line.strip() != ''))
            return d

        elif method == 'lynx':
                logger.error("'method: lynx' is no longer supported. Use 'method: html2text' instead")
...
class Pdf2TextFilter(FilterBase):
    ...
    def filter(self, data, subfilter):
        ...
        if not isinstance(data, bytes):
            raise ValueError('The "html2text: pdf2text" filter needs bytes input (is it the first filter?)')

        try:
            import pdftotext
        except ImportError:
            raise ImportError('Python package "pdftotext" (and OS-specific dependencies) is not installed; cannot use'
                              ' "html2text: pdf2text" filter')
...
class Ical2TextFilter(FilterBase):
    ...
    def filter(self, data, subfilter):
        try:
            ...
            raise ImportError('Python package "vobject" is not installed; cannot use "html2text: ical2text" filter')
        result = []
...
class GrepFilter(KeepLinesFilter):
    """Deprecated; use keep_lines_matching instead"""

    __kind__ = 'grep'

    def filter(self, data, subfilter):
        return super().filter(data, subfilter)


class DeleteLinesFilter(FilterBase):
    """Remove lines matching a regular expression"""

    __kind__ = 'delete_lines_matching'
...
class InverseGrepFilter(DeleteLinesFilter):
    """Deprecated; use delete_lines_matching instead"""
...
class RegexSub(FilterBase):
    ...
    def filter(self, data, subfilter):
        if 'pattern' not in subfilter:
            raise ValueError(f'{self.__kind__} needs a pattern')
...
class ShellPipeFilter(FilterBase):
    ...
    def filter(self, data, subfilter):
        if 'command' not in subfilter:
            raise ValueError(f'{self.__kind__} filter needs a command')
...
class OCRFilter(FilterBase):
    """Convert text in images to plaintext (requires Python package 'pytesseract')"""
    ...
    def filter(self, data, subfilter):
        ...
        try:
            ...
            raise ImportError('Python package "pytesseract" is not installed; cannot use the "ocr" filter')
