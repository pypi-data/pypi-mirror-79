...
or you can use the ``webchanges --edit`` command.
...
Quickly adding new URLs to the job list from the command line::
    ...
    webchanges --add url=http://example.org,name=Example
...
be notified daily if ``webchanges`` still works, you can watch the output
...
report every time webchanges is run.
...
``webchanges`` command with the ``torify`` wrapper to access .onion pages:
...
Only show added or removed lines (rudimentary)
----------------------------------------------
...
webchanges, so if it outputs any status, the ``CHANGED`` notification that
webchanges does will contain the output of the custom script, not the original
...
a frontend (with GUI) to ``webchanges``. The tool can be used to
select a region of a web page. It then generates a configuration
for ``webchanges`` to run ``pyvisualcompare`` and generate a hash
