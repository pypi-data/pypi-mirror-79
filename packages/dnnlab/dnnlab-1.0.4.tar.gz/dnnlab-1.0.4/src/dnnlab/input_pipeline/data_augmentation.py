...
class YoloAugmentation:
    ...
    # TODO: make permutation of original images + augmented images to increase size of dataset
    def augment_data(self,
        ...
        for elements in dataset:
            ...
            [orig_imgs_list.append(image) for image in images]
            [orig_labels_list.append(label) for label in labels]

            boxes, indices = self.convert_to_imgaug_bboxes(labels)
            aug_imgs, bbs = self.augmenters(images=images,
                                            bounding_boxes=boxes)
            aug_labels = self.convert_to_yolo_bboxes(bbs, indices, labels)

            [aug_imgs_list.append(image) for image in aug_imgs]
            [aug_labels_list.append(label) for label in aug_labels]

        imgs_list = orig_imgs_list + aug_imgs_list
        labels_list = orig_labels_list + aug_labels_list

        # np_img = np.array(imgs_list)
        np_img = np.array(aug_imgs_list)

        # np_label = np.array(labels_list)
        np_label = np.array(aug_labels_list)

        # create tf.data.Dataset from numpy-arrays
        aug_dataset = tf.data.Dataset.from_tensor_slices((np_img, np_label))
        ...
        # TODO: prefetching and cache - necessary a second time???
        # Prefetching
        if prefetching:
