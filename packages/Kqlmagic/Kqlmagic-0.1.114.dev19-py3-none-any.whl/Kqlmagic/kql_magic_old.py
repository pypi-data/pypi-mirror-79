...
@magics_class
class Kqlmagic_old(Magics, Configurable):
    ...
    device_code_login_notification = Enum(
        ["auto", "frontend", "browser", "terminal", "email"],
        "auto", 
        config = True, 
    ...
    def __init__(self, shell):
        ...
        # self.ip = get_ipython()  # pylint: disable=undefined-variable
    ...
    def _init_options(self):
        ...
        app = getattr(self, "notebook_app") or "auto"
    ...
    def _start_temp_files_server(self, **options):
        ...
        if folder_name is not None:
            ...
            self.temp_files_server_manager = FilesServerManagement(server_py_code,None, adjust_path(f"{root_path}"), folder_name, **options)
    ...
    def execute(self, line:str, cell:str="", local_ns:dict={},
        ...
        if self.shell is not None:
            ...
            import inspect
            user_ns = inspect.stack()[1][0].f_globals.copy()
            user_ns.update(inspect.stack()[1][0].f_locals)
        user_ns = self.shell.user_ns.copy()
        user_ns.update(local_ns)
    ...
    def _add_help_to_jupyter_help_menu(self, user_ns, start_time=None, **options):
        if Help_html.showfiles_base_url is None and self.notebook_app not in ["azuredatastudio", "ipython", "visualstudiocode"]:
    ...
    def execute_query(self, parsed, user_ns: dict, result_set=None, override_vars=None):
        ...
        try:
            ...
            if options.get("columns_to_local_vars", self.columns_to_local_vars):
                ...
                if options.get("feedback", self.feedback) and self.shell is not None:
            ...
            if options.get("result_var") and result_set is None and self.shell is not None:
    ...
    def _override_default_configuration(self):
        ...
        if app is not None:
            ...
            if app is not None:
                ...
                setattr(self, 'notebook_app', app.strip())
        ...
        if email_details:
            ...
            setattr(self, 'device_code_notification_email', email_details.strip())
        ...
        if load_mode:
            ...
            if load_mode.startswith("'") or load_mode.startswith('"'):
                ...
                if load_mode == "silent":
                    # self.ip.run_line_magic("config", f'{Constants.MAGIC_CLASS_NAME}.show_init_banner = True')
                    setattr(self, 'show_init_banner', True)
