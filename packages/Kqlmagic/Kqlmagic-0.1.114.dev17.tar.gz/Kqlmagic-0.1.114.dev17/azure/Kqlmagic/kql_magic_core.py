...
from .constants import Constants, Cloud, ConnStrKeys, SsoEnvVarParam, Email
...
class ShortError(Exception):
    ...
    def __init__(self, exception, info_msg):
...
class Kqlmagic_core(object):
    ...
    def _init_options(self):
        ...
        if app == "auto": # ELECTRON_RUN_AS_NODE
            notebook_service_address = getattr(self.default_options, "notebook_service_address") or ""
            ...
            elif notebook_service_address.endswith(".azureml.net") or notebook_service_address.endswith(".azureml.ms"):
                app = "azuremljupyternotebook" if os.getenv("AZUREML_NB_PATH") is not None else "azuremljupyterlab"
                _kernel_location = "remote"
            elif os.getenv("AZUREML_NB_PATH") is not None:
                app = "azuremljupyternotebook"
                _kernel_location = "remote"
            elif os.getenv("HOME") == "/home/azureuser" and os.getenv("LOGNAME") ==  "azureuser" and os.getenv("USER") == "azureuser":
                app = "azuremljupyterlab"
                _kernel_location = "remote"
            elif (os.getenv("AZURE_NOTEBOOKS_HOST") or os.getenv("AZURE_NOTEBOOKS_SERVER_URL") or os.getenv("AZURE_NOTEBOOKS_VMVERSION")) is not None:
                app = "azurenotebook"
                _kernel_location = "remote"
        ...
        if notebook_service_address is None:
            notebook_service_address = os.getenv("AZURE_NOTEBOOKS_HOST")
        ...
        try:
            ...
            for parsed in parsed_queries:
                ...
                if command is None or command == "submit":
                    ...
                    self.last_execution["query"] = KqlClient.last_query
    ...
    def execute_bug_report_command(self, param:str=None, options:dict={}):
    ...
    def execute_conn_command(self, param:str=None, options:dict={}):
    ...
    def execute_help_on_options(self, topic:str):
...
"""
    ...
    def _execute_query(self, parsed, user_ns: dict, result_set=None, override_vars=None):
        ...
        try:
            ...
            messages = self._get_connection_info(**options) if not connection_string and Connection.connections and not suppress_results else None
            raise ShortError(e, messages)
    ...
    def obfuscate_default_env(self)->dict:
        ...
        for e in env:
            if e  == f"{Constants.MAGIC_CLASS_NAME_UPPER}_CONNECTION_STR":
                val = env[e]
                ...
                env[e] = val
            elif e == f"{Constants.MAGIC_CLASS_NAME_UPPER}_SSO_ENCRYPTION_KEYS": 
                ...
                env[e] = val
            elif e == f"{Constants.MAGIC_CLASS_NAME_UPPER}_DEVICE_CODE_NOTIFICATION_EMAIL":
                ...
                env[e] = val
            elif e == f"{Constants.MAGIC_CLASS_NAME_UPPER}_CONFIGURATION":
                # try_token
    ...
    def _override_default_configuration(self):
        ...
        #
        # KQLMAGIC_NOT_INSTALLED_PACKAGES
        #
        not_installed_packages = os.getenv(f"{Constants.MAGIC_CLASS_NAME_UPPER}_NOT_INSTALLED_PACKAGES")
        if not_installed_packages is not None:
            setattr(self.default_options, "not_installed_packages", not_installed_packages)
        ...
        if warn_missing_dependencies is not None:
            ...
            setattr(self.default_options, "warn_missing_dependencies", not is_false)
        ...
        # KQLMAGIC_NOTEBOOK_SERVICE_ADDRESS
        #            
        notebook_service_address = os.getenv(f"{Constants.MAGIC_CLASS_NAME_UPPER}_NOTEBOOK_SERVICE_ADDRESS")
        if notebook_service_address is not None:
            if notebook_service_address.find("://") < 0:
        ...
        #            
        azureml_compute = os.getenv(f"{Constants.MAGIC_CLASS_NAME_UPPER}_AZUREML_COMPUTE")
        if azureml_compute is not None:
            if azureml_compute.find("://") < 0:
                azureml_compute = f"https://{azureml_compute}"
            setattr(self.default_options, "notebook_service_address", azureml_compute)
