...
class CacheClient(KqlClient):
    ...
    def execute(self, database_at_cluster, query, **options):
        ...
        try:
            ...
            # collect this inormation, in case bug report will be generated
            self.last_query["response"]["status_code"] = 400 # pylint: disable=unsupported-assignment-operation, unsubscriptable-object
            self.last_query["response"]["error"] = str(e) # pylint: disable=unsupported-assignment-operation, unsubscriptable-object
            raise e
