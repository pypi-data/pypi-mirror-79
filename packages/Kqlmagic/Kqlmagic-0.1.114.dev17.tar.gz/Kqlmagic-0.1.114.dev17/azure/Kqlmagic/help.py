...
## {Constants.MAGIC_CLASS_NAME_UPPER}_INSTALL
Specify the install mode. The following modes are supported: 'default' and 'saw' (saw === secure-access-workstation)<br>
SAW installation conatins only dependencies that are allowed to run on SAW. As not all dependencies are installed, some features will be disabled.<br>
Default: 'default' (will install the all dependencies as listed in setup.py)<br>
<br>

## {Constants.MAGIC_CLASS_NAME_UPPER}_NOT_INSTALLED_PACKAGES
Specify dependencies packages that should not be installed. {Constants.MAGIC_PACKAGE_NAME} won't warn on missing packages in the list.<br>
Features that require the missing dependency will be disabled.<br>
Default: None (will install all packages according to the value set in {Constants.MAGIC_CLASS_NAME_UPPER}_INSTALL).<br>
<br>
...
Specified azureml backend compute address. If not specified popup windows might not work properly.<br>
...
```
set {Constants.MAGIC_CLASS_NAME_UPPER}_INSTALL=saw
...
```python
%env {Constants.MAGIC_CLASS_NAME_UPPER}_INSTALL=saw 
!pip install {Constants.MAGIC_PACKAGE_NAME}
```
<br>
