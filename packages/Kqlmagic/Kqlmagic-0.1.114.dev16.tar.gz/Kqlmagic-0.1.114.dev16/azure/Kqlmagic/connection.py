...
class Connection(object):
    ...
    def connection_list(cls):
        return [k for k in sorted(cls.connections) if k.find("@") > 0 and cls.connections[k].bind_url != k]
    ...
    @classmethod
    def get_connections_info(cls, cnames:list=None):
        conn_list = cls.connection_list()
        for cname in (cnames or []):
            if cname not in conn_list:
                raise ValueError(f"connection: '{cname}' not found")
        return {cname: cls.connections[cname].get_details() for cname in conn_list if (cnames is None or cname in cnames)}
    ...
    @classmethod
    def get_current_connection_name(cls):
        return cls.current.get_conn_name() if cls.current else None
