...
from .constants import Constants, Cloud, PrintColor
...
class AuthenticationMethod(object):
    ...
    client_app_type = {
        ...
        aad_device_login:            ClientAppType.public,
    }
...
class _MyAadHelper(object):
    ...
    def _get_token_access_token(self, token:dict, default_access_token:str=None)->str:
        return (token.get(TokenResponseFieldsV1.ACCESS_TOKEN) or
                token.get(TokenResponseFieldsV2.ACCESS_TOKEN) or
                token.get(OAuth2TokenFields.ACCESS_TOKEN) or
                default_access_token)


    def _get_token_client_id(self, token:dict, default_client_id:str=None)->str:
        return (token.get(TokenResponseFieldsV1._CLIENT_ID) or 
                token.get(OAuth2TokenFields.CLIENT_ID) or 
                default_client_id)
    ...
    def _get_token_token_type(self, token:dict, default_token_type:str=None)->str:
        return (token.get(TokenResponseFieldsV1.TOKEN_TYPE) or 
                token.get(TokenResponseFieldsV2.TOKEN_TYPE) or 
                token.get(OAuth2TokenFields.TOKEN_TYPE) or 
                default_token_type)
    ...
    def _get_token_resource(self, token:dict, default_resource:str=None)->str:
        ...
        if resource is None:
            ...
            if scope is not None:
                resource = '/'.join(scope.split()[0].spilt('/')[:3])
    ...
    def _get_token_refresh_token(self, token:dict, default_refresh_token:str=None)->str:
        return (token.get(TokenResponseFieldsV1.REFRESH_TOKEN) or
                token.get(TokenResponseFieldsV2.REFRESH_TOKEN) or
                token.get(OAuth2TokenFields.REFRESH_TOKEN) or 
                default_refresh_token)


    def _get_token_id_token(self, token:dict, default_id_token:str=None)->str:
        return (token.get(OAuth2TokenFields.ID_TOKEN) or
                token.get(TokenResponseFieldsV2.ID_TOKEN) or
                default_id_token)
    ...
    def _migrate_to_msal_app(self, token:dict)->str:
        if token is not None:
            ...
            if refresh_token is not None:
                ...
                app = PublicClientApplication(client_id, authority=authority_uri)
    ...
    def _create_client_app(self, cache=None):
        ...
        if self._client_app_type == ClientAppType.public:
            client_app = PublicClientApplication(self._client_id, authority=self._authority_uri, token_cache=cache)
        elif self._client_app_type == ClientAppType.confidential:
            client_app = ConfidentialClientApplication(self._client_id, authority=self._authority_uri, 
                client_credential=self._client_credential, token_cache=cache)
        return client_app
