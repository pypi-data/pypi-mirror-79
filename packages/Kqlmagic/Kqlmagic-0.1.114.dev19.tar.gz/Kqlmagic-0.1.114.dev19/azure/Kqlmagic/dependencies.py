...
class Dependencies(object):
    ...
    BASE_TAG = "#B#"      # Kqlmagic base faeture, however they may be disabled
    ...
    @classmethod
    def warn_missing_env_variables(cls, options={}):
        if options.get("notebook_app") in ["azureml", "azuremljupyternotebook", "azuremljupyterlab"]:
            if options.get("notebook_service_address") is None:
                ...
                message = f"{cls.MISSING_ENV_TAG}Popup windows might not work properly (you can copy/paste compute host value from the address bar of the notebook when activated in azureml Jupyter mode)"
