...
def docx_report_callback(test_record):
    ...
    #plt.pie(size, labels=names, colors=['green','red','blue','skyblue','orange'])
    ...
    template['graph'] = graph  # adds the InlineImage object to the context
    file_string = str(template['dut_id'])+'_'+str(template['test_name'])+'_'+str(template['test_outcome'])+'.docx'
