...
class GPT2:
	...
	def generateDocument(self, n, selection='nucleus', genIter=None, k=5, prob=0.8, uniq=True):
		...
		with torch.no_grad():
			while len(res) < n:
				...
				for _ in range(maxLen):
					...
					curIds = torch.cat([curIds.long() ,torch.tensor([[nextTokenId]]).long().to(self.device)], dim = 1)
					if nextTokenId in self.__tokenizer.encode(self.__eos):
						break

				doc = self.__tokenizer.decode(list(curIds.squeeze().to('cpu').numpy())[2:-1]).strip()
				if uniq:
		...
		return list(res)
print(__name__)
if __name__ == "genn.gpt2":
	try:
		from pytorch_transformers import GPT2Tokenizer, GPT2LMHeadModel, AdamW, WarmupLinearSchedule
	
	except ModuleNotFoundError:
		raise Exception("Please install pytorch_transformers module first.\n"
					  "Using: pip install pytorch_transformers ")
