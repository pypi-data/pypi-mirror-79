...
def write_anchor(args, synteny_parent=None, mailbox_reader=None, logger=None):
    ...
    if in_anchor > 0:
        anchor_frame.sort_values(by=["syn.shingle.sub", "frag.idx", "frag.pos"], inplace=True)
        anchor_frame.sort_values(by=["syn.shingle.sub", "frag.idx", "frag.pos"], inplace=True)
        for sub_no, subframe in anchor_frame.groupby(by=["syn.shingle.sub"]):
            ...
            anchor_props[f"syn.anchor.{sub_no}.hash"] = hash_array(pd.util.hash_pandas_object(anchor_subframe).to_numpy())
            if write_frame:
                write_tsv_or_parquet(
                    anchor_subframe,
