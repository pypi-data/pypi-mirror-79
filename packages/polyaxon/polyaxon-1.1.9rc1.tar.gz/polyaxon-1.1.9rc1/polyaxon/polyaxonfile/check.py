...
from polyaxon.polyaxonfile.specs import OperationSpecification, get_specification, kinds
...
def check_polyaxonfile(
    ...
    try:
        ...
        for preset_plx_file in polyaxonfile:
            preset_plx_file = OperationSpecification.read(
                preset_plx_file, is_preset=True
            )
            plx_file = plx_file.patch(
                preset_plx_file, strategy=preset_plx_file.patch_strategy
            )
        ...
        if git_init:
            ...
            plx_file = plx_file.patch(git_preset, strategy=V1PatchStrategy.POST_MERGE)
