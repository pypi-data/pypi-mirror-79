...
class HamsterSRecorder:
    ...
    def start(self, file_path='./data.csv'):
        ...
        left_wheels = []
        right_wheels = []
        state = HamsterSRecorder._STATE_IDLE
        hamster = HamsterS()

        while True:
            if state == HamsterSRecorder._STATE_IDLE:
                if self._pressed_key == Key.space:
                    state = HamsterSRecorder._STATE_MOVE
            elif state == HamsterSRecorder._STATE_MOVE:
                diff = (left_floor - 50) * 0.5
                left_wheel = 30 + diff
                right_wheel = 30 - diff
                hamster.wheels(left_wheel, right_wheel)
