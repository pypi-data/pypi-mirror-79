...
class WITS:
    ...
    spm_channels = ["pump_spm_1", "pump_spm_2", "pump_spm_3"]
    status = {}
    ...
    def spm_status(self, low_spm_threshold: float) -> [SPMStatus, None]:
        """
        ...
        if pump_spm_total > low_spm_threshold:
            return SPMStatus.ON

        return SPMStatus.OFF

    def mud_flow_in_status(self, low_mud_flow_in_threshold: float) -> [MudFlowInStatus, None]:
        """
        ...
        if not operations.is_number(self.mud_flow_in):
            return MudFlowInStatus.MISSING

        if self.mud_flow_in > low_mud_flow_in_threshold:
            return MudFlowInStatus.ON

        return MudFlowInStatus.OFF
