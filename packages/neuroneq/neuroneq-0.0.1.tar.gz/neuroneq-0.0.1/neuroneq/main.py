...
class NeuronEQWindow():
    ...
    def parameters_page(self,root):
        ...
        def plot():
            ...
            inf_expression = inf_expression.replace("exp(","np.exp(")
            y = eval(inf_expression)
            self.infline = self.infplot.plot(v,y)
            self.infcanvas.draw()
            ...
            tau_expression = tau_expression.replace("exp(","np.exp(")
            y = eval(tau_expression)
            self.tauline = self.tauplot.plot(v,y)
            self.taucanvas.draw()
        ...
        self.betaplot.title.set_text('Regression')
        #Create the canvas for the membrane vs time graph.
