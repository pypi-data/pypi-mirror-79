...
class ZWMongo(object):
    ...
    def leftjoin(self, coll, coll_right, fld, fld_right, nameas, conds=None, fetchall=False):
        conn = self.get_connection()
        rtn = conn.leftjoin(coll, coll_right, fld, fld_right, nameas, conds, fetchall)
        if fetchall:
            conn.close()
        return rtn
...
class ZWMongoConnection(object):
    ...
    def leftjoin(self, coll, coll_right, fld, fld_right, nameas, conds=None, fetchall=False):
        conds = conds or {}
        self._cursor = self._db[coll].aggregate([{
            '$lookup': {
                'from'          : coll_right,
                'localField'    : fld,
