...
def _repo_name(wid, cid):
    return "{}-{}".format(wid, cid)


def _firestore_asset(document_path):
    return "/".join(document_path.split("/")[5:])


def _firestore_asset_id(document_path):
...
class ParsedRequest:
    def __init__(self, request: Request):
        ...
        self.data = data
        self.repo = _repo_name(data.get("wid"), data.get("cid"))
...
class DBEvent:
    def __init__(self, event):
        ...
        self.asset = "/".join(value["name"].split("/")[5:])
