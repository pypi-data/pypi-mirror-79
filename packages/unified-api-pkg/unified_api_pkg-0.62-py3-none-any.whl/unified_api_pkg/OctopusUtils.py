...
def packRespMsg(language,transData):
    if language == "E":
        if transData["RESP"] < 100000:
            transData["RESPMSG"] = "REMIND BALANCE: $"+transData["REMINBAL"]
        elif transData["RESP"] in OCT_RESPONSE_ENGLISH_MAP:
            transData["RESPMSG"] = OCT_RESPONSE_ENGLISH_MAP[transData["RESP"]]
        else:
            transData["RESPMSG"] = "REMIND BALANCE: $"+transData["REMINBAL"]
    else:
        if transData["RESP"] < 100000:
            transData["RESPMSG"] = "餘額: $"+transData["REMINBAL"]
        elif transData["RESP"] in OCT_RESPONSE_CHINESE_MAP:
            transData["RESPMSG"] = OCT_RESPONSE_CHINESE_MAP[transData["RESP"]]
        else:
            transData["RESPMSG"] = "錯誤 999999"
