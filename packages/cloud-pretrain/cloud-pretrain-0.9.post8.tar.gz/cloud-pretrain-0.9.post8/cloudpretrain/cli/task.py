...
@click.option("-pc", "--priority_class", default = "best-effort", required=True, help="cloudml resource type")
@click.option("-ts", "--test_set", default = "", required=True, help="local test txt, if not set, will use the test.txt in datasets")
...
@click.option("-ds", "--is_decoder", is_flag=True, help="tmp parameters, if input saved model is transformer based decoder, this flag should be true")
...
def serving(job_name, saved_model_folder, deploy_type, replicas, gpu_type, gpu_memory, \
    use_server_batching, serving_name, serving_version, is_decoder, enbale_fast, priority_class, telephone, online):
    ...
    if enbale_fast:
        model.docker_image = FAST_DECODER_MODEL_SERVING_IMAGE if is_decoder else FAST_MODEL_SERVING_IMAGE
