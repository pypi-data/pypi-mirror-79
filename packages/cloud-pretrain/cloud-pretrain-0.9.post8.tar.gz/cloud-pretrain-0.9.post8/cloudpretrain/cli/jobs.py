...
def _show_single_job_type(job_info, job_type):
    ...
    if job_type == Job_type.TEST:
        task_table_header = ["CLOUDML_TASK", "STATUS", "MODEL", "METRICS", "TEST_PATH"]
...
def _get_test_task_info(cloudml_task, job_name):
    ...
    test_path = 'test.txt in your datasets'

    if hasattr(cloudml_task, 'local_test_path') and cloudml_task.local_test_path:
        test_path = cloudml_task.local_test_path

    if cloudml_task.test_metrics is not None:
        return [cloudml_task.name, cloudml_task.state, cloudml_task.model, cloudml_task.test_metrics, test_path]
    ...
    return [cloudml_task.name, state, cloudml_task.model, metrics, test_path]
